...
2. **Discover notebooks** - Use `python scripts/notebook_tools/notebook_tools.py validate {target} --quick` for structure check
3. **Execute** - Use `python scripts/notebook_tools/notebook_tools.py execute {target}` for full execution, or MCP for interactive control
...
Tools available:
- **Scripts (preferred for batch)**: `python scripts/notebook_tools/notebook_tools.py validate`, `python scripts/notebook_tools/notebook_helpers.py execute`
