...
5. **Ledgers de dette partages** : rafraichir les deux (`issue-debt`, `pr-actions`) en pliant l'export du journal + le snapshot precedent comme checkpoint (`python scripts/coordination/debt_ledger.py reduce`, Phase 3 bis). Les observations des lanes arrivent par les dashboards dedies ; le snapshot n'est ecrit que par ai-01.
...
### Phase 3 bis - Ledgers de dette partages (2 dashboards dedies)

1. **Rafraichir, puis decider.** Les deux ledgers vivent sur des dashboards workspace **dedies** -- `CoursIA-issue-debt-ledger` (`issue-debt`) et `CoursIA-pr-action-ledger` (`pr-actions`) -- et **jamais** dans un fichier de `$ROOSYNC_SHARED_PATH` (Drive n'a ni verrou ni compare-and-swap : deux ecrivains = last-write-wins, donc des observations perdues en silence). L'adjoint, Hermes et NanoClaw **appendent** une observation (`content` = une ligne `[OBS] {...}`) ; ai-01 est **seul** a ecrire le snapshot, dans la section `status` du dashboard du ledger via `update`/`replace`. Schema, CLI et contrat de checkpoint : [scripts/coordination/README.md](../../../scripts/coordination/README.md).
2. **Tour de cloture issue-debt, sans complaisance.** Les lignes `closeable-now` / `closeable-after-followup` du resume sont une **file de travail, pas un verdict** : pour chacune, verifier l'acceptance **firsthand** (body + criteres, jamais le label), ouvrir une **issue de suivi NOMMEE** pour tout residuel reel **avant** de closer (ou consigner un waiver motive), puis closer. Un « closeable » non reverifie firsthand se traite comme un doute (G.9), pas comme un feu vert.
3. **Ledger PR : deleguer REVIEW_READY, invalider sur mutation de tete.** Les lignes `review-ready` se repartissent a l'adjoint **et** aux bots (Hermes, NanoClaw) ; ai-01 ne re-fabrique pas le dossier. **Toute mutation de tete invalide les observations `head_bound`** (reserves, checks, update-branch, dossier, next-action) : une observation prise contre une tete remplacee est `stale_head`, jamais un verdict -- un push ou un `update-branch` perime donc dossier et reserves du moment.
4. **Update-branch borne — `scripts/ci/update_stale_pr_branches.py` (livre par #16562, branche `feature/16562-auto-update-pr-branches` ; absent de `main` jusque-la).** Le **dry-run est le defaut** (aucun flag a ajouter). Forme appliquee, bornee et nominative : `python scripts/ci/update_stale_pr_branches.py --pr <N> [--pr <M>] --apply --max-updates K`. Le geste **invalide les observations `head_bound`** de la PR (point 3) ; `scripts/ci/merge_dwell.py` reste l'autorite sur le plancher de merge.
...
1. **Gate d'entree AVANT toute lecture personnelle (HARD)** : pour chaque candidate oldest-first, lancer `python scripts/check_adjoint_prevalidation.py <PR>`. **Exit 0 seulement** autorise ai-01 a ouvrir le body, les commentaires, les reviews, les threads ou le diff. Exit 1 (absent, incomplet, BLOCKED, ancien SHA ou surfaces modifiees) = router la candidate a l'adjoint `myia-po-2025:CoursIA-2`, l'exclure de la file personnelle jusqu'a un nouveau dossier exact-head, puis prendre la candidate suivante. Exit 2 = organe injoignable, donc meme refus fail-closed. Le bloc `[ADJOINT PREFLIGHT]` est genere par `python scripts/check_adjoint_prevalidation.py <PR> --template`, puis complete par l'adjoint ; le compte des commentaires exclut le commentaire-dossier lui-meme. **Interdit de contourner le gate par un sous-agent, une lecture API directe ou un ancien dossier d'inbox.**
