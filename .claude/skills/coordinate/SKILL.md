...
1. **Un cycle tient en 1 h a 1 h 30 de travail entre deux crons**, puis la session se rendort. **Cadence courante : 2 h** (mandat user 2026-09-22, apres la reinitialisation de credits Anthropic de la sortie Opus — elle leve la contrainte de consommation du 2026-09-13 qui avait impose 4 h). Le budget de travail par cycle **ne change pas** : c'est un plafond d'effort, pas une fraction de la fenetre. A 2 h, la marge entre deux tirs se resserre — **c'est la delegation qui absorbe**, jamais le grounding ni le dispatch. Verbatim user : « Ca donne entre 1h et 1h30 max de travail entre 2 crons, c'est deja beaucoup je pense, et il ne faudrait pas depasser ca. Sinon c'est un defaut de delegation. »
...
4. **Cron** : `CronList` — si le job coordinateur a disparu (session-only), re-armer `CronCreate("47 1-23/2 * * *", "/coordinate", recurring)` — **2 h, minute off-`:00`** (mandat user 2026-09-22 ; la cadence 4 h du 2026-09-13 repondait a une crise de consommation Anthropic **levee** par la reinitialisation de credits). Le jitter evite de frapper l'API a la meme seconde que le reste de la flotte. **Le cron est session-only** : il meurt avec la session, donc `CronList` en Phase 2 n'est pas une formalite — c'est la seule chose qui distingue « la cadence tourne » de « je le crois ». Cadence unique, PAS de 2e cron ni ScheduleWakeup en plus — un cycle plus long que sa cadence annule deja ses propres declenchements, en empiler un second ne fait qu'ajouter de la conso.
