...
5. **Le rouge sans lane est a MOI.** Le garde "reparer son rouge d'abord" ([proactive-coordination](../../rules/proactive-coordination.md) R5) renvoie chaque lane sur ses propres PRs bloquees — mais une PR **sans tag `Grain:` lisible** n'est imputable a aucune lane et reste donc invisible a tous les gardes (7 des 23 bloquees de plus de 24 h, le 2026-08-22). Personne ne viendra les reprendre : les lister en fin de passe de merge (`python scripts/pick_idle_grain.py --lane myia-ai-01:CoursIA --json`, champ `unattributed_blocked`) et les traiter — reparer, dispatcher nommement, ou fermer en le disant. Le coordinateur est soumis au meme garde pour **sa propre** lane : son tirage sort en code 2 tant que ses PRs rouges trainent.
