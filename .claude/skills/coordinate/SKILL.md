...
4. **Update-branch borne — `scripts/ci/update_stale_pr_branches.py` (desormais sur `main`).** Le **dry-run est le defaut** (aucun flag a ajouter). Forme appliquee, bornee et nominative : `python scripts/ci/update_stale_pr_branches.py --pr <N> [--pr <M>] --apply --max-updates K`. Le geste **invalide le dossier et les reserves** de la PR (point 3) ; `scripts/ci/merge_dwell.py` reste l'autorite sur le plancher de merge.
