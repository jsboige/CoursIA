...
3. **Dossier par PR : deleguer, invalider sur mutation de tete.** La prevalidation d'une PR se delegue a l'adjoint **et** aux bots (Hermes, NanoClaw) ; ai-01 ne re-fabrique pas le dossier. **Toute mutation de tete invalide le dossier, les reserves, les checks et la prochaine action** : une observation prise contre une tete remplacee est perimee, jamais un verdict -- un push ou un `update-branch` rend donc caducs dossier et reserves du moment.
4. **Update-branch borne — `scripts/ci/update_stale_pr_branches.py` (livre par #16562, branche `feature/16562-auto-update-pr-branches` ; absent de `main` jusque-la).** Le **dry-run est le defaut** (aucun flag a ajouter). Forme appliquee, bornee et nominative : `python scripts/ci/update_stale_pr_branches.py --pr <N> [--pr <M>] --apply --max-updates K`. Le geste **invalide le dossier et les reserves** de la PR (point 3) ; `scripts/ci/merge_dwell.py` reste l'autorite sur le plancher de merge.
