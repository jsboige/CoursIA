...
## Ce que l'instrument de dossier ne voit pas — identité et position

Six tells mesurés le 2026-09-26, tous sur la frontière entre ce que l'émetteur croit avoir posé et ce que l'organe lit réellement.

- **Un dossier posté sous une identité TIERCE est structurellement inerte, et il périme par SURFACE les dossiers calculés avant lui (mesuré 2026-09-26, #17062)** : `_is_own_later_act` neutralise **inconditionnellement** les commentaires des **deux** logins neutres (`COORDINATOR_LOGIN` et `SHARED_GITHUB_LOGIN`, #16883) ; tout autre compte est donc une **surface neuve** au sens du `surfaces-sha256`, y compris quand le commentaire est lui-même un dossier valide sur le fond. Le trou est **silencieux côté émetteur** — aucune erreur au POST, rien dans le retour de `gh` — et le gate rend `NO-DOSSIER -- discussion surfaces changed` avec `comments-reviewed is stale: dossier=15, live=17`. Coût mesuré : **9 dossiers** postés sous un compte étranger sur 3 PRs d'une même lane, **0 verdict READY**. Parade : épingler `GH_TOKEN=$(gh auth token --user jsboige --hostname github.com)` avant tout POST de dossier ; jamais `gh auth switch`.
- **Le littéral d'en-tête est exigé EXACT, en PREMIÈRE ligne (mesuré 2026-09-26, #17062)** : un en-tête qui se suffixe **à l'intérieur des crochets** — `[ADJOINT PREFLIGHT — réponse à la réserve …]` — n'est reconnu par aucun parseur : le gate rend `NO-DOSSIER` et `_strip_adjoint_dossier` ne le voit pas non plus. La conséquence est **distincte** de celle de la variante inline (qui, elle, fabrique un faux HEAD-DIFF) : ici l'émission est **perdue**, pas mal lue. Un suffixe explicatif va **après** `[/ADJOINT PREFLIGHT]`, jamais dans l'en-tête.
- **Deux organes ne s'accordent pas sur l'ENDROIT du tag `Grain:` (mesuré 2026-09-26, #17921)** : `variation-protocol` §1 exige le tag en **première** ligne, tandis que le garde de genre parse le body **entier** (`grain_tag.parse_grain_tag`). Un tag posé en **dernière** ligne produit donc un verdict **scindé** : le garde de genre l'évalue pleinement — il nomme lane, genre et prédécesseur, et prononce le ban G-VAR-3 — pendant que la conformité de position rougit dans `Always-on guards`. Ne pas lire ce rouge comme un défaut de contenu : c'est un **déplacement de ligne**, et le détail du job vit dans son log (le summary du check-run, lui, est vide).
- **Un commentaire d'AUTOMATISATION périme un dossier sans aucun geste humain, et c'est ce qui tue les dossiers des PRs jeunes (mesuré 2026-09-26, #17920)** : `surfaces_fingerprint` hache **tous** les commentaires de la discussion, et `_is_own_later_act` ne neutralise que les deux logins coordonnateur. Un compte de bot (`github-actions[bot]`) est donc une **surface neuve** au même titre qu'un tiers. Trois dossiers successifs sur #17920 (`08:05:38Z`, `08:56:29Z`, `09:44:54Z`) ont tous été périmés par l'automatisation du dépôt elle-même — `<!-- PR-PATH-COLLISION:START -->` (`10:02:54Z`) puis `<!-- trivial-diff-15740 -->` (`10:15:23Z`) — le gate rendant `discussion changed after dossier` à chaque fois, sans qu'aucun humain n'ait rien écrit. **Ce n'est pas une cadence** : le collisionneur est idempotent à marqueur, et `plan_actions` rend `verb="none"` — **zéro écriture réseau** — quand le corps désiré est identique à l'existant (`render_comment` ne porte aucun horodatage ; seule la note de résolution en porte un), donc son cron `7,27,47 * * * *` ne repostule pas. Le dossier ne meurt que si le **jeu de collisions change**, c'est-à-dire précisément quand un voisin merge. Geste : émettre **après** que le flux de bots a atterri — pas sur une PR qui vient de naître — puis re-mesurer le gate, jamais supposer. Corollaire de lecture : `UNKNOWN -- pull request changed while prevalidation snapshot was read` est une **course** (le snapshot a bougé pendant la lecture, souvent à cause du POST lui-même ou d'un bot), **pas un refus** — re-mesurer rend le verdict réel (mesuré sur #17920 : `UNKNOWN` puis `READY` à la seconde lecture).
- **Le BODY de la PR est une surface comme une autre : une edition de body perime le dossier A TETE IDENTIQUE (mesure 2026-09-26, #17428)** : `surfaces_fingerprint` hache le body **et** les commentaires, et `_is_own_later_act` ne neutralise que des lignes *comment*/*review* — il ne couvre **pas** le body. Un dossier pose a la tete `7bcbc9ec3689` a donc ete perime par la **substitution de body** de sa propre lane, la tete n'ayant pas bouge d'un octet (gate : `NO-DOSSIER -- discussion surfaces changed`). Consequence d'ordre : quand une lane annonce une reecriture de body, **attendre qu'elle l'ait posee** avant d'emettre — un dossier emis avant est perdu, et le re-emettre coute un second geste sur une surface qui vient de bouger.
- **`--template` AUTO-REMPLIT tout ce qui se mesure, donc la seule faute d'emission est le PAYLOAD REUTILISE (mesure 2026-09-26, #17062/#17059/#17064)** : `head`, `comments-reviewed`, `reviews-reviewed`, `threads-reviewed`, `threads-unresolved`, `surfaces-sha256` et les `diff-*` sont **calcules** ; seuls les champs de jugement restent en `REPLACE_WITH_*` (`complete`, `body`, `checks`, `b0`, `scope`, `domain`, `verdict`). Regenerer le template **a l'instant du POST** rend donc le dossier juste **par construction**, et aucun champ de compte ne se remplit a la main. Les **9 dossiers sans un seul verdict READY** postes par une lane tierce sur ces 3 PRs l'ont ete en trois vagues dont deux causes vivent ici — mauvais compte (`jsboigeEpita`), puis comptes **perimes** dans un payload reutilise — le POST reussissant a chaque fois. Corollaire : les comptes d'un dossier ne sont pas une mesure a reporter d'un cycle a l'autre, ce sont des **sorties d'organe**. Avant tout POST : `head -1` = `[ADJOINT PREFLIGHT]` et `grep -c REPLACE_WITH` = 0 ; apres : `errors=[]`, seul verdict qui vaut.
