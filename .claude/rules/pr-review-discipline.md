...
**Tant que le job n'est pas câblé sur le lake de la PR**, B.3 se lit **non applicable** (à écrire explicitement dans le body), jamais comme un gate silencieusement sauté (#8677). **Même règle quand le job est câblé sur le lake mais que ses `target-modules` n'atteignent pas le module modifié** — ni directement, ni par clôture d'imports : un vert hors-cible est visuellement indiscernable d'un vert sur cible dans le rollup, c'est la seule différence qu'un reviewer ne voit pas sans ouvrir le YAML (#8782). B.3 se lit alors **non applicable pour ce module** et doit être **écrit** comme tel dans le body ; le job advisory `target-coverage` rend l'écart lisible dans le log. État du câblage : les lakes câblés sont **exactement les workflows qui appellent** `lean-axiom.yml` (`grep -ln 'lean-axiom' .github/workflows/*.yml`, moins le fichier lui-même) — mesure mécanique, pas un compte recopié. Triage par lake : [docs/reference/lean-axiom-coverage.md](../../docs/reference/lean-axiom-coverage.md).

### C. ML : multi-seed obligatoire

Toute PR claim « BEATS » / « improvement » sur métriques ML/trading **DOIT** inclure : (1) walk-forward 5-fold ; (2) **≥4 seeds** parmi 0/1/7/42/99 ; (3) **la conjonction** edge ≥ 2σ cross-seed **ET** Diebold-Mariano `dm_p_median < 0.05` — les deux conditions requises, pas σ seul. *Contre-exemple inscrit* : `edge_sigma = +19.97σ` avec `DM p = 0.236` n'est **pas** un BEATS (`validate_xrp_dt_holdout.py` holdout_fresh 06/08) — le dénominateur de `edge_sigma` mesure la dispersion *inter-seeds* (reproductibilité de la procédure), pas la significativité de l'edge, donc σ croît sans borne quand les seeds s'accordent sans que l'edge soit réel. Un σ seul sans DM est un flag « noise ». **Le test DM lui-même doit porter sur une perte qui préserve le signe** (`loss_fn="linear"` dans `scripts/dm_test.py`, additif à `mse`/`mae`) : `mse` et `mae` sont symétriques (`(-e)² = e²`, `|-e| = |e|`) et rendent des `dm_stat` / `p_value` **bit-identiques** pour une série et son exact opposé. *Contre-exemple inscrit* (intégration #10232) : série gagnante `e` vs son opposé `-e` contre baseline nulle — sous `mse` `dm_stat = 10.0754` pour les deux, sous `linear` `dm_stat = -0.1771` vs `+0.1771` (signes opposés, donc test discriminant). Pin : `test_dm.py::test_linear_loss_distinguishes_opposite_series` ; (4) comparaison à majority baseline + coûts de transaction (5bps SPY, 10bps crypto) ; (5) **pas de FAANG/Mag7** en training ; (6) verdict honnête « BEATS » / « NO BEATS » / « INCONCLUSIVE » — jamais « promising ».
