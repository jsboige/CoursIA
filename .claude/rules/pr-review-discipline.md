...
3. Lien vers `Proof integrity SUCCESS` (axiom check) — produit par le step CI `Proof integrity (axiom check)` du workflow réutilisable `lean-build.yml` (input `axiom-gate: true`, câblé lake par lake) qui exécute `LeanVerifier.check_axioms(module, fail_on_sorry=True)` : détecte un `sorry` **transitif** (l'axiome `sorryAx` dans la chaîne de dépendances) qu'un `grep -c sorry` ne verra jamais. Reproduction locale : `python3 agent_tests/check_axioms_gate.py <Module> [<Module>...]` depuis la racine du lake (CLI dédié, politique review-gate `fail_on_sorry=True`, `--timeout 300` car le chargement de la closure d'imports Mathlib dépasse à froid le 60s du prover). **Tant que le job n'est pas câblé sur le lake de la PR**, B.3 se lit **non applicable** (à écrire explicitement dans le body), jamais comme un gate silencieusement sauté — un reviewer ne doit pas avoir à choisir entre bloquer sur un artefact impossible et merger sur un gate mort (#8677). Pilote câblé : `knot_lean` (`lean-knot.yml` → `axiom-gate: true`).
