...
3. Lien vers `Proof integrity SUCCESS` (axiom check) — produit par le job CI `proof-integrity` (câblé lake par lake, pilote `knot_lean`) qui exécute `LeanVerifier.check_axioms(module, fail_on_sorry=True)` : détecte un `sorry` **transitif** (l'axiome `sorryAx` dans la chaîne de dépendances) qu'un `grep -c sorry` ne verra jamais. **Depuis le fix parser multiline (PR #8740 c.947, MERGED 2026-07-28), la gate couvre en outre `native_decide.*` et `Classical.choice` en sus de `sorryAx`** — la classe `native_decide` était silencieusement ignorée avant ce fix (le nom d'axiome `<theorem>._native.native_decide.ax_1_1` dépasse la largeur pretty-print de Lean et forçait un retour à la ligne que l'iteration ligne-à-ligne ne capturait pas). Triage des 22 lacs au 2026-07-29 : cf [`docs/reference/lean-axiom-coverage.md`](../docs/reference/lean-axiom-coverage.md) — seul `knot_lean` est câblé aujourd'hui (GREEN post-#8725), `conway_lean` est RED si câblé (whitelist `native_decide.*` recommandée, justification pédagogique dans `Conway/README.md` §15). Reproduction locale : `python -c "from lean_server import LeanVerifier; print(LeanVerifier('<lake-root>').check_axioms('<Module>', fail_on_sorry=True))"` depuis `agent_tests/`. **Tant que le job n'est pas câblé sur le lake de la PR**, B.3 se lit **non applicable** (à écrire explicitement dans le body), jamais comme un gate silencieusement sauté — un reviewer ne doit pas avoir à choisir entre bloquer sur un artefact impossible et merger sur un gate mort (#8677).
...
5. **PR « alignement doc-honesty » (#8052/#3801) : diagnostic C.4 obligatoire.** Toute PR qui ré-aligne une prose/interprétation sur un output committé (titre/body citant #8052/#3801, « match committed output », « reconcile stale », « -> observed/observe », « fabricat », « contradicted ») **DOIT** porter dans le body la section `## Diagnostic dérive` de la règle C.4 ([notebook-conventions.md](notebook-conventions.md)) : POURQUOI l'output a dérivé (cause **a**-env/kernel · **b**-claim antérieure fabriquée · **c**-moteur upstream · **d**-régression dépendance · **e**-stochasticité non-seedée) + verdict `CAUSE_FIXED` / `CAUSE_DOCUMENTED_ONLY` / `CAUSE_INTRINSIC`.

**Refus (`CHANGES_REQUESTED`) si :**
- le body **ne contient pas** `## Diagnostic dérive` — citer #8364 en simple label ne suffit pas, le diagnostic doit être **écrit** ;
- verdict `CAUSE_DOCUMENTED_ONLY` **sans** issue fille `See #N` traitant la cause (= COSMETIC « jambe de bois repeinte », consacre la dégénérescence) ;
- la valeur ré-alignée est un **nombre de perf/timing/accuracy/coût** ET le notebook est **re-exécutable localement** (kernel Python/.NET dispo, règle F) : la valeur doit alors venir d'une **re-exécution fraîche** (`CAUSE_FIXED` avec preuve d'exec), jamais d'un byte-surgical markdown-align sur l'ancien output — enshriner un nombre qui va changer au prochain passage kernel = la dérive même que C.4 interdit. A fortiori si une re-exec est **déjà due** sur ce notebook (Stop-&-Repair leak, cellule cassée) : **fold** l'alignement dans cette re-exec, pas une PR markdown séparée. Incident fondateur : #8479 (MusicGen 02-3, RTF `0.5-2x`→`0.21-0.24x` aligné sur un run non-optimisé alors qu'une re-exec Stop-&-Repair était déjà due).
