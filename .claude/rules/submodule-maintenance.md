...
**Statut de gate par sous-module (mise à jour c.14463 / c.14566 / c.312)** : l'organe externe à la R3 est la **liste des submod avec un workflow fonctionnel** — la substitution R3 s'applique par défaut, sauf si la liste ci-dessous dit « gate acquis ». Une PR de bump qui omet les deux vérifications et qui ne cite pas un submod à « gate acquis » **manque à R3** ; un submod listé à « gate acquis » qui perd son workflow (drift, suppression) **redevient** soumis à la substitution. Le passage d'un submod d'un état à l'autre est lui-même un **geste tracké** : PR dédiée sur le dépôt submod (câblage ou re-câblage), référence dans le tableau ci-dessous, et revue coord pour valider la bascule. **Six états** (cf. tableau) : `Absent`, `Câblé jamais déclenché sur pile`, `Câblé déclenché vert récent`, `Drift / perte de gate`, `Câblé en PR non mergée`, `Câblé déclenché rouge récent`.

**Six états de gate** (mesurés firsthand par la commande de la colonne « vérifié le ») :
...
4bis. **Câblé en PR, non mergé** (`jsboige/MetaGeneticSharp` au 2026-09-08 via PR jsboige/MetaGeneticSharp#53) — un workflow existe sur une **branche de PR** (pas la branche par défaut du submod) et un run vert y a été mesuré, mais la PR n'est pas encore mergée. **État intermédiaire** entre (1) et (3) : le **câblage** est posé et **vérifié vert** sur la branche PR, mais la **promotion** vers la branche par défaut du submod n'a pas eu lieu. Substitution R3 **active** (c'est la branche par défaut qui fait foi), avec **bascule** : un futur merge de la PR promeut le submod vers (3). **NE PAS** confondre avec (3) : un run vert sur une branche de PR ne protège pas le bump suivant tant que la PR n'est pas mergée — un bump qui cite ce run mais omet la substitution **manque R3**. C'est précisément le trou pointé par la review Hermes du 2026-09-08 sur PR #15161 (comment `c5509f9d88af711f3dc8536636fefa3dea7db193`) : « l'état 3 exige un run sur branche par défaut ; un run sur branche PR ne le satisfait pas » — la présente clause 4bis ferme la lacune.
...
|---|---|---|---|---|
| `MyIA.AI.Notebooks/Search/MetaGeneticSharp` | **Câblé en PR, non mergé** (workflow `dotnet-ci.yml` posé sur `feature/dotnet-ci-gate`, run `34179637581` `success` 2026-09-08T02:19:08Z matrix multi-OS Windows + Linux, sur branche `feature/dotnet-ci-gate` SHA `102eec8c` — Windows tous tests verts, Linux build + tests hors `System.Drawing` verts) | vert sur branche PR | OUI (jusqu'au merge de #53 → bascule vers (3)) | #15133 (c.985, 2026-09-08, PR jsboige/MetaGeneticSharp#53, amend c.312) |
...
**Application concrète** : une PR de bump sur `MetaGeneticSharp` qui se contente de citer un SHA upstream **manque R3** tant que A2 n'est pas acquis (#14408). Une PR de bump qui cite un run vert **sur la branche par défaut** du submod **satisfait** A2 et la substitution R3 **cesse** de s'appliquer à `MetaGeneticSharp` (les bumps suivants peuvent omettre les deux vérifications). Une PR de bump qui cite un run vert **sur une branche de PR non mergée** ne satisfait **pas** A2 — c'est précisément l'état 4bis : la substitution R3 reste active jusqu'au merge de la PR submod. Le passage d'« aucun workflow » (1) à « workflow acquis sur branche par défaut » (3) est un **commit sur le submod** (câblage `.github/workflows/dotnet-ci.yml` sur `jsboige/MetaGeneticSharp`) **puis merge sur `main` du submod**, suivi d'une **mise à jour du tableau ci-dessus** dans une PR sur CoursIA-2. Le passage (1) → (4bis) → (3) passe par une PR submod ouverte, mesurée verte, et mergée — pas par un raccourci « j'ai mesuré le vert sur la branche PR donc c'est acquis ».
