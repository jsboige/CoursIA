...
**Conséquence croisée du scope mort — le verrou epic-wide s'applique aussi aux lanes disjointes (#12905)** : réserver un chemin qu'on s'apprête à créer (nouveau lake, nouveau répertoire) est le cas nominal d'une clause `paths:` — mais tant que la cible n'existe pas, le claim est porté epic-wide et **verrouille toute l'issue-parapluie, scopes disjoints inclus** (mesuré sur #12844 : lane scopée vivante et disjointe, bloquée par un claim de lake inexistant). Le verdict BLOCKED de `check_lane_claim.py` nomme désormais cette cause (`DEAD_SCOPE_BLOCKER`) avec les trois échappatoires (worktree à jour / `[RELEASED]` + réservation en prose / `[OVERRIDE]` coordinateur), et le JSON expose le témoin `dead_scope_blockers`. **Pratique écrite** : pour réserver un travail à venir sur une cible vide, ne pas poser de `[CLAIMED] paths:` mort — écrire la priorité en prose et poser le `[CLAIMED]` scopé quand le chemin porte son premier fichier (contournement appliqué sur #12844).
