...
**Conséquence d'un scope entièrement mort — verrou epic-wide, issue-parapluie entière (#12844)** : un claim actif dont le scope est **entièrement** mort ne se contente pas de rester epic-wide (cf. ci-dessus) — il **verrouille toute l'issue-parapluie, scopes disjoints inclus**. Défaut mesuré sur #12844 : lane A scopée **vivante et disjointe**, `blocking=[B]` ; le code portait déjà `query_scope`, `epic_wide_on_umbrella` et un WARN stderr, mais aucun ne reliait à la **conséquence**. Un lecteur du WARN croit « worktree en retard » alors que le claim détient l'issue entière — y compris les scopes disjoints qu'il ne touche pas. **#12927 a corrigé le texte de blocage** ; cette clause écrit la conséquence dans la **règle** pour que le prochain coordinateur la lise sans relire le code. **Geste de sortie** (déjà documenté ci-dessus, #12740) : écrire la cible avec un métacaractère de répertoire (`scripts/notebook_tools/*markdown*`) pour un fichier pas encore créé — jamais avec un glob qui tombe sur un chemin nul qui fait porter la claim epic-wide et verrouille l'issue entière.
