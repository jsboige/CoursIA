...
**Le pool ne se scanne plus à la main — il se TIRE, et le tirage est le PREMIER GESTE DE CHAQUE CYCLE (HARD, mandats user 2026-08-14 et 2026-08-20).** `gh issue list` plafonne à **30 résultats** triés par récence : un scan manuel ne voit que le récent et referme la boucle de monoculture — la troncature demande un organe, pas plus de vigilance (mesures : [détail, section Picker](../../docs/reference/proactive-coordination-detail.md)). Un steering nommé du coordinateur est **un grain de plus dans la main de la lane**, jamais une condition de départ : on le brûle s'il est déjà là, on ne constate pas son absence. Son équilibrage entre familles et genres reste dû au coordinateur (§4 de [variation-protocol.md](variation-protocol.md)).
...
9. **L'absence d'instruction n'est pas un etat (HARD, mandat user 2026-09-12).** Aucun geste du coordinateur — steer, review, capture, arbitrage, merge, reponse a un DM — n'est un **prealable** a produire. Une lane qui n'a rien recu **tire**, et c'est son regime normal, pas un mode degrade. Trois formulations disent la meme faute et sont bannies comme statut de cycle : « j'attends la re-review », « j'attends le merge », « j'attends la decision ». Ce qui attend, c'est la **candidate** ; la lane, elle, est deja sur le grain suivant.

   **Ce que le coordinateur ne peut pas te demander** : un message de sa part qui te suspend sans te dire quoi faire a la place est un **defaut de sa part, pas une consigne** — tu le signales en une ligne et tu tires. Lui non plus n'a pas le droit de l'ecrire ([coordinator-discipline.md](coordinator-discipline.md) R0).
