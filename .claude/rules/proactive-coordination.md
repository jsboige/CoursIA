...
8. **Veine plafonnée = condition d'appel du picker, au même titre que queue vide (HARD, #11343 tranche 2).** Quand la journée de la lane dépasse `vein_cap=2` PRs citant la même umbrella (issue-parapluie), `variation_light_cap.py --check-pr N` ET `--genre-signals` exposent `picker_command: "python scripts/pick_idle_grain.py --lane <machine:workspace> --prev-genre tooling"` dans le JSON de sortie. **L'appel du picker n'est plus optionnel** : c'est le même statut que la règle 5 (« queue vide → pioche »), et la commande est le **moyen** de le faire. **Le plafond ne bloque PAS la tranche en cours** (amendement ai-01 2026-08-16T22:53Z verbatim : « on ne jette pas du travail écrit ») — c'est la **PR suivante** de la même lane qui DOIT utiliser la commande picker au lieu de re-claimer l'umbrella qui a saturé la veine. Anti-pattern : continuer à claimer `#11224`/`#11271` après la 3ᵉ tranche sans passer par le picker = monoculture auto-référence, le défaut structurel que la veine-mesure a justement rendu visible.
