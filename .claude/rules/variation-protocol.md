...
- **G-VAR-3 — Pas deux fois le même GENRE consécutif.** Une lane ne livre pas le même GENRE sur deux grains d'affilée. `guard`→`guard` bloqué ; le suivant doit changer de genre. (Durcit le « après 3 grains similaires » de R6, trop laxiste : la vague se forme dès 2.)
...
- **2ᵉ même-GENRE consécutif** (G-VAR-3 violé) → **HOLD** : « variation-protocol G-VAR-3 : `<genre>`→`<genre>` ; change de genre ». Ne pas merger.
