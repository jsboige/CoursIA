...
- **G-VAR-1 — Plat principal DEEP, dans un genre de CONTENU.** La PR-plancher du cycle (R1 de proactive-coordination) **DOIT** être DEEP **et** porter un genre de la classe CONTENU. Le mandat user du 2026-09-12 durcit ce plancher d'un cran — il lisait « DEEP ou MED », et **MED le tenait presque toujours** (400 des 583 merges de la semaine ; 15 % de DEEP seulement). MED reste pleinement admis **au-delà** du plancher : c'est le tier de la plupart des bons grains, ce n'est simplement plus celui qui porte le cycle. **Le durcissement ne se paie pas en confiance** : exiger un DEEP incite à sur-coter le tier (signal bot `TIER-INFLATION`), et le contre-poids existe déjà — le merge-gate **re-qualifie lui-même** un tag mal dérivé (§3) et le litmus DEEP est objectif, pas déclaratif. **Une LIGHT ne satisfait JAMAIS le plancher ; un genre META non plus, quel que soit son tier.** Le pool global porte toujours du DEEP/MED de contenu : la monoculture vient du choix du plus facile *disponible*, pas d'une absence de substance.
...
- **G-VAR-3 — Pas deux fois le même GENRE LIGHT consécutif.** Sur les genres LIGHT (`guard` · `ledger` · `docs` · `readme` · `test`) : bloqué dès 2 grains consécutifs de même genre, **sauf** par l'exception mécanique ci-dessous. Les genres de CONTENU ne relèvent pas de G-VAR-3 — un spécialiste Lean qui enchaîne deux preuves DEEP distinctes n'est **pas** la monoculture visée.

  **Exception MED/DEEP mesurable (#14357).** Deux grains consécutifs d'un même genre LIGHT sont autorisés **si et seulement si** (i) le **second** est de tier MED ou DEEP, et (ii) les deux grains ne partagent **aucun fichier** (intersection des `files` des deux PRs vide). Le critère est mécanique — rendu par l'organe de comptage (`variation_light_cap.py`, clé `exempt_runs`) **et consommé par le gate bloquant G-VAR-3** (`variation_adjacency_guard.py`, verdict `exempted`), jamais jugé à la main : l'intersection vide des fichiers est le proxy observable de la « substance genuinement distincte ». Un run de ≥3 du même genre LIGHT ne s'exempte que si **chaque paire adjacente** satisfait (i) et (ii). Ce que l'organe ne peut pas lire (files absentes du jeu de comptage) **ne s'exempte pas** — fail-CLOSED. Tell décisif inchangé : le litmus LIGHT — générable en scannant l'instance d'à-côté → bloqué **même sous une étiquette DEEP**.
