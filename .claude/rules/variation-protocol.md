...
- **G-VAR-2 — Budget LIGHT proportionnel : `max(1, grains_mergés_du_jour // 3)`**, par lane et par jour, **toutes catégories LIGHT confondues**. Une lane à 1-5 grains garde l'ancien plafond d'une LIGHT ; à 6 elle en a deux, à 19 elle en a six. Au-delà : la LIGHT attend demain ou cède la place à du DEEP/MED. Le budget se **calcule** — [`scripts/variation_light_cap.py`](../../scripts/variation_light_cap.py) — il ne se déclare pas.
  ...
  **Note d'arbitrage (#11154) — `DEFECT-ALIVE` et dette #11044.** Les PRs `DEFECT-ALIVE` issues de l'Epic #11044 (nits de review jamais levés, mergés quand même) ont la **forme** LIGHT (défaut + 1 ligne + scan-able) mais **pas la fonction** (chacune rembourse une dette de review). Trois options, **non tranchées** par ce protocole — l'arbitrage est explicite à `ai-01`/user :
  1. **Statu quo** — `DEFECT-ALIVE` consomme le budget LIGHT. La dette #11044 se rembourse au rythme du budget de variété, et chaque merge demande un arbitrage manuscrit.
  2. **Exemption de classe** — une PR qui ferme un `DEFECT-ALIVE` mesuré sur `origin/main` et référence #11044 **ne consomme pas** le budget LIGHT. Mitigation : exiger la **preuve de vivacité** (le défaut mesuré sur `main`, pas seulement une remarque retrouvée dans un historique). Risque : étiquette « je tague `DEFECT-ALIVE` et je passe ».
  3. **Budget séparé** — un quota `DEFECT-ALIVE` distinct, dimensionné sur le reste de la dette, qui n'empiète pas sur le budget de variété ordinaire et s'épuise quand la dette est remboursée.

  **En attendant la décision** : option 1 **avec exception écrite et mesure citée** à chaque merge d'un `DEFECT-ALIVE` au cap — l'arbitrage implicite mesuré sur c.108 (po-2023, PR #11101 + #11102) est tenu, pas improvisé. **Cette note d'arbitrage ne modifie pas G-VAR-2** : elle consigne les options et le mode dégradé explicite. Voir [#11154](https://github.com/jsboige/CoursIA/issues/11154) pour le détail du conflit et la justification de la non-tranche.
