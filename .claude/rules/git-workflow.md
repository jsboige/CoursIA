...
- **`gh pr update-branch` ne remet PAS le plancher DWELL à zéro quand le rafraîchissement ne porte aucun contenu** — #15859 décrivait un défaut **réel**, corrigé depuis par #16149. Le plancher (120 min, `scripts/ci/merge_dwell.py`) se mesure depuis `last_authoritative_committed_at` : la date de **committer** du dernier commit qui **modifie le côté PR**. La chaîne first-parent est remontée **au-delà** des fusions de rafraîchissement de base **prouvées content-free**, sous trois conditions **conjointes** : (1) deux parents ; (2) le second parent est **ancêtre de la base** ; (3) l'arbre du merge est **identique à l'auto-merge** des parents (`git merge-tree --write-tree` — CR ai-01 2026-09-16 : la forme des parents ne prouve pas l'absence de contenu). Ce que chaque geste fait au plancher :
...
- **Ce qui reste vrai du DWELL** : sur une PR dont le **seul** rouge est `DWELL`, `update-branch` n'est pas nuisible mais **inutile** — et il déclenche une rafale CI complète sur un pool de runners partagé, donc il n'est pas neutre pour les voisins. Le geste **gratuit** est de rejouer la jambe (`gh run rerun <run_id> --job <job_id>`) : aucun commit, donc aucun ré-armement. Le balayage `pr-gate-stale-sweep.yml` lève seul, mais **sa cadence n'est pas horaire** malgré son cron `7 * * * *` : mesurée **2 h 33 – 5 h 18** entre tirs (#15197), comme le dit le message du gate lui-même. Ce rouge n'est **pas un défaut de la PR** : le gate le nomme `DWELL -- ... leve au premier balayage suivant <ts>`. Ne pas re-pusher pour « réparer » : chaque push **de contenu** re-arme le plancher depuis la nouvelle tête.
