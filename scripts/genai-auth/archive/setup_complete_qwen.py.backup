    ...
    def test_image_generation(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.warning(f"⚠️ Test de génération d'image échoué (code {result.returncode})")
                logger.info("Vérifier les logs du script de test")
                return True  # Ne pas bloquer l'installation
                
        except subprocess.TimeoutExpired:
            logger.warning("⚠️ Timeout test génération (>600s)")
            return True  # Ne pas bloquer l'installation
        except Exception as e:
            logger.warning(f"⚠️ Exception test génération: {e}")
            return True  # Ne pas bloquer l'installation
