#!/usr/bin/env python3
"""
Script de test du workflow Qwen avec le client ComfyUI
...
def main():
    ...
    try:
        if client.test_connectivity():
            print("✅ Connexion réussie au serveur ComfyUI")
        else:
    ...
    try:
        ...
        if is_valid:
            ...
            print(f"❌ Workflow invalide: {len(errors)} erreurs trouvées")
            for error in errors:
                print(f"  - {error}")
            return False
    except Exception as e:
    ...
    # Test 4: Soumission du workflow (optionnel)
