#!/usr/bin/env python3
"""Proof-integrity axiom check, the CI step shared by two callers (#17336).

Extracted VERBATIM from the `Run axiom check` heredoc of
.github/workflows/lean-axiom.yml (reusable, non-matrix lakes) so the matrix
path (.github/actions/lean-axiom -> lean-build.yml ci-matrix) runs the SAME
rule instead of carrying a second copy. The heredoc's own design rule --
"the filter lives in the script (unit-tested) so the gate never carries a
second copy" -- applies to the gate itself too.

Contract (identical to the workflow step it replaces): all inputs arrive via
environment variables, never via cwd --

  MODULES               dotted module list, comma-separated, or '*' (#10889:
                        runtime derivation by walking every *.lean under the
                        project root, exactly what `lake build` compiles)
  PROJECT_PATH          repo-relative lake root
  DISPLAY_NAME          job label
  ALLOW                 comma-separated extra whitelisted axioms
  FAIL_ON_SORRY         'true' (default) gates on transitive sorryAx
  INCLUDE_I18N_SIBLINGS 'true' keeps `_en` siblings in the '*' derivation

Both roots come from GITHUB_WORKSPACE + PROJECT_PATH, never from the cwd
(the #8712 defect). stdlib-only at import time; lean_server and
check_target_coverage are imported lazily from the repo checkout.

Exit 0 = no forbidden axioms (and no transitive sorryAx when gated).
Exit 1 = violation, source gap, or lake-level vacuity -- see per-module
