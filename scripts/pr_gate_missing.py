...
REMEDIATION_RETARGET = (
    ...
    "l'action `edited`, que pr-gate.yml n'ecoutait pas (types par defaut "
    "`opened` / `synchronize` / `reopened`) : aucune fenetre n'a rerendu le "
    "check. Depuis #16624, pr-gate.yml ecoute `edited` -- les NOUVEAUX "
    "retargets declenchent le gate directement ; ce signalement ne concerne "
    "donc que les retargets anterieurs au fix.\n\n"
