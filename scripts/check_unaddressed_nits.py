...
def analyse(pr_data: dict, threads: list[dict], cutoff: datetime) -> dict:
    ...
    for (when, kind, login, body, src) in signals:
        ...
        if src == "review:CHANGES_REQUESTED":
            ...
            if lifted:
                continue
        # #12319 + #12836 : meme regime pour un nit porte par un commentaire
        # ou une review COMMENTED. Une phrase explicite de levee ne compte que
        # si elle vient de l'auteur du nit (ou d'un override coordinateur
        # nomme) ; une re-review APPROVED du meme auteur reste une levee native.
        elif (any(
