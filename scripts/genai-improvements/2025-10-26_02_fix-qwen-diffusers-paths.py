#!/usr/bin/env python3
"""
Script simplifié pour corriger les chemins des modèles Qwen format Diffusers dans ComfyUI
Utilise des commandes plus robustes et mieux structurées
"""
...
def run_docker_cmd(cmd, description=""):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return True  # Retourne True pour succès
        else:
            print(f"❌ {description}: ÉCHEC (code {result.returncode})")
            print(f"Stderr: {result.stderr}")
            return False  # Retourne False pour échec
...
def create_model_symlinks():
    ...
    model_check = run_docker_cmd(
        'test -d /home/jesse/SD/workspace/comfyui-qwen/ComfyUI/models/checkpoints/Qwen-Image-Edit-2509-FP8',
        "Vérification modèle Diffusers"
    )
    
    print(f"🔍 Résultat vérification modèle: {model_check}")
    
    if not model_check:
        print("❌ Modèle Diffusers non trouvé")
        # Diagnostic supplémentaire
        print("🔍 Diagnostic du problème...")
        run_docker_cmd('ls -la /home/jesse/SD/workspace/comfyui-qwen/ComfyUI/models/checkpoints/', "Listage répertoire checkpoints")
        return False
    
    # Créer répertoires standards s'ils n'existent pas
    print("📁 Création répertoires standards...")
    run_docker_cmd('mkdir -p /home/jesse/SD/workspace/comfyui-qwen/ComfyUI/models/vae', "Répertoire VAE")
    run_docker_cmd('mkdir -p /home/jesse/SD/workspace/comfyui-qwen/ComfyUI/models/unet', "Répertoire UNET")
    run_docker_cmd('mkdir -p /home/jesse/SD/workspace/comfyui-qwen/ComfyUI/models/text_encoders', "Répertoire Text Encoders")
    
    # Supprimer anciens liens symboliques
    print("🗑️ Nettoyage anciens liens...")
    run_docker_cmd('rm -f /home/jesse/SD/workspace/comfyui-qwen/ComfyUI/models/vae/qwen_image_vae.safetensors', "Suppression VAE")
    run_docker_cmd('rm -f /home/jesse/SD/workspace/comfyui-qwen/ComfyUI/models/unet/qwen_image_edit_2509_fp8_e4m3fn.safetensors', "Suppression UNET")
    run_docker_cmd('rm -f /home/jesse/SD/workspace/comfyui-qwen/ComfyUI/models/text_encoders/qwen_2.5_vl_7b.safetensors', "Suppression CLIP")
    ...
    run_docker_cmd(vae_cmd, "Création lien VAE")
    ...
    run_docker_cmd(unet_cmd, "Création lien UNET")
    ...
    run_docker_cmd(clip_cmd, "Création lien CLIP")
    
    # Vérifier les liens créés
    print("🔍 Vérification des liens créés...")
    run_docker_cmd('ls -lh /home/jesse/SD/workspace/comfyui-qwen/ComfyUI/models/vae/', "Vérification VAE")
    run_docker_cmd('ls -lh /home/jesse/SD/workspace/comfyui-qwen/ComfyUI/models/unet/', "Vérification UNET")
    run_docker_cmd('ls -lh /home/jesse/SD/workspace/comfyui-qwen/ComfyUI/models/text_encoders/', "Vérification CLIP")
...
def test_workflow():
    ...
    sys.path.append('d:/Dev/CoursIA')  # Ajouter le chemin local pour les imports
