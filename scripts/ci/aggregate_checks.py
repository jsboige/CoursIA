...
#
# Le check-run `name` est le nom du JOB (cf `ci-required-aggregator.yml`,
# `jobs.aggregate-checks.name = "Aggregate upstream check-runs"`). On
# matche ce qu'on a vu dans l'API firsthand : "Aggregate upstream
# check-runs" est le nom canonique, "aggregate-checks" est un alias
# défensif pour les exécutions où le workflow a été renommé ou copié.
AGGREGATOR_SELF_KEYS = ("aggregate upstream check-runs", "aggregate-checks")
...
def is_self(name: str | None) -> bool:
    ...
    Match against the canonical job names emitted by this aggregator's
    workflow (see `AGGREGATOR_SELF_KEYS`). Substring + case-insensitive
    to tolerate display renames. Without this filter, the aggregator
    sees its own prior runs (conclusion=failure when it BLOCKed on a
    pending check) and re-emits BLOCK indefinitely -- caught at
    c.1331+42-L1 ★★.
    """
    if not name:
        return False
    n = name.lower()
    return any(key in n for key in AGGREGATOR_SELF_KEYS)
