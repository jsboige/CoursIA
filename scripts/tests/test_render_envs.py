...
#
# Builds a synthetic services tree under tmp_path and asserts:
# - .env is auto-created when a service has a compose file that references
#   a SECRET_KEY present in master.env;
# - .env is NOT created when no compose file exists or when the compose file
#   references no SECRET_KEY;
# - existing .env files are left untouched (sync's territory);
# - the helper returns None when master.env is absent.
# --------------------------------------------------------------------------- #
