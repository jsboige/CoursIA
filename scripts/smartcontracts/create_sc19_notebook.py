...
"| **Hook** | Programme WASM execute lors d'une transaction (smart contract) |", "intro-concepts"))
...
'        print(f"  {key:20s} : {val}")', "intro-comparison-code"))
...
"(flexibilite limitee comparee a Ethereum).", "intro-interpretation"))

# === SECTION 2: SETUP xrpl-py ===
cells.append(md(
"---\n"
"\n"
...
"via un **faucet** (robinet) pour experimenter sans risque.", "setup-intro"))
...
'    print("Les cellules suivantes ne fonctionneront pas sans cette bibliotheque.")', "setup-check"))
...
"\n"
"**Unites** : 1 XRP = 1 000 000 **drops** (l'unite atomique du XRPL).", "setup-interpretation"))

# === SECTION 3: XRP TRANSACTIONS ===
cells.append(md(
"---\n"
"\n"
"## 3. Transactions XRP\n"
...
"par le consensus en 3-5 secondes.", "tx-intro"))
...
'    print("  Frais  : 12 drops (0.000012 XRP)")', "tx-payment-code"))
...
'    print("  - Pas de trust line = impossible de recevoir cet IOU")', "tx-trustline-code"))
...
'    print("  - Pas de frais de gas supplementaires")', "tx-offer-code"))
...
"(avant l'arrivee des Hooks, cf. section 5).", "tx-interpretation"))

# === SECTION 4: PAYMENT CHANNELS ===
cells.append(md(
"---\n"
"\n"
...
"de transaction on-chain seraient prohibitifs.", "channel-intro"))
...
'print("-> Latence quasi-nulle pour les claims off-chain")', "channel-simulation-code"))
...
'    print("  - Streaming de paiements (IoT, API, contenu)")', "channel-real-code"))
...
"Bob de reclamer des fonds apres la fermeture.", "channel-interpretation"))
...
'print("  5. Desinstallation via SetHook (wasm vide)")', "hooks-code"))
...
"lies aux **paiements** : limites, compliance, routage automatique.", "hooks-comparison"))
...
"transfrontalier** avec conversion automatique de devises.", "exercise-intro"))
...
'# except Exception as e:\n'
'#     print(f"Erreur : {e}")', "exercise-code"))
...
for i, cell in enumerate(cells):
    ...
    if isinstance(src, str):
        ...
        issues += 1
if issues == 0:
    print("Format OK (toutes les sources sont des listes de lignes)")
else:
