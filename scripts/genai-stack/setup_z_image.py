...
def main():
    ...
    # Chemins de base (robuste)
    current_dir = Path(__file__).resolve().parent
    # Si lancé depuis scripts/genai-stack
    if current_dir.name == "genai-stack":
        repo_root = current_dir.parent.parent
    else:
        # Fallback au dossier courant (si lancé depuis la racine)
        repo_root = Path(os.getcwd())

    base_dir = repo_root / "docker-configurations"
    models_dir = base_dir / "shared/models"
    workspace_dir = base_dir / "services/comfyui-qwen/workspace"
    
    if not base_dir.exists():
        print(f"❌ Erreur: Impossible de trouver le dossier 'docker-configurations' à {base_dir}")
        return
