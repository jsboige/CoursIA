...
Companion notebooks (`Search-02-Uninformed.ipynb` + `Search-03-Informed.ipynb`, Search series, Part1-Foundations): pedagogical presentation + BFS-vs-A* counter-example on a weighted graph. The historical `Exploration_non_informée_et_informée_intro.ipynb` (formerly at Search root, archived 2026-07-03 in `../archive/`) covered the same material in a single document. The notebook wiring belongs to the Search series owner.
