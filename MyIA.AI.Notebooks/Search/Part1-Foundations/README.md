...
| 5 (.NET) | [Search-5-GeneticAlgorithms (C#)](Search-5-GeneticAlgorithms-Csharp.ipynb) | .NET (C#) | **Jumeau .NET** : algorithme générique from-scratch `GeneticAlgorithm<T>` (sélection roulette/tournoi, crossover 1-point/arithmétique, mutation bit-flip/gaussienne), élitisme, taux de mutation adaptatif — port C# fidèle validé sur OneMax (convergence 20/20) et Rastrigin (f(x)≈1.3, optimum local évité) | ~1h |
...
| 10 (.NET) | [Search-10-SymbolicAutomata (C#)](Search-10-SymbolicAutomata-Csharp.ipynb) | .NET (C#) | **Jumeau .NET** (tranche 1) : automates finis classiques — classes DFA/NFA hand-rolled (`Dictionary<(string,char),string>` déterministe vs `Dictionary<(string,char),HashSet<string>>` non-déterministe), NFA « contient ab » (acceptation multi-chemins), DFA nombre pair de 'a' (parité), opérations ensemblistes (complément par inversion des finaux sur DFA complet, intersection par produit cartésien vérifiée = AND logique) — port C# fidèle §1-2, tranche 2 (symbolique Z3 avec `Microsoft.Z3`) à suivre | ~45 min |
