...
| 10 (.NET) | [Search-10-SymbolicAutomata (C#)](Search-10-SymbolicAutomata-Csharp.ipynb) | .NET (C#) | **Jumeau .NET** (tranches 1+2) : automate fini classiques (DFA/NFA hand-rolled, opérations ensemblistes par produit cartésien) **+** automates symboliques avec `Microsoft.Z3` (NuGet 4.12.2) — classe `SymbolicAutomaton` (transitions = prédicats Z3, décision via solveur SMT), intervalle [10,100], parité, multiples de 5, opérations symboliques (union/intersection/complément via `MkAnd`/`MkOr`/`MkNot` vérifiées exactes) — port C# fidèle §1-4, tranche 3 (vérification de propriétés, lien Sudoku) à suivre | ~1h30 |
