...
| 12 | [Search-15-NetworkX](Search-15-NetworkX.ipynb) | Python 3 | Bibliothèque de graphes `networkx` : `Graph`/`DiGraph`, DFS/BFS, Dijkstra, Bellman-Ford, centralités de degré, composantes connexes, MST, Floyd-Warshall, parité structurelle avec QuikGraph (Search-13) | ~1h |
| 13 | [Search-16-QuikGraph](Search-16-QuikGraph.ipynb) | .NET (C#) | Bibliothèque de graphes QuikGraph 2.5.0 (NuGet) : AdjacencyGraph / BidirectionalGraph / UndirectedGraph, DFS/BFS, Dijkstra, Bellman-Ford, centralités de degré, composantes connexes, Edmonds-Karp (flot max), parité avec NetworkX | ~1h |
...
> **Note de numérotation.** Suite à l'issue [#5224](https://github.com/jsboige/CoursIA/issues/5224) (collision de numérotation entre les deux parties), cette Partie 1 utilise **15-16** pour les **bibliothèques de graphes** (parité `networkx` ↔ `QuikGraph`) et libère les slots **12-14** pour la [Partie 3 (Recherche avancée)](../Part3-Advanced/README.md), qui les réserve à présent aux **heuristiques avancées** (`Search-12-PatternDatabases`, `Search-13-LimitedDiscrepancySearch`, `Search-14-WeightedAstar`). Quand un lien renvoie à `Search-12`, `Search-13` ou `Search-14`, la partie d'appartenance lève l'ambiguïté.
