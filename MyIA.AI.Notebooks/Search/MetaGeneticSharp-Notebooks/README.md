...
Neuf notebooks .NET Interactive (C#) qui reconstruisent les métaheuristiques publiées à partir de **primitives composables** plutôt que comme des boîtes noires importées. Le fil conducteur : un algorithme doit pouvoir s'énoncer en quelques lignes déclaratives, et chaque brique (sélection, croisement, mutation, réinsertion) doit pouvoir être interceptée et recomposée.
...
L'arc pédagogique va de l'application *d'une* métaheuristique à la **composition** de plusieurs, puis à la **structuration** de la population, à la **reconstruction** explicite des métaheuristiques publiées (WOA, EO, FBI) depuis leurs primitives, à la **comparaison** honnête sur des benchmarks standard, et enfin à la **généralité** de la grammaire (représentation combinatoire) et à la **visualisation** des paysages de fitness, y compris sur un relief terrestre réel.
...
| 5 | [MGS-5-CompoundMetaheuristics](MGS-5-CompoundMetaheuristics.ipynb) | Construire les composés publiés | `IfElse`, `Match`, `GenerationMetaHeuristic`, factory `MetaHeuristicsService` |
| 6 | [MGS-6-Benchmarks](MGS-6-Benchmarks.ipynb) | Comparaison honnête | `KnownFunctions`, composé WOA vs Uniform vs Islands |
| 7 | [MGS-7-TSP](MGS-7-TSP.ipynb) | Grammaire agnostique à la représentation | `TspFitness`, `OrderedCrossover`, permutation + `Islands` |
| 8 | [MGS-8-LandscapeExplorer](MGS-8-LandscapeExplorer.ipynb) | Visualiser la surface de fitness | heatmaps PNG graphiques, 3 modes (fonction / carte d'altitude / image), trajectoire de convergence |
| 9 | [MGS-9-EverestRelief](MGS-9-EverestRelief.ipynb) | Relief terrestre réel comme paysage | `DemGrid` (DEM), interpolation bilinéaire, GA/WOA/EO vs PSO |
...
D'où viennent les algorithmes « publiés » (WOA, EO, FBI) ? Ce notebook les **reconstruit depuis les primitives** et montre que le raccourci factory (`MetaHeuristicsService.CreateMetaHeuristicByName`) n'est pas une boîte noire : il renvoie exactement l'arbre de primitives que la méthode `BuildMainHeuristic()` de chaque classe assemble — preuve apportée par le type racine (`IfElse`/`Match`/`Generation`/`Island`). Pour chaque composé du catalogue (`KnownCompoundMetaheuristics`), le même parcours en trois temps : (1) la fiche descriptive dans l'esprit de `mealpy`, (2) la reconstruction lue transparentement depuis `IfElse`, `Match`, `GenerationMetaHeuristic` et les croisements géométriques, (3) le raccourci factory et la preuve d'équivalence. Trois exercices : la variante `WhaleOptimisationNaive`, un hybride EO-then-WOA (qui n'existe dans aucune bibliothèque monolithique), et la preuve d'équivalence factory <-> classe. Fitness test définies en inline (Sphere/Rastrigin/Rosenbrock) pour garder le notebook autonome.
