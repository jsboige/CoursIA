...
| 4 | [Search-15-CombinatorialDiscrepancy](Search-15-CombinatorialDiscrepancy.ipynb) | Python 3 | Discrépance combinatoire : colorer ±1 pour ne pas concentrer — borner inf `√n/2`, Beck–Fiala `2k−1` implémenté (variables flottantes), CP-SAT en oracle exact vs garantie pire-cas, frontière Spencer/Banaszczyk/Bansal–Jiang 2025 | ~1h | — |

Les trois premiers notebooks (Search-12, Search-13, Search-14) répondent à la même question — « l'heuristique ne suffit pas à résoudre à l'optimum » — et forment un **triptyque** aux stratégies complémentaires : Search-12 *renforce* l'heuristique (précalcul d'une PDB plus informée, **en gardant l'optimalité**), Search-13 *borne les écarts* à l'heuristique dont on dispose (parier que l'optimum est un proche voisin du chemin greedy, **toujours optimal**), et Search-14 *relâche l'optimalité de façon contrôlée* (accepter une solution au plus $W$ fois l'optimal pour explorer bien moins de nœuds). Les deux premières relèvent la qualité de l'estimation ou de la recherche sans lâcher l'optimum ; la troisième change de contrat — une **borne de qualité paramétrable** contre un gain de vitesse, jusqu'à l'extrême non borné du Greedy.

**Search-15 change de terrain.** Le quatrième notebook quitte la recherche d'un chemin pour une question distincte mais parente : *colorer* un système d'ensembles en `±1` pour qu'**aucune** somme colorée ne concentrate — la discrépance combinatoire. Il est là parce que la série Search est le lieu des algorithmes exacts et des bornes, pas seulement de l'exploration d'états, et il ouvre vers le lake [`discrepancy_lean`](../discrepancy_lean/README.md) (issue #12823). Attention à la **désambiguïsation** : « discrepancy » ici n'a rien à voir avec la Limited Discrepancy Search de Search-13.
...
| Search-15 (discrépance combinatoire) | Beck, J., & Fiala, T. (1981) — « Integer-making theorems », *Discrete Applied Mathematics*. La borne `2k−1` (via variables flottantes) ; Spencer (1985) — « Six standard deviations suffice », *TAMS* : `6√n` ; Banaszczyk (1998) ; Bansal, N., & Jiang, H. (2025) — *arXiv:2508.03961* : `Õ(√k + √log n)` par découplage d'indépendance spectrale affine. |
