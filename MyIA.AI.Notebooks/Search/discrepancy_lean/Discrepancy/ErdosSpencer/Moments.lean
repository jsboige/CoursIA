...
/-!
# Boute p1 — moments de la somme de Rademacher colorée

Module extrait de l'ancien monolithe `ErdosSpencer.lean` (découpage #13508,
Volet 2 — déplacements byte-identiques) : le cœur quantitatif de la méthode
probabiliste d'Erdős–Spencer. Moments de la somme de Rademacher colorée
`Z S = ∑ i, c i • sign (S i)` — `E[Z] = 0`, `E[Z²] = n`, `E[Z⁴] = 3n² − 2n`,
tous uniformes en la coloration — Paley–Zygmund discret et minoration de
queue `prob_tail_ge_of_isColoring`. La borne explicite et ses corollaires
vivent dans `Discrepancy.ErdosSpencer.LB`.
-/
