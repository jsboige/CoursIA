...
Cette partie se compose de onze notebooks .NET Interactive (C#), hébergés dans ce répertoire et consommant le sous-module voisin [`MetaGeneticSharp/`](../MetaGeneticSharp/) :
...
| 10 | [MGS-10-CenterBias](MGS-10-CenterBias.ipynb) | Biais central vs robustesse au déplacement | banc `CenterBiasBenchmark` (Kudela 2022), composés WOA/EO/FBI/DE/BBPSO vs GA/Random, signature $\Delta$ | ~45 min |
...
MGS-6 montrait qu'aucune métaheuristique publiée ne bat la composition en performance brute ; MGS-10 ajoute une **seconde preuve**, plus profonde, en mesurant un défaut que la performance masque : le **biais central**. Le notebook câble le banc `CenterBiasBenchmark` du fork (protocole *centré-vs-déplacé* de Kudela, 2022) : chaque optimiseur est lancé sur une même fonction (Sphere, Rastrigin, Ackley ; dimension 5, NFE = 5000) en deux versions, l'optimum au **centre** puis **relocalisé** hors centre via `ShiftedFitness`. La signature $\Delta = f^\star_{\text{déplacé}} - f^\star_{\text{centré}}$ révèle si l'optimiseur résout la fonction *où que soit l'optimum* ($\Delta \approx 0$, non biaisé — la recherche aléatoire uniforme en est le contrôle canonique) ou s'il n'exploite que le placement central ($\Delta \gg 0$, biaisé vers le centre). Sept optimiseurs sont confrontés : **Random** (contrôle *seedé* `seed:7`, la référence non biaisée), **GA**, et les cinq composés géométriques **WOA, EO, FBI, Differential Evolution, Bare-Bones PSO**. Résultat (Kudela 2022, « composants > métaphores ») : les composés **métaphoriques** (WOA, EO, FBI) sont biaisés vers le centre ($\Delta \gg 0$ sur au moins une fonction), tandis que les assemblages **non métaphoriques** (GA, DE) y échappent ($\Delta \approx 0$). Le cas **DE** est décisif : même couche géométrique que WOA/EO/FBI (`GeometricCrossover`, `MatchMetaHeuristic`), mais opérateur d'**arithmétique nue** ($v = r_1 + F \cdot (r_2 - r_3)$) — sa robustesse isole la cause du biais : ce n'est pas la composition géométrique qui biaise vers le centre, c'est la **métaphore**. Leçon de lecture (honnêteté, G.9) : un $\Delta$ élevé n'est un *biais* que si l'optimiseur résolvait effectivement le cas centré ; sinon il signe juste de la faiblesse — d'où trois exercices (stabilité multi-graine de la signature, balayage de l'amplitude du déplacement, extension de la suite à Rosenbrock et Griewank).
...
- **La caractérisation du biais central** (MGS-10) — le protocole centré-vs-déplacé (Kudela 2022) révèle que les composés **métaphoriques** (WOA, EO, FBI) sont biaisés vers le centre, tandis que les assemblages **non métaphoriques** (GA, DE) y échappent. **DE**, même couche géométrique que WOA/EO/FBI mais opérateur d'arithmétique nue, isole la cause : la **métaphore**, pas la composition géométrique, biaise vers le centre. Une **seconde preuve** de « composants > métaphores » au-delà de la performance brute (MGS-6).
