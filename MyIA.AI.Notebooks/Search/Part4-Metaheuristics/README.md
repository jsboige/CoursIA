...
Cette partie se compose de dix notebooks .NET Interactive (C#), hébergés dans ce répertoire et consommant le sous-module voisin [`MetaGeneticSharp/`](../MetaGeneticSharp/) :
...
MGS-6 montrait qu'aucune métaheuristique publiée ne bat la composition en performance brute ; MGS-10 ajoute une **seconde preuve**, plus profonde, en mesurant un défaut que la performance masque : le **biais central**. Le notebook câble le banc `CenterBiasBenchmark` du fork (protocole *centré-vs-déplacé* de Kudela, 2022) : chaque optimiseur est lancé sur une même fonction (Sphere, Rastrigin, Ackley ; dimension 5, NFE = 5000) en deux versions, l'optimum au **centre** puis **relocalisé** hors centre via `ShiftedFitness`. La signature $\Delta = f^\star_{\text{déplacé}} - f^\star_{\text{centré}}$ révèle si l'optimiseur résout la fonction *où que soit l'optimum* ($\Delta \approx 0$, non biaisé — la recherche aléatoire uniforme en est le contrôle canonique) ou s'il n'exploite que le placement central ($\Delta \gg 0$, biaisé vers le centre). Huit optimiseurs sont confrontés : **Random** (contrôle *seedé* `seed:7`, la référence non biaisée), **GA**, et les six composés géométriques **WOA, EO, FBI, Differential Evolution, Bare-Bones PSO, Recuit simulé** (Metropolis, 1953 ; Kirkpatrick, Gelatt & Vecchi, 1983). Résultat (Kudela 2022, « composants > métaphores ») : les composés dont l'opérateur s'**ancre à un attracteur géométrique fixe** (WOA, EO, FBI, BBPSO) sont biaisés vers le centre ($\Delta \gg 0$ sur au moins une fonction), tandis que ceux à **exploration arithmétique libre** (GA, DE, Recuit simulé) y échappent ($\Delta \approx 0$). Le cas **DE** est décisif : même couche géométrique que WOA/EO/FBI (`GeometricCrossover`, `MatchMetaHeuristic`), mais opérateur d'**arithmétique nue** ($v = r_1 + F \cdot (r_2 - r_3)$) — sa robustesse isole la cause du biais : ce n'est pas la composition géométrique qui biaise vers le centre, c'est l'ancrage de l'opérateur à un attracteur géométrique fixe (le centre). Le **Recuit simulé** confirme ce raffinement : bien qu'il porte lui aussi une métaphore (physique du recuit, température décroissante), son opérateur est arithmétique — perturbation gaussienne isotrope centrée sur la position courante, acceptée par le critère de Metropolis — donc, comme DE, il échappe au biais central ($\Delta \approx 0$). Ce n'est pas l'étiquette « métaphore » qui biaise, mais la structure de l'opérateur (ancrage géométrique fixe vs exploration arithmétique libre). Leçon de lecture (honnêteté, G.9) : un $\Delta$ élevé n'est un *biais* que si l'optimiseur résolvait effectivement le cas centré ; sinon il signe juste de la faiblesse — d'où trois exercices (stabilité multi-graine de la signature, balayage de l'amplitude du déplacement, extension de la suite à Rosenbrock et Griewank).
...
Les notebooks chargent les DLL via `#r "c:/dev/MetaGeneticSharp/..."` (chemin du checkout de travail du fork). **MGS-1..5** pointent sur `src/MetaGeneticSharp.Domain/bin/Debug/net8.0/` (4 DLL : GeneticSharp.Domain, GeneticSharp.Infrastructure.Framework, MetaGeneticSharp.Domain, MetaGeneticSharp.Infrastructure) ; **MGS-6..9** pointent sur `src/MetaGeneticSharp.Extensions/bin/Debug/net9.0/` (Extensions + SkiaSharp + System.Drawing.Common). Les résultats numériques des notebooks sont **stochastiques** (le RNG du framework n'est pas seedé ; seul le contrôle `Random` de MGS-10 l'est) : les outputs committés sont une exécution valide, les valeurs varient d'une exécution à l'autre. À MGS-10 cela ne concerne que les **nombres** individuels — la caractérisation qualitative (composés métaphoriques biaisés vs assemblages géométriques robustes) est stable d'une exécution à l'autre.
...
Cette quatrième partie a changé la question : non plus *« quelle métaheuristique choisir »* (Parties 1-2), mais *« comment construire et combiner des métaheuristiques à partir de primitives »*. L'arc pédagogique, porté par dix notebooks C# .NET 9 au-dessus de [MetaGeneticSharp](https://github.com/jsboige/MetaGeneticSharp), démontre la thèse **composants > métaphores** :
...
- **La caractérisation du biais central** (MGS-10) — le protocole centré-vs-déplacé (Kudela 2022) confronte huit optimiseurs (Random, GA, WOA, EO, FBI, DE, BBPSO, Recuit simulé) et révèle que les composés dont l'opérateur s'**ancre à un attracteur géométrique fixe** (WOA, EO, FBI, BBPSO) sont biaisés vers le centre, tandis que ceux à **exploration arithmétique libre** (GA, DE, Recuit simulé) y échappent. **DE** isole la cause (même couche géométrique que WOA/EO/FBI mais opérateur d'arithmétique nue), et le **Recuit simulé** la confirme (métaphore physique du recuit, mais opérateur arithmétique → robuste) : ce n'est pas l'étiquette « métaphore » qui biaise vers le centre, mais la structure de l'opérateur. Une **seconde preuve** de « composants > métaphores » au-delà de la performance brute (MGS-6).
