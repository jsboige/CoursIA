...
![Morphogenèse générative : t=0 (germe localisé en rouge sur fond bleu sombre) à gauche, t=6000 (motif auto-entretenu de ~25 taches rougeâtres, structure=0.0095) à droite](assets/readme/ict2-selfsorting.png)

*Figure extraite de `ICT-2-SelfSortingMorphogenesis.ipynb` (cellule 7, output 0) — deux panneaux côte-à-côte illustrant la **morphogenèse générative** par réaction-diffusion (Gray-Scott) : à gauche l'état initial (t=0) avec un germe localisé au centre d'un fond uniforme sombre, à droite le motif auto-entretenu à t=6000 (structure=0.0095, ~25 taches rougeâtres). **Limitation illustrative assumée** : la figure montre le **déclenchement** d'un motif (passage d'un germe localisé à un pattern stationnaire) mais pas la **robustesse aux cellules défectueuses** ni l'auto-réparation, qui sont discutées dans le texte du notebook mais non visualisées ici (c'est le rôle d'ICT-3 et ICT-4).*
...
![Métriques d'auto-réparation : gauche « Inversions (désordre global) » 60→0 en ~220 pas, droite « Sortedness vs erreur de monotonie » (sortedness 0.5→1.0 croissante, erreur locale qui décroît)](assets/readme/ict9-gray-scott.png)

*Figure extraite de `ICT-9-AgencyRegeneration.ipynb` (cellule 3, output 0) — deux panneaux mesurant l'**auto-réparation** : à gauche, le nombre d'inversions (désordre global) décroît de ~60 à 0 en ~220 pas (signature d'un tri monotone) ; à droite, la sortedness croît de 0.5 à 1.0 tandis que l'erreur de monotonie locale décroît (signal d'une mise en ordre locale persistante). **Note d'honnêteté** : le nom de fichier `ict9-gray-scott.png` est trompeur — la figure ne montre **pas** une dynamique de Gray-Scott mais les **métriques de réparation** (inversions / sortedness / erreur de monotonie) ; la dynamique Gray-Scott elle-même est rendue dans `ict2-selfsorting.png`. **Limitation illustrative assumée** : la figure montre la **trajectoire de réparation** dans le morphospace (sortedness globale) mais pas le **gain** de réparation RD vs diffusion (qui est mesuré dans une cellule séparée du notebook comme `repair_gain`).*
