...
class WheelStrategyAlgorithm(QCAlgorithm):
    """
    Wheel Strategy on SPY -- Pedagogical reference for the ESGF course.

    Concept: continuously sell out-of-the-money cash-secured puts on SPY.
    If assigned (PUT exercised), sell out-of-the-money covered calls until
    called away. Repeat. The 'wheel' generates option premiums while the
    underlying tends to move up over time.

    Filters and risk management:
    - VIX > 20 -> skip selling new PUTs (volatility filter)
    - 21 days-to-expiry, 5% OTM strike
    - Cash account, max 80% capital exposure

    Backtest: 2015-2024 ($1M capital, IBKR Cash, minute resolution).
    Note: option backtests are heavy -- expect ~2-5 minute compute time.
    """
    ...
    def Initialize(self):
        ...
        self.days_to_expiry = int(self.GetParameter("days_to_expiry", 21))
        self.otm_threshold = float(self.GetParameter("otm_threshold", 0.05))
        self.max_exposure_fraction = float(self.GetParameter("max_exposure_fraction", 0.8))
