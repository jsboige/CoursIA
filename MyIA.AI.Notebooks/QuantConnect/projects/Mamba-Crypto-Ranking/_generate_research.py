...
cells = [
    ...
    code([
        ...
        "print(f'  d_model=64, d_state=8, n_layers=1 (v2: reduced for CPU convergence)')",
    ]),
    ...
    md([
        "## 3. Gradient Flow Verification (v2)\n",
        "\n",
        "Before training, verify that the selective scan produces non-zero state dynamics.\n",
        "If h.std() ≈ 0, the model collapsed to noise (v1 issue).",
    ]),

    code([
        "# Gradient flow check: verify Mamba selective scan learns\n",
        "np.random.seed(42)\n",
        "torch.manual_seed(42)\n",
        "mamba_check = MambaModel(INPUT_DIM, d_model=64, d_state=8, n_layers=1)\n",
        "dummy_input = torch.randn(4, SEQUENCE_LENGTH, INPUT_DIM)\n",
        "mamba_check.zero_grad()\n",
        "out = mamba_check(dummy_input)\n",
        "loss = out.sum()\n",
        "loss.backward()\n",
        "\n",
        "# Check gradient magnitude in SSM blocks\n",
        "grad_norms = []\n",
        "for name, p in mamba_check.named_parameters():\n",
        "    if p.grad is not None:\n",
        "        grad_norms.append((name, p.grad.norm().item()))\n",
        "\n",
        "# Check hidden state std after forward\n",
        "with torch.no_grad():\n",
        "    out_check = mamba_check(dummy_input)\n",
        "    h_std = out_check.std().item()\n",
        "\n",
        "print(f'Gradient verification (v2):')\n",
        "print(f'  Output std: {h_std:.6f} (target > 0.01)')\n",
        "print(f'  Top-5 grad norms:')\n",
        "for name, gn in sorted(grad_norms, key=lambda x: -x[1])[:5]:\n",
        "    print(f'    {name}: {gn:.6f}')\n",
        "\n",
        "if h_std < 0.01:\n",
        "    print('  WARNING: Output std < 0.01, selective scan may not be learning!')\n",
        "else:\n",
        "    print(f'  OK: Selective scan produces non-trivial output (std={h_std:.4f})')",
    ]),

    md([
        "## 4. Entrainement et Walk-Forward Validation\n",
        "\n",
        ...
        "\n",
        "**v2 changes**: Mamba epochs 20→80, lr 1e-3→1e-4 with warmup,\n",
        "d_state 16→8, n_layers 2→1.",
    ]),
    ...
    code([
        ...
        "                warmup_epochs=0):\n",
        "    \"\"\"Train a model with optional LR warmup and early stopping.\"\"\"\n",
        ...
        "            if logits.dim() == 3:\n",
        "                logits = logits[:, -1, :]  # last timestep for sequence models\n",
        "\n",
    ...
    md([
        "## 5. Walk-Forward Multi-Seed Validation (v2)",
    ]),

    code([
        "EPOCHS_MAMBA = 80  # v2: increased from 20 for CPU convergence\n",
        "EPOCHS_BASELINE = 20\n",
        "BATCH_SIZE = 64\n",
        "INPUT_DIM = len(feature_cols)\n",
        "\n",
        ...
        "        model_configs = [\n",
        "            ('Mamba', MambaModel,\n",
        "             {'input_dim': INPUT_DIM, 'd_model': 64, 'd_state': 8, 'n_layers': 1},\n",
        "             EPOCHS_MAMBA, 1e-4, 10),  # v2: 80 epochs, lr=1e-4, warmup=10\n",
        "            ('DLinear', DLinearModel,\n",
        "             {'seq_len': SEQUENCE_LENGTH, 'input_dim': INPUT_DIM},\n",
        "             EPOCHS_BASELINE, 1e-3, 0),\n",
        "            ('TFT-Lite', TFTLiteModel,\n",
        "             {'input_dim': INPUT_DIM, 'd_model': 32, 'n_heads': 2},\n",
        "             EPOCHS_BASELINE, 1e-3, 0),\n",
        "        ]\n",
        "\n",
        "        for model_name, model_cls, kwargs, epochs, lr, warmup in model_configs:\n",
        "            np.random.seed(SEED)\n",
        "            torch.manual_seed(SEED)\n",
        "            model = model_cls(**kwargs)\n",
        "            model = train_model(model, X_train_s, y_train, epochs, BATCH_SIZE, DEVICE,\n",
        "                                lr=lr, warmup_epochs=warmup)\n",
        "\n",
    ...
    code([
        ...
        "\n",
        "print(f'\\nResume final (v2):')\n",
        "print(f'  Mamba: selective SSM (pure PyTorch, d_model=64, d_state=8, 1 layer)')\n",
        "print(f'  Mamba training: {EPOCHS_MAMBA} epochs, lr=1e-4, warmup=10')\n",
