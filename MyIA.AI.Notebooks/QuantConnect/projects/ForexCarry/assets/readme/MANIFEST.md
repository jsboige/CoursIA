...
| Diagnostic corrélation + rendements | `forex-corr-cumreturns.png` | 1200×465 | 60 Ko | cellule 3 · output 1 | **Dual-panel** (1520×590 natif, downscale + palette 256 couleurs, `plt.subplots(1, 2, figsize=(16, 6))`). **Gauche** : heatmap corrélation 7×7 des rendements FX normalisés (force devise vs USD, inversion USD/XXX). Quasi-uniformément rouge-orange (corrélations 0.5-1.0) → « corrélation élevée = signal momentum dilué par redondance » (commentaire verbatim cellule 3). **Droite** : courbe de rendement cumulé normalisé par devise vs USD sur 2015-2025 — dispersion 0.70 (GBPUSD orange perdant) à ~1.18 (USDJPY marron leader). **Verdict** : univers très directionnel-dollar mais avec une **forte redondance** qui limite le gain marginal d'une position top-2. Cette figure sert de **diagnostic structurel** avant les tests d'hypothèses (h1-h6) — elle motive les filtres testés ensuite. **Note** (c.743) : figure extraite c.743 via `nbformat.read + base64.b64decode`, **absente du dépôt avant cette PR** (MANIFEST c.450 l'avait documentée comme « NON PRÉSENTE dans assets/ » = silent gap C731, hors-scope à l'époque). |

**Total** : 7 figures, 942 Ko. **Politique** (#5654) : ≤200 Ko/fichier, downscale ≤1200 px max. H3/H4 (240–274 Ko natifs, denses) sont downscaled à 800×397 ; H1/H5/H6/synthèse passent natifs (58 Ko, 163 Ko×2, 138 Ko) ; diagnostic corr+cumreturns (271 Ko natif) downscale + palette 256 → 1200×465 / 60 Ko. Arc : diagnostic structurel (cell 3) → baseline FX momentum (h1) → long-only vs L/S (h3) → impact du choix de paires (h4) → filtres DXY (h5) et volatilité/régime (h6) → configuration optimale v3e (synthese).
...
**Note historique (c.450)** : la cellule cell[3] du notebook `research.ipynb` produit une figure **dual-panel matplotlib** (`plt.subplots(1, 2, figsize=(16, 6)`) avec :
- **Gauche** : « Correlation des rendements FX normalises » (heatmap corrélation 7×7)
- **Droite** : « Rendement cumule normalise (devise vs USD) » (7 lignes une par devise)

c.450 avait documenté cette figure comme **NON PRÉSENTE dans `assets/readme/`** — uniquement visible à l'exécution du notebook (inline matplotlib). Conséquence : le MANIFEST ne documentait que 6 figures sur 7 PNG effectivement produites par le notebook.

**c.743** : extraction cell[3]·out[1] (271400B source → 60700B après downscale 1200×465 + palette 256 couleurs, conforme politique #5654 ≤200 Ko / ≤1200 px). PNG désormais présent sur disque en tant que `forex-corr-cumreturns.png`. Cette figure sert de **diagnostic structurel** dans le README (section « Diagnostic préalable » avant les hypothèses h1-h6) — elle motive les filtres testés dans la suite. **Note PR body c.743** : ce silent gap (MANIFEST listait 6/7 mais ne nommait pas la 7ᵉ) est analogue au pattern C731 rattrapage c.742 LongShortHarvest (PNG manquant + prose anti-sur-vente). Amendement : la section « Découverte additionnelle » est renommée « extraite c.743 » pour refléter la résolution du gap.
