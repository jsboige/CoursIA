...
- **bias_har is NOT ≈ 0 after debias** (c.951 said "essentially zero", -0.002 to -0.003). With symmetric calibration on all three models, the residual `bias_har` is **0.171 / 0.056 / 0.109** (h=1/5/10). The c.951 reading came from HAR-only calibration + a 60-bar tail that happened to flatten HAR; under the corrected apples-to-apples protocol, the bias is non-trivial. The identity `mse_har_debiased = mse_har_raw` holds to 4 decimals because the symmetric correction is applied to both — this is a sanity check, not a claim of zero bias.

  **`MSE_har_debiased = mse_har_raw` is now a structural identity by construction** (the 60-bar tail mean is subtracted equally from all three), not an empirical fact about post-tail residuals.
