...
<p align="center">
  <a href="m5_hmm_regime_research.ipynb"><img src="assets/readme/mltp-hmm-regime.png" alt="Scatter HAR-RV-J vs HAR : 18 points coloré par horizon (h=1 vert, h=5 rouge, h=10 bleu) sur axes Δ-MSE et Δ-Sharpe. Pattern pedagogue : HRJ degrade la precision point (Δ-MSE +60 a +240% pour h=10) mais ameliore le sizing (Δ-Sharpe +0.10 a +0.22), quadrant haut-droit peuples par h=10 (sizing sauve la prevision), haut-gauche par h=1 (HRJ gagne des deux cotes)." width="380"/></a>
  <a href="m12_har_rv_j_research.ipynb"><img src="assets/readme/mltp-har-rv.png" alt="Scatter Log-LSTM vs HAR : pattern dense ~75 points (25 configurations × 3 horizons) sur axes Δ-MSE et Δ-Sharpe. Axes croises a (0,0) en gris tirets ; majorite des points a droite de l'axe Y=0 (LSTM perd en MSE, +5 a +25%) mais au-dessus de l'axe X=0 (LSTM gagne en Sharpe, delta +0.05 a +0.15). h=10 bleus plus disperses, h=5 rouges concentres autour de (10%, 0%), h=1 verts repartis sur tout l'axe X. Conclusion visuelle : LSTM est moins bon en prediction point mais meilleur en allocation sizing." width="380"/></a><br>
...
<p align="center">
  <a href="m4_dlinear_vol_research.ipynb"><img src="assets/readme/mltp-dlinear-vol.png" alt="Bar chart DLinear vs HAR par crypto (7 cryptos, Reduction MSE %) : BTC-USD vert dominant 27.5% (seul gain substantiel, mesure sur 2278 jours), tous les autres gris non significatifs (ADA-USD ~4.5%, DOT-USD ~0.8%, ETH-USD ~4.3% sur 1495j, LTC-USD ~7.5%, SOL-USD ~5.5%, XRP-USD ~2.7%, tous sur ~725 jours). Pattern pedagogue : BTC a assez d'historique pour distinguer DLinear du HAR classique (27.5% reduction), les autres cryptos n'ont pas assez de donnees (periode trop courte)." width="380"/></a>
  <a href="m15_lstm_rv_research.ipynb"><img src="assets/readme/mltp-lstm-vol.png" alt="Bar chart HMM regime-switching vs HAR classique (Reduction MSE %) sur 3 horizons × 2 cryptos, baseline 0% ligne horizontale verte tirets. BTC-USD (orange) : +7% (h=1), -11% (h=5), -28% (h=10). ETH-USD (violet) : +10% (h=1), -18% (h=5), -53% (h=10). Pattern pedagogue : la sophistication regime-switching nuit aux horizons longs (passage de benefique court terme a destructif long terme), confirmant la these 'HMM nuit aux longs horizons' documentee dans le README." width="380"/></a><br>
...
<p align="center">
  <a href="m9_tft_vol_research.ipynb"><img src="assets/readme/mltp-tft-vol.png" alt="Boxplot diagnostic ensemble HAR-Kelly : 5 strategies × 35 combinaisons comparees sur Sharpe, ligne horizontale grise tirets a 0 (baseline). K60 (single best) vert mediane -0.65, K30 mu120 bleu clair mediane -0.35 (la moins mauvaise), Vol-target HAR orange mediane -0.85, Ensemble (equal-w) violet mediane -0.80, Buy & hold gris mediane -0.85. Conclusion visuelle : Ensemble (violet) quasi superpose a Vol-target HAR (orange), pas de diversification, juste une moyenne ponderee qui dilue la performance du K60 ; toutes les strategies sous-performent Buy & hold en mediane." width="380"/></a>
  <a href="m11e_ensemble_research.ipynb"><img src="assets/readme/mltp-ensemble.png" alt="Line plot Edge vs n_train (taille du jeu d'entrainement par fold 500-2500+) sur baseline majority-class (ligne pointillee verte horizontale Y=0, edge=0). 4 series principales : BTC h=1 orange + ETH h=1 violet. Catastrophe du fold 1 (708 echantillons, BTC -0.09, ETH -0.075), tendance croissante vers 0 apres ~1500 echantillons, oscillation entre -0.02 et +0.01 sans devenir systematiquement positif. Pattern pedagogue : performance depend fortement de la taille du jeu, fold 1 catastrophe, asymptote a 0 (modele atteint la baseline sans la surpasser)." width="380"/></a><br>
