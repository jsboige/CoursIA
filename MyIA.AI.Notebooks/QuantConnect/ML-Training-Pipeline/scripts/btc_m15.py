...
`evaluate_one_combo` patch in PR #12744).
...
def analyze_one_combo(row: dict) -> dict:
    ...
    patched run (PR #12744) populates all required fields.
    """
    h = int(row["horizon"])
    seed = int(row["seed"])
    ...
    # If raw errors are persisted (PR #12744 patch), use them.
...
def run_smoke(
    ...
    if "har_errors" not in row or "lstm_errors" not in row:
        ...
        out = {
            "ok": True,
            "analyzable": False,
            "reason": "patch not active in the m15 run -- rerun after PR #12744 merge",
