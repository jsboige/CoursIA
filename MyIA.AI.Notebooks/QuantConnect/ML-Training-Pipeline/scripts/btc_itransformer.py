"""iTransformer vs train-only-debiased HAR on BTC daily log-RV (issue #14860).

The leg that #14081 (PatchTST) leaves open: a deep-seq architecture that is
*implemented* in the repo (``train_itransformer.py``, class ``iTransformerModel``)
but never revalidated against a symetrically calibrated, train-only-debiased
baseline. This wrapper reuses the existing pipeline modules without modifying
them:

- ``iTransformerModel`` from ``train_itransformer.py`` (Li et al., ICLR 2024) --
  the CLASS only. The generic CLI targets SPY returns/direction and its
...
- ONE iTransformer per (fold, seed). Normalisation stats (mean, std of log-RV)
  ...
  selection.
- Target = mean future log-RV over the next h days. iTransformer predicts the
...
``test_btc_itransformer.py``). By contrast, subtracting the mean error measured
...
(itransformer_variance / har_debiased_variance), and edge vs the debiased HAR.
...
Usage:
    python btc_itransformer.py --dry-run                      # synthetic log-RV, CPU
    python btc_itransformer.py                                # real BTC sweep
    python btc_itransformer.py --horizons 1 5 --seeds 0 1 7 42 --device cpu
"""
...
DEFAULT_OUT_JSON = str(_HERE.parent / "results" / "btc_itransformer_har_debiased.json")

# Dry-run overrides: deterministic synthetic log-RV, 2 folds, 1 seed, 1 horizon,
# tiny model/epochs, CPU, no download.
DRY_RUN_OVERRIDES = {
    "horizons": [1],
    "seeds": [0],
    "n_splits": 2,
...
def load_btc_rv() -> pd.Series:
    ...
    print(f"[btc_itransformer] BTC: {len(rv)} RV days")
    return rv
...
) -> tuple[iTransformerModel, dict]:
    """Train one iTransformer on a single fold's TRAINING sequences.
...
def walk_forward_itransformer(
    ...
    *,
    ...
    """Expanding walk-forward evaluation of iTransformer on daily log-RV.
    ...
    for fold_idx, (train_end, test_start, test_end) in enumerate(splits):
        ...
        model, info = train_itransformer_fold(
    ...
    forecasts = pd.Series(
        preds_arr, index=pd.DatetimeIndex(dates), name="itransformer_logrv_pred"
    )
    targets = pd.Series(
...
def align_by_timestamps(
    ...
    if not np.allclose(
        ...
        raise ValueError(
            "target mismatch on aligned dates: iTransformer and HAR target "
...
def evaluate_combo(aligned: dict, horizon: int) -> dict:
    ...
    - ``var_ratio`` < 1 means iTransformer is more precise than the debiased
      HAR.
    - ``edge_vs_debiased_har_pct`` > 0 means iTransformer has lower total MSE
    ...
    if len(model_errors) < 10:
        return {
            ...
            "var_ratio_itransformer_over_har_debiased": float("nan"),
        }

    itransformer_decomp = _mse_decomposition(model_errors)
    ...
    model_mse = itransformer_decomp["mse"]
    edge_pct = (
    ...
    var_ratio = (
        float(itransformer_decomp["variance"] / har_decomp["variance"])
    ...
    return {
        "n_aligned": int(len(model_errors)),
        "itransformer_mse_logrv": float(itransformer_decomp["mse"]),
        "itransformer_bias": float(np.mean(model_errors)),
        "itransformer_bias_sq": float(itransformer_decomp["bias_sq"]),
        "itransformer_variance": float(itransformer_decomp["variance"]),
        ...
        "var_ratio_itransformer_over_har_debiased": var_ratio,
...
def aggregate_by_horizon(rows: list[dict]) -> list[dict]:
    ...
    for h, sub in sorted(grouped.items()):
        ...
        var_ratios = np.asarray(
            [r["var_ratio_itransformer_over_har_debiased"] for r in sub], dtype=float
        )
        ...
        results.append({
            ...
            "mean_var_ratio_itransformer_over_har_debiased": float(np.nanmean(var_ratios)),
            "mean_itransformer_mse": float(np.nanmean([r["itransformer_mse_logrv"] for r in sub])),
            "mean_har_debiased_mse": float(
                np.nanmean([r["har_debiased_mse_logrv"] for r in sub])
            ),
            "mean_itransformer_bias": float(np.nanmean([r["itransformer_bias"] for r in sub])),
...
def run_pipeline(
    ...
    *,
    ...
    """Full pipeline: HAR (train-only debias) vs walk-forward iTransformer.
    ...
    rv_idx = rv.index
    print(
        f"[btc_itransformer] {coin}: {len(rv)} RV days, log_rv var={log_rv.var():.4f} "
        f"(dry_run={dry_run})"
    )
    ...
    for h in horizons:
        ...
        for seed in seeds:
            itr_out = walk_forward_itransformer(
                log_rv_arr, rv_idx,
            ...
            if "itransformer_mse_logrv" not in metrics:
                rows.append({
            ...
            rows.append(row)
            print(
                f"  h={h} seed={seed} iTransformer MSE={metrics['itransformer_mse_logrv']:.5f} "
                f"bias={metrics['itransformer_bias']:+.5f} "
                f"edge={metrics['edge_vs_debiased_har_pct']:+.2f}% "
                f"var_ratio={metrics['var_ratio_itransformer_over_har_debiased']:.3f} "
    ...
    if aggregated:
        print("\n=== iTransformer vs debiased HAR (BTC) ===")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="iTransformer vs train-only-debiased HAR on BTC daily log-RV "
                    "(issue #14860)"
    )
    ...
    params = dict(
        ...
        calibration_size=args.calibration_size, d_model=args.d_model,
        n_heads=args.n_heads, n_layers=args.n_layers, dropout=args.dropout,
        ff_dropout=args.ff_dropout, epochs=args.epochs,
        ...
        if params["out_json"] == DEFAULT_OUT_JSON:
            params["out_json"] = str(
                Path(DEFAULT_OUT_JSON).with_name(
                    "btc_itransformer_har_debiased_dry_run.json"
                )
            )
