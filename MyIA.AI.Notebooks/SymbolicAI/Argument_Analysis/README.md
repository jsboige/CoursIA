...
| **Fallacy_Rules_Symboliques** | Distiller un détecteur de sophismes symbolique (organe 15 motifs spaCy Matcher / 5 familles), mesurer son rappel réel sur un corpus annoté (3/9 sur `fr_core_news_sm`) et diagnostiquer token par token pourquoi les motifs syntaxiques échouent (virgule, déterminant, tag POS modèle-dépendant) | 35 min |
