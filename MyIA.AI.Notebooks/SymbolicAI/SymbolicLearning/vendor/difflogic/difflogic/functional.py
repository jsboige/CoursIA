...
def bin_op(a, b, i):
    ...
    # SL-12 fork: cast inputs to float before arithmetic. PyTorch does NOT support
    # the `-` operator on two bool tensors (RuntimeError: Subtraction, the `-`
    # operator, with two bool tensors is not supported). The upstream code uses
    # bool inputs in eval mode (model.eval() + bin_op_s with one-hot weights),
    # which triggers the RuntimeError. Casting to float in bin_op is safe: in
    # training mode the inputs are already floats; in eval mode the cast makes
    # the arithmetic work and the result is multiplied by i_s (0/1) so the
    # values stay in {0, 1} when summed.
