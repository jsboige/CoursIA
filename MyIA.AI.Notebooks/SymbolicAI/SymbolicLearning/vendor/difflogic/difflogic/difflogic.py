import torch
# SL-12 fork: guard the difflogic_cuda import. CUDAExtension is only built when
# torch CUDA runtime matches the system CUDA Toolkit version (see setup.py).
# Without this guard, the unconditional `import difflogic_cuda` raises
# ImportError on CPU-only or mismatched-CUDA-Toolkit installs, even when the
# user only needs `implementation='python'` (pure PyTorch fallback).
try:
