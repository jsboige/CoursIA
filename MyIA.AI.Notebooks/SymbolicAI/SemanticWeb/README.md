...
breakdown: SemanticWeb=22
maturity: PRODUCTION=22
-->
...
| Sidetracks Python miroirs (SW-2b/3b/4b/5b/6b/7b) | 6 | PRODUCTION=6, BETA=0 | Équivalent Python des notebooks .NET — rdflib + owlready2 (SW-3b/6b ajoutés depuis le marathon parité #4956) |
| Jumeaux .NET modernes (SW-8-CSharp/9-CSharp/10-CSharp/13-CSharp) | 4 | PRODUCTION=4, BETA=0 | Parité .NET ⇄ Python marathon #4956 (SHACL, JSON-LD, RDF-Star, RDFS Reasoners) — dotNetRDF (`VDS.RDF.Shacl`, `JsonLdParser`, `Graph(IRefNode)`) |
| **Total** | **24** | **PRODUCTION=24, BETA=0** | Double stack .NET C# (dotNetRDF) / Python (rdflib, pySHACL, owlready2, kglab) — 100% production |

> **Note d'audit §E** : `CATALOG-STATUS` (l. 7) annonce `pedagogical_count: 22` — ce chiffre reflète **22 notebooks source** en racine du dossier (les `_output.ipynb` sont des artefacts Papermill sauvegardés à part, cf note bug #5211, pas des notebooks pédagogiques distincts). La table ci-dessus, mise à jour pour la cohérence interne, aligne `breakdown` à 24 entrées visibles (incluant SW-3b/6b + 4 jumeaux C# récents du marathon parité .NET ⇄ Python #4956). Si l'écart CATALOG ↔ breakdown persiste après merge, le cron catalog-cron ou le bot catalog-drift.yml ré-alignera au prochain passage — **cf [catalog-pr-hygiene.md R1](../../../../.claude/rules/catalog-pr-hygiene.md)** : on ne régénère PAS le catalogue sur cette branche.
