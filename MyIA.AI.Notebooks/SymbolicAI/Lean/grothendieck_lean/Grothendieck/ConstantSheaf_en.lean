/-
Grothendieck Part 18 — The constant sheaf (English mirror)
...
`code-style.md` §Lean i18n): this module is the **English mirror** of the
FR-canonical sibling `ConstantSheaf.lean` (Option A, sibling pair model).
The body — signatures, proofs, tactics, theorem names — is byte-identical
between the two files; only the docstrings/comments and the namespace suffix
(`ConstantSheaf` ↔ `ConstantSheaf_en`) differ.
-/
...
namespace Grothendieck.ConstantSheaf_en
...
end Grothendieck.ConstantSheaf_en
