...
operations. This module states and proves them :
  ...
  - `iInf_covering` / `iInf_covers` : the indexed version of the infimum —
    `S ∈ (⨅ i, J i) X` if and only if `S ∈ J i X` for every `i`.
  - `iSup_covering` / `iSup_covers` : the indexed version of the supremum —
    the topology generated by the family `J`.
...
/-!
## Section 4 : indexed families (iInf, iSup)

Sections 2-3 characterize the binary bounds (`inf`, `sup`) and the
set-indexed ones (`sSup`). This section establishes the **indexed** versions :
the infimum of a family `J : ι → GrothendieckTopology C` is the pointwise
intersection of their covering sieves, the supremum the topology generated
by the family. Each indexed bound is the `sInf`/`sSup` of the image
(`Set.range J`) by definition in Mathlib, and the characterizations
reuse `mem_sInf` and `sSup_covering`.
-/

/-- Infimum of an indexed family, pointwise characterization :
    `S ∈ (⨅ i, J i) X` if and only if `S ∈ J i X` for every `i`.
    Proof : `⨅ i, J i = sInf (Set.range J)` by definition, `mem_sInf`
    gives the quantification over members, and membership in `Set.range J`
    decomposes via `Set.mem_range_self` / `Set.mem_range`. -/
...
/-- Translation of `iInf_covering` to the arrow form : covering by the
    infimum of a family is equivalent to covering by every member.
    Proof : `covers_iff` on both sides then `iInf_covering`. -/
...
/-- Supremum of an indexed family, covering characterization :
    `S ∈ (⨆ i, J i) X` if and only if `S ∈ K X` for every topology `K`
    above the whole family (the generated topology).
    Proof : `⨆ i, J i = sSup (Set.range J)` by definition, then
    `sSup_covering` and transport between member- and index-quantification. -/
...
/-- Translation of `iSup_covering` to the arrow form.
    Proof : `covers_iff` on both sides then `iSup_covering`. -/
