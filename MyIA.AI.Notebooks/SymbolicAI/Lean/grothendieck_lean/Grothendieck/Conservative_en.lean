...
/-
  English mirror of `Conservative.lean` (EN canonical, header mono-lingual EN).
...
/-!
## 12. Bridges on the practical criterion (SGA 4 IV 6.5) and HasEnoughPoints

Sections 3-7 documented by `#check` the practical conservativity criterion
(SGA 4 IV 6.5 (a)): a family P of points is conservative iff the covering-sieve
condition is detectable point by point. This grain adds the proper bridges on
the central constructions that remained documentary:

  - `conservative_family_field`: the structure itself (type-sig re-export);
  - `jointly_reflect_ofArrows_mem_field`: a sieve generated by a family of
    arrows is J-covering iff the arrows are jointly surjective on every fiber
    (the heart of SGA 4 IV 6.5);
  - `jointly_reflect_ofArrows_mem_of_small_field`: variant for small families
    of points;
  - `W_iff_field`: f ∈ J.W iff f induces an iso on every fiber — the
    stalk-by-stalk characterization of the class of morphisms inverted by
    sheafification;
  - `has_enough_points_field`: the site has enough points (existence of a
    small conservative family).

All bridges are L902 ★★ Tier 5: resident args, structural instances, no
polymorphic universe constructor.
-/

/-- Bridge: the conservative family of points — the central structure of the
    module: the fiber functors `Sheaf J (Type w) ⥤ Type w` at the points of P
    jointly reflect isomorphisms (Stacks 00YK (1)). Direct re-export of the
    Mathlib structure `ObjectProperty.IsConservativeFamilyOfPoints`.
    Type-sig bridge (L902 ★★ Tier 5) — direct re-export of the Prop structure. -/
def conservative_family_field : Prop :=
  P.IsConservativeFamilyOfPoints

/-- Bridge: the detection of covering sieves via the points (SGA 4 IV 6.5) —
    a sieve generated by `{f : U i ⟶ X}` is J-covering iff, for every point
    Phi of P and every x in the fiber of X, some arrow `f i` reaches x.
    Delegates directly to the Mathlib lemma `jointly_reflect_ofArrows_mem`.
    Lemma call direct (L902 ★★ Tier 5) — resident args. -/
...
/-- Bridge: the variant for small families of points — same detection of
    covering sieves, with `[ObjectProperty.Small.{w} P]` instead of
    `[Small.{w} ι]`. Delegates to the Mathlib lemma
...
/-- Bridge: the stalk-by-stalk characterization of J.W — a morphism f belongs
    to J.W (the class of morphisms inverted by sheafification) iff it induces
    an iso on the fiber at every point of the conservative family P.
    Delegates directly to the Mathlib lemma `W_iff`.
...
/-- Bridge: the site (C, J) has enough points — the Prop class
    `GrothendieckTopology.HasEnoughPoints`: the existence of a small
    conservative family of points. This is the condition guaranteeing that
    stalk-by-stalk reasoning is complete. Direct re-export of the Mathlib class.
    Type-sig bridge (L902 ★★ Tier 5) — direct re-export of the Prop class. -/
