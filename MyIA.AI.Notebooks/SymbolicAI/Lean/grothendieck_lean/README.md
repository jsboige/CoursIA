...
Les **60 modules leaf** (0 `sorry`, 0 axiome ajouté) tracent un chemin cohérent,
...
La formalisation couvre **60 modules leaf** + **1 umbrella** `Grothendieck.lean`
...
|--------|---------|-------|---------|--------|
| racine | `Grothendieck.lean` | (bilingue inline) | **Racine umbrella** (imports-only + doctring bilingue FR/EN) ; importe **les 60 leaf FR** + 11 siblings `_en` (couverture FR complète via umbrella ; les 60 siblings EN sont compilés par les `globs` du lakefile, l'umbrella n'importe pas tous les `_en` ; `ExceptionalDirect` importé c.2026-08-15, fermeture #11286) | 261 |
...
- **Build** : `lake build` (WSL requis). La cible défaut (`globs := #[`Grothendieck.*]` du `lakefile.lean`) compile **tous** les modules FR et `_en`. Dernier build vérifié : 2026-08-28, « Build completed successfully » (merge Partie 60 + Parties 61-62, PR #12718) ; la structure compte 60 leaf + 60 siblings `_en` + 1 umbrella (Parties 55a-c, 58-62 de #2159 — classifieur, Lawvere–Tierney, dictionnaire, foncteurs continus/lemme de comparaison, construction Plus ; sous-dossier `SheafCohomology/` inclus).
