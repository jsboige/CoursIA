...
Les **51 modules leaf** (0 `sorry`, 0 axiome ajouté) tracent un chemin cohérent,
...
La formalisation couvre **51 modules leaf** + **1 umbrella** `Grothendieck.lean`
...
|--------|---------|-------|---------|--------|
| racine | `Grothendieck.lean` | (bilingue inline) | **Racine umbrella** (imports-only + doctring bilingue FR/EN) ; importe **les 51 leaf** FR + 51 siblings `_en` (couverture complète, plus aucun module en attente — `ExceptionalDirect` importé c.2026-08-15, fermeture #11286) | 227 |
...
| 47 | `Grothendieck/CoversCoherentArrow.lean` | `CoversCoherentArrow_en.lean` | Forme flèche de la **topologie cohérente** (`GrothendieckTopology.IsCoherent`, condition de stabilité par intersection finie de familles couvrantes) : `covers_iff_coherent`, `coherent_of_pretopology`, `coherent_covering_stable_inter`, `covers_coherent_precomp` (Phase 5 de #2159) | 183 |
| 48 | `Grothendieck/CoversExtensiveArrow.lean` | `CoversExtensiveArrow_en.lean` | Forme flèche de la **topologie extensive** (sup de topologies, caractérisation par famille couvrante maximale) : `covers_iff_extensive`, `extensive_covering_iff`, `extensive_of_precoverage`, `covers_extensive_sSup` (Phase 5 de #2159) | 181 |
| 49 | `Grothendieck/CoversPretopologyArrow.lean` | `CoversPretopologyArrow_en.lean` | Forme flèche de la **pré-topologie** (couverture maximale vue comme ensemble d'objets couvrants) : `covers_iff_pretopology_max`, `pretopology_of_covering`, `pretopology_arrows_iff`, `covers_pretopology_stable` (Phase 5 de #2159) | 189 |
| 50 | `Grothendieck/CoversRegularArrow.lean` | `CoversRegularArrow_en.lean` | Forme flèche de la **topologie régulière** (condition de stabilité par changement de base, classe de Siebeck) : `covers_iff_regular`, `regular_covering_pullback`, `regular_covering_isSheaf`, `covers_regular_precomp` (Phase 5 de #2159) | 175 |
...
- **Build** : `lake build` (WSL requis). La cible défaut (`globs := #[`Grothendieck.*]` du `lakefile.lean`) compile **tous** les modules FR et `_en`. Dernier build vérifié : 2026-08-19, « Build completed successfully » (51 leaf + 51 siblings `_en` + 1 umbrella, `CoversRegularArrow` importé — Phase 5 de #2159).
...
- **i18n** (EPIC #4980, convention Option A ratifiée 2026-07-04) : couverture bilingue complète — **52 fichiers FR** (1 umbrella + 51 leaf canoniques) et **51 siblings `_en.lean`**, ratio 1:1 intégral (le gap historique `PullbackFunctor.lean` sans `_en` est comblé : `PullbackFunctor_en.lean` présent sur disque ; c.2026-08-19, l'audit §E a constaté — la prose antérieure le disant manquant était périmée). Namespaces `_en` anti-collision, contenu non-docstring byte-identique, vérifiable par CI. L'umbrella est bilingue inline *by design* (FR canonique d'abord, EN en miroir dans le même fichier). **[`README.en.md`](./README.en.md)** est le miroir EN du présent fichier. Hors-scope : `.lake/packages/`, libs vendored.

*Note de cohérence* : couverture 1:1 intégrale — 51 leaf FR canoniques et 51 siblings `_en` (le gap `PullbackFunctor` sans `_en`, nommé dans une version antérieure de cette note, est comblé sur disque). Le `globs` du lakefile auto-découvre tous les modules présents, FR comme `_en`.
...
- EPIC #4980 — convention i18n Lean (Option A sibling pair ; 51 paires `_en` dans ce lake)
