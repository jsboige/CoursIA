...
La formalisation couvre **l'ensemble des modules leaf** + **1 umbrella** `Grothendieck.lean`
...
| 24 | `Grothendieck/YonedaLemma.lean` | `YonedaLemma_en.lean` | The Yoneda lemma (embedding, equivalence, naturality, fully-faithful, coyoneda) | 286 |
...
| 78 | `Grothendieck/StalkCharacterization.lean` | `StalkCharacterization_en.lean` | **Caractérisation du faisceau par les tiges** : l'équivalence `IsSheaf F ↔ <séparation par les germes> ∧ <recollement des familles localement représentables>` — le capstone des Parties 74-75, et le seul endroit du lake où la condition de faisceau est *dérivée* plutôt que consommée (via `isSheaf_of_isSheafUniqueGluing_types`) | 152 |
...
- **Build** : `lake build` (WSL requis). La cible par défaut (`globs := #[`Grothendieck.*]` dans `lakefile.lean`) compile **tous** les modules FR et `_en` (1 umbrella + les leaf FR, plus les modules `_en` ; les comptes exacts sont mesurés par `git ls-tree -r HEAD`). Dernier build vérifié sur la branche de cette PR : `lake build Grothendieck` SUCCESS local (preuve jointe dans le body de PR, §Validation). Le compte disque (leaf FR, leaf `_en`, umbrella) est contrôlé par `scripts/lean/check_grothendieck_readme.py` (sortie JSON, code non nul en cas de dérive).
...
- Epic #1646 (hommage à Grothendieck) — Issue #2159 (profondeur de formalisation : Phase 1 livrée, Phase 2 = #10357, Phase 5 = Parties 35-44, puis Parties 64-77)
...
3. **Reconnection debt resolved**: `#11286` — umbrella import of `ExceptionalDirect` **CLOSED 2026-08-16** (PR #11294, cf §See also); the umbrella now imports every FR leaf and never an `_en` (#16154), while the lakefile `globs` also build the `_en` siblings.
