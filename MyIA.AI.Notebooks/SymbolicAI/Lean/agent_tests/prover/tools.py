...
def _count_sorries_from_build_output(raw_output) -> int:
    ...
    Defensive coercion (Run-4 crash, #6790): mirrors ``_parse_lean_errors`` --
    some callers fall back to ``errors`` (a list) when ``raw_output`` is empty,
    which would crash on ``.split("\\n")``.  Coerce list/tuple to str.
    """
...
def _parse_lean_errors(raw_output: str) -> list:
    ...
    Defensive coercion (Run-4 crash, #6790 gate preserve): several callers use
    the pattern ``result.get("raw_output", "") or result.get("errors", "")``.
    ``errors`` in the verifier result is a **list** of pre-extracted error
    lines, not a string -- so when ``raw_output`` is empty, the fallback passes
    a list here and the ``.split("\\n")`` below raised ``AttributeError:
    'list' object has no attribute 'split'`` (crashing the preserve gate at
    ``provers.py:_reverify_compiles_clean`` -> Run-4 exit 1).  Coerce list/tuple
    to a joined string so the parser is type-safe regardless of caller.
    """
    if isinstance(raw_output, (list, tuple)):
        raw_output = "\n".join(str(x) for x in raw_output)
    if not raw_output:
