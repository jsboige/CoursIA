...
DEMOS = {
    ...
    62: {
        ...
        "description": (
            ...
            "PRIOR ATTEMPT (BG run 2026-07-16, near-miss, see #6790 forensic):\n"
            "the chain `hfacts destructuring (32 components) ->\n"
            "node_wf_level_of_four + wave1_result_facts on the 4 wave-1 nodes ->\n"
            "p4_wave2_ih_step` DID produce\n"
            "`hcc_nw : centralCorrect (node r1 r2 r4 r5) (k-1)`, but the final\n"
            "application `(centralCorrect_mem_shift _ (k-1) (2^k) (2^k) p\n"
            "hcc_nw).mp hnw` FAILED to typecheck: hnw mentions\n"
            "`hashlifeResultAux (k + 1) ...` while the lemma at level k-1\n"
            "expects `hashlifeResultAux (k - 1 + 2) ...`. Nat subtraction is\n"
            "truncated, so `k - 1 + 2 = k + 1` is NOT definitional: rewrite\n"
            ...
            "NEAR-MISS run-4 (BG 2026-07-16, REJECTED by independent build):\n"
            "materializing steps 3-5 INLINE (`hcc_nw := p4_wave2_ih_step k hk1\n"
            "_ _ _ _ r1.1 r2.1 r4.1 r5.1 (wf_of_cellWf r1.2) ... ih`, then\n"
            "`set qnw := node (hashlifeResultAux (k+1) ...) x4 with hqnw;\n"
            "clear_value qnw`, `hkeq : (k-1)+2 = k+1 := by omega`, then\n"
            "`centralCorrect_mem_shift qnw (k-1) (2^k) (2^k) p hcc_nw'`)\n"
            "elaborates past step 5 but BLOWS the declaration's\n"
            "200000-heartbeat budget: whnf timeout reported at the def head\n"
            "(2860:85). `set + clear_value` does NOT contain the cost. DO NOT\n"
            "retry inline. Instead FACTOR steps 3-5+ into a STANDALONE helper\n"
            "lemma OUTSIDE p4_succ_membership (the heartbeat budget resets per\n"
            "declaration), e.g. `p4_nw_shift_lemma` consuming (r1, r2, r4, r5,\n"
            "ih) and producing the shifted membership characterization; the nw\n"
            "case then just applies the lemma + the evolve_half_step /\n"
            "evolve_cone_agree bridge. Fallback only if still needed: scoped\n"
            "`set_option maxHeartbeats 400000 in`. Full rejected diff: #6724\n"
            "comment 2026-07-16.\n"
