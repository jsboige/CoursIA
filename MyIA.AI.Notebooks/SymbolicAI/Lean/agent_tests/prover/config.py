...
DEMOS = {
    ...
    9: {
        ...
        "proof_scaffolding": (
            "  -- Unfold and rename for readability\n"
            "  classical\n"
            "  set m := median_peak peaks with hm_def\n"
            "  set l := sorted_peaks_list peaks with hl_def\n"
            "  have hperm : l ~ Finset.univ.toList.map peaks := by\n"
            "    unfold_let l sorted_peaks_list; exact List.mergeSort_perm _ _\n"
            "  have hlen : l.length = Fintype.card ι := by\n"
            "    unfold_let l sorted_peaks_list\n"
            "    simp [List.length_mergeSort, List.length_map, Finset.length_toList]\n"
            "  -- TODO: pairwise sortedness + counting at l[n/2]\n"
            "  -- TODO: transfer counts to Finset.filter via Perm.countP_eq + toList_filter\n"
            "  -- TODO: combine with hodd to derive strict inequality\n"
            "  sorry  -- prover replaces with full counting proof\n"
        ),
        "difficulty": "very_hard",
    },
    14: {
