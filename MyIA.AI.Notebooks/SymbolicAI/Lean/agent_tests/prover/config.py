...
PROVIDERS = {
    ...
    "openrouter": {
        ...
        # gpt-5.6-luna defeats the entire point of the Director escalation.
...
DEMOS = {
    ...
    62: {
        ...
        "description": (
            "BG-prover target (hashlife nibble plan #3846, N3): the\n"
            "p4_succ_membership decomposition (PR #6792) = 5 named sub-sorries\n"
            "under `refine (?mp, ?mpr)`. Post-#6869 line numbers: 4 mp quadrant\n"
            "cases (nw L2970, ne L2973, sw L2975, se L2977) + 1 mpr routing case\n"
            "(L2982).\n"
            "NW STATUS — DO NOT TARGET NW: steps 3-5 are FACTORED as the\n"
            "sorry-free private theorem `p4_nw_shift_lemma` (L2846, PR #6869\n"
            "merged 2026-07-16): body = p4_wave2_ih_step ->\n"
            "centralCorrect_mem_shift, nw offsets (2^k, 2^k) hardcoded in the\n"
            "statement. The nw residual sorry (L2970) = the G3 bridge consumption\n"
            "(evolve_half_step + evolve_cone_agree -> global evolve 2^k window),\n"
            "handled on the po-2026 manual track.\n"
            "CURRENT TARGET = ne quadrant (L2973): from hypothesis\n"
            "`hne : p in out_ne.toGrid (2^k, 2^k + 2^level)` prove the RHS\n"
            "membership disjunct. PROVEN RECIPE (mirror the nw case, all steps\n"
            "branch-local — the nw branch's `obtain` is NOT in scope here):\n"
            "(1) destructure hfacts into the 32 grandchild components\n"
            "(hnw_nw_l..hse_se_w); (2) build the wave-1 result facts for the NE\n"
            "super-cell nodes n2/n3/n5/n6 via node_wf_level_of_four +\n"
            "wave1_result_facts (ne quadrant uses nodes 2,3,5,6 of the 3x3\n"
            "wave-1 grid, cf p4_double_nine_shape; nw used n1/n2/n4/n5);\n"
            "(3) FACTOR a standalone private `p4_ne_shift_lemma` analogous to\n"
            "p4_nw_shift_lemma but with the ne offset pair —\n"
            "centralCorrect_mem_shift already takes both offsets as arguments\n"
            "(cf L2866), so only the statement's offsets change; (4) apply the\n"
            "lemma + the evolve_half_step + evolve_cone_agree bridge. Watch the\n"
            "same Nat-subtraction trap: `k - 1 + 2 = k + 1` is NOT definitional,\n"
            "rewrite with hk1 : 1 <= k (Nat.sub_add_cancel / omega) BEFORE any\n"
            ".mp application.\n"
            "HARD LESSON (run-4 forensic, full rejected diff: #6724 comment\n"
            "2026-07-16): inlining the wave2-ih + shift chain inside\n"
            "p4_succ_membership BLOWS the 200000-heartbeat budget (whnf timeout\n"
            "at the def head; `set + clear_value` does NOT contain the cost).\n"
            "ALWAYS factor a standalone private lemma — the heartbeat budget\n"
            "resets per declaration (proven by p4_nw_shift_lemma, #6869).\n"
            "Fallback only if still needed: scoped\n"
            "`set_option maxHeartbeats 400000 in`.\n"
            "When the ne case is proved, re-point `line` to sw (L2975), then se\n"
            "(L2977), then mpr (L2982; mpr uses quad_partition_bounds to route p\n"
            "to its quadrant, then the reciprocal G2+G1+G3 chain).\n"
