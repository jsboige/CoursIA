...
DEMOS = {
    ...
    55: {
        ...
        "description": (
            ...
            "\n"
            "PROOF STRATEGY (LP duality) -- UPDATED 2026-06-20 with the po-2026\n"
            "diagnosis from the prior BG trace:\n"
            "  PRIOR ITERATIONS THRASHED ~1800s on `ProperCone.hyperplane_separation`\n"
            "  and `geometric_hahn_banach_*`: the product space (N -> R) x R lacks the\n"
            "  automatic ordered-cone instances (IsOrderedAddMonoid / PartialOrder\n"
            "  needed for `ProperCone.positive`), so those lemmas do not apply off the\n"
            "  shelf. AVOID that route unless you FIRST construct the missing instances.\n"
            "  PREFER one of these two routes that sidestep the product-space cone:\n"
            "  A. Distinguished-coordinate encoding: keep the whole LP inside (N -> R)\n"
            "     (encode the v(N) bound in a dedicated coordinate / scalar) so the\n"
            "     existing order structure on (N -> R) applies, then separate there.\n"
            "  B. Compactness + extremum (cone-free): { x in P | (sum i, x i) <=\n"
            "     v(N)+1 } is closed and (by balancedness) nonempty/bounded -> attains\n"
            "     its inf via `IsCompact.exists_isMinOn` / `IsCompact.exists_forall_le`;\n"
            "     show the inf <= v(N) from the balanced collection's weights.\n"
            "\n"
