...
/-!
# Phase 3b — Converse §11 : briques probabilistes de la borne inférieure

Ce module formalise les briques probabilistes du converse du seuil `2 log N`
...
intervalles de coordonnées est exactement le produit `∏ (1 − mᵢ)` puis
`≤ (1−p)^n ≤ e^{−np}` — le squelette complet de l'argument d'impossibilité
du §11, sans `sorry`.
-/
...
/-- `(1 − p)^n ≤ e^{−n·p}` pour `p ≤ 1` (§11 : `(1−p)^N ≤ e^{−Np}`).
Instance directe de Mathlib `one_sub_div_pow_le_exp_neg`. -/
...
/-! ## Brique 3 — Hanson–Wright pour le bruit standard -/

/-- Les coordonnées de `stdGaussianPi` sont sous-gaussiennes de paramètre `1`
(transport du certificat `N(0,1)` via la loi image `map_eval_stdGaussianPi`). -/
...
/-- Hanson–Wright pour le vecteur de bruit standard `w ~ stdGaussianPi n` :
queue de la forme quadratique centrée `|X ᵀA X − E X ᵀA X|` (§11 : concentration
chi-square de `‖w‖²` et des formes `h_iᵀ w`). Cas `K = 1` du théorème du lake
SLT, certificats sous-gaussiens des coordonnées transportés depuis `N(0,1)`. -/
...
/-- Par indépendance des coordonnées du bruit (`Measure.pi_pi`), la probabilité
que `w` échappe à chacun des `n` intervalles de coordonnées
`[c−ε/2, c+ε/2] ⊆ [−2,2]` est le produit exact `∏ᵢ (1 − mᵢ)` puis
`≤ (1−p)^n ≤ e^{−np}` avec `p = ε·φ(2)`. Squelette complet du converse §11 :
chaque intervalle porte masse `≥ p` (Brique 1), les échappements sont
indépendants (`Measure.pi_pi`), et l'union bound complémentaire conclut
(Brique 2). -/
