/-
  ...
  EN sibling of `Sensitivity/MainTheorem.lean` (FR-first canonical).
  ...
  module `Sensitivity.Operator_en` (transitively VectorSpace_en, Hypercube_en).
  Self-contained within the EN namespace: proves the flagship Huang sensitivity
  theorem (huang_degree_theorem) and exists_eigenvalue. No cross-namespace
  dot-notation on a FR-defined type (safe profile, same as the other _en modules
  in this lake).
...
theorem huang_degree_theorem (H : Set (Q m.succ)) (hH : Card H ≥ 2 ^ m + 1) :
  ...
  calc
    ...
    _ = |ε q (f m.succ y)| := by
      -- rc1 : `y ∈ ↑(g m).range` (Submodule.range coe'd to Set) ne defeq plus vers
      -- `∃ v, (g m) v = y` ; convertir via `Submodule.mem_range` (rc1 API drift).
      obtain ⟨v, hv⟩ := (g m).mem_range.mp y_mem_g
      rw [← f_image_g y ⟨v, hv⟩]
