...
| `Conway/Life/HashlifeCorrectness.lean` | — | 0 | Bounded correctness `hashlife_correct` (proven but vacuous at large-n); prover target (Epic #1453, #2162). **P4 PROVEN sorry-free post-split #9883/#9884** and **P5.2 `p5_large_n_jumpN` PROVEN sorry-free** (L6749, re-signed b3' trajectory-capture 2026-08-15; `hashlife_correctN` L6808 reduced c.95 to `p5_small_n_fallback` + `p5_large_n_jumpN`). 0 sorry in this file — the lake's only debt lives in `HashlifeMarginFragment.lean`. The old P4 items `p4_nw_supercell_agree`/`p4_nw_membership_arm` and line numbers L2893-3706 date from the pre-split monolith (7082 lines) — **stale**. |
| `Conway/Life/HashlifeMarginFragment.lean` | `HashlifeMarginFragment_en.lean` | 1 | The lake's **single distinct debt**: `hashlife_correct_margin` (L158, `sorry` L167, documented INTRINSIC verdict — bounded P4/P5 assembly via `supportInMargin`, offset-matching chain `p4_nw_overlap_wall`) |
...
- **P5 large-n** (1 residual sorry): **P5.2 is closed** — `p5_large_n_jumpN` (L6749, the genuine jump `evolveHashlifeFast n g = evolve n g`) is proven sorry-free since b3' (re-signing 2026-08-15: the old hypothesis `BoxAssezGrandN` was tautological — `box_assez_grandN_trivial`, c.1035 — replaced by the trajectory capture `jumpCaptured`, non-tautological and equipped with the witnesses `jumpCaptured_block`/`jumpCaptured_glider`). What remains is `hashlife_correct_margin` (`HashlifeMarginFragment.lean` L158, sorry L167, INTRINSIC — the bounded P4/P5 assembly via `supportInMargin`, the open research heart). `hashlife_correctN` (L6808) is proven by c.95 reduction to `p5_small_n_fallback` (L6168) + `p5_large_n_jumpN`; `p5_large_n_jump` (L6274) is the legacy fixed-frame statement, proven.

**Concrete next step**: `hashlife_correct_margin` (`HashlifeMarginFragment.lean` L167) is the sole remaining target — the bounded P4/P5 assembly via `supportInMargin`, leaning on the offset-matching chain `p4_nw_overlap_wall` (`Walls/NW.lean`, proven sorry-free since the post-split discharge). The jump itself is closed: P5.2 `p5_large_n_jumpN` (L6749) is proven (b3') via `hashlifeResult_central_correct` + trajectory capture (`hashlifeJump_correct_of_captured` in `JumpCapture.lean` requires `jumpCaptured c = true` — the `restrictGridTo` clip is transparent under capture, i.e. the light cone stays inside the central window, containment condition `window_cheb_cone_in_domain` in `ConeGeometry`). This is the live BG-prover target (`agent_tests/prover/`); the multi-wave light-cone composition resists current tactical automation.
...
now **proven sorry-free post-split** (#9883/#9884); the P5.2 jump (`p5_large_n_jumpN`)
is **closed** (b3'), the residual research-level target is the margin lock
`hashlife_correct_margin` (INTRINSIC), and `hashlife_correct` itself is proven
...
flowchart TD
    ...
    P5["P5 — Large-n jump  <b>⚠ 1 residual sorry (the margin lock)</b><br/>P5.2 p5_large_n_jumpN <b>PROVEN</b> (b3' L6749, trajectory capture)<br/>1 sorry · hashlife_correct_margin [L167, INTRINSIC]<br/><i>p5_small_n_fallback + evolve_dead_of_cone_dead + p5_inductive_step (c.310 #5998) + p5_large_n_jump + p5_large_n_jumpN + hashlife_correctN (c.95) proven sorry-free; hashlife_correct proven but vacuous at large-n (BoxAssezGrand capped n≤2)</i>"]
    GOAL["hashlife_correct  <b>proven (vacuous at large-n)</b> · hashlife_correct_margin = live target (INTRINSIC)"]
...
- **The sorry-free additive ingredients** (level/wf preservation, `box_assez_grand` arithmetic) accumulated behind the P4 lock and are **deployed** — the P4 inductive step is now proven post-split, and they feed `hashlifeResult_central_correct`. The same dynamic now applies behind the margin lock (`hashlife_correct_margin`), the P5.2 jump being closed.

### Next steps

1. **BG-prover on the margin lock**: attack `hashlife_correct_margin` (`HashlifeMarginFragment.lean` L167, INTRINSIC verdict) via the multi-agent harness (`agent_tests/prover/`) — the assembly leans on the offset-matching chain `p4_nw_overlap_wall` (`Walls/NW.lean`). With P5.2 (`p5_large_n_jumpN`, proven b3' via `hashlifeResult_central_correct` + `window_cheb_cone_in_domain`) closed, this is the live research-level target.
2. **Margin chain**: `hashlife_correct_margin` requires the bounded P4/P5 assembly via `supportInMargin` over the offset-matching chain `p4_nw_overlap_wall` (`Walls/NW.lean`) — separate research heart.
