...
| `Conway/MathlibMap.lean` | `MathlibMap_en.lean` | 0 | Mathlib pinned-snapshot (`54f98fd6`) satellite — what Mathlib provides for Conway's work |
...
- **P5 large-n** (1 residual sorry): **P5.2 is closed** — `p5_large_n_jumpN` (L6749, the genuine jump `evolveHashlifeFast n g = evolve n g`) is proven sorry-free since b3' (re-signing 2026-08-15: the former `BoxAssezGrandN` hypothesis was tautological — `box_assez_grandN_trivial`, c.1035 — replaced by the trajectory capture `jumpCaptured`, non-tautological and witnessed by `jumpCaptured_block`/`jumpCaptured_glider`). Remains `hashlife_correct_margin` (`HashlifeMarginFragment.lean` L158, sorry L167, INTRINSIC — the bounded P4/P5 assembly via `supportInMargin`, open research heart). `hashlife_correctN` (L6808) is proven by reduction c.95 to `p5_small_n_fallback` (L6168) + `p5_large_n_jumpN`; `p5_large_n_jump` (L6274) is the former fixed-frame statement, proven.

**Concrete next step**: `hashlife_correct_margin` (`HashlifeMarginFragment.lean` L167) is the sole remaining target — the bounded P4/P5 assembly via `supportInMargin`, whose open link is the offset-matching `p4_nw_overlap_wall` (`Walls/NW.lean`, sorry 10→9 by #9745/#9760). The jump itself is closed: P5.2 `p5_large_n_jumpN` (L6749) is proven (b3') via `hashlifeResult_central_correct` + the trajectory capture (`hashlifeJump_correct_of_captured` in `JumpCapture.lean` requires `jumpCaptured c = true` — the `restrictGridTo` clip transparent under capture, i.e. the light cone stays inside the central window, containment condition `window_cheb_cone_in_domain` in `ConeGeometry`). This is the live BG-prover target (`agent_tests/prover/`); the multi-wave light-cone composition resists current tactical automation.

*The `hashlife_correct` correction pyramid: the proven foundation (base cases, P1-P3,
`p5_small_n_fallback`) carries the theorem, the P4 double-nine inductive step is
**proven sorry-free post-split** (#9883/#9884), and the large-`n` P5.2 jump
(`p5_large_n_jumpN`) is **closed** (b3'); the residual research-level target
is the margin lock `hashlife_correct_margin` (INTRINSIC), and `hashlife_correct` itself is proven
...
1. **BG-prover on the margin lock**: attack `hashlife_correct_margin` (`HashlifeMarginFragment.lean` L167, INTRINSIC verdict) via the multi-agent harness (`agent_tests/prover/`) — the open link is the offset-matching assembly `p4_nw_overlap_wall` (`Walls/NW.lean`). With P5.2 (`p5_large_n_jumpN`, proven b3' via `hashlifeResult_central_correct` + `window_cheb_cone_in_domain`) closed, this is the live research-level target.
2. **Margin chain**: `hashlife_correct_margin` requires the offset-matching chain `p4_nw_overlap_wall` (`Walls/NW.lean`) — separate research heart, bounded P4/P5 assembly via `supportInMargin`.
