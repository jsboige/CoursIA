...
Le binding Z3.Linq traduit des requêtes LINQ C# en formules SMT — on décrit les contraintes, le solveur produit la solution. Série .NET 9 de **16 notebooks** (~11h), du théorème linéaire à l'optimisation sous contraintes molles (sac à dos, MaxSAT) — en passant par les cryptarithmes positionnels (arithmétique positionnelle + contrainte globale `Distinct` sur le classique SEND + MORE = MONEY), la vérification de débordement sur **bit-vectors** (la théorie SMT originelle de Z3, née chez Microsoft Research pour la vérification de drivers et de matériel), et le **raisonnement exact sur les réels** (irrationnels algébriques comme √2, preuve d'absence de racine).
