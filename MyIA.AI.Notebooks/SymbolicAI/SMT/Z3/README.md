...
| 05b | [Meal Planner Real Corpus Hierarchical](05b_Meal_Planner_Real_Corpus_Hierarchical.ipynb) | **Le théorème hiérarchique `int[][]` de 05, à l'échelle réelle** : le corpus jouet cède la place à **138 plats réels** issus de la fusion **Ciqual ANSES 2025 × Recettes** (le corpus qui porte la structure en créneaux `ordre_plat`). Bornes de catégorie par créneau + montée en gamme + permutation totale (`Distinct` cross-row, `CollectionHandling.Array`) passent **sans modification** du jouet au réel (7 jours × 5 créneaux, SAT en ~80 ms). Sœur structurelle de 07b (qui traite l'axe nutritionnel à l'échelle) | ~40 min | BETA |
| 05c | [Dietetique Load Faithful Port](05c_Dietetique_Load_Faithful_Port.ipynb) | **Le portage *fidèle* de `Dietetique.Load`** (Demo2 `MealPlanning` du fork Z3.Linq) : avant que le théorème SMT (05/05b/07b) ne tourne, il faut **construire le corpus dérivé** à partir des sources brutes. Pipeline LINQ à **deux jointures** (`CONST ⋈ COMPO ⋈ ALIM` de Ciqual ANSES 2025) + **matcheur lexical flou français** (`CalculeProximiteLexicale`, sac-de-mots normalisé : 198 denrées Recettes appariées à leur aliment Ciqual le plus proche) + **agrégation `Zip`/`Aggregate`** terme-à-terme des vecteurs de teneur (74 constituants). Cache `dietetique.json` (UTF-8). Le « préprocesseur » de données que les notebooks de résolution présupposaient — porté ligne-à-ligne depuis la source C#. Slice A du capstone MealPlanner | ~45 min | BETA |
| 05d | [Hierarchical Patient Restrictions](05d_Hierarchical_Patient_Restrictions.ipynb) | **Le cœur solveur du capstone** : port *fidèle* de `PlanificateurDeMenus.Create` (Demo2). Théorème **hiérarchique à 4 niveaux** Patient→Menus→Plats→Denrées sur le corpus réel issu de 05c, avec ce que 05/05b n'avaient pas — le **linking de composition** (`PlatId != candidat \|\| Comp == teneur`, variable nutritionnelle liée au plat réellement choisi, source l.60-99) **et** les **restrictions patient Min/Max** par constituant (fenêtre énergie + plancher protéines + plafond lipides). Sous-ensemble curé tractable (2 menus × 3 créneaux × 5 candidats × 3 constituants, SAT ~120 ms) ; le passage au corpus complet fait exploser le linking (~357k assertions) = problème de convergence à l'échelle réservé à la Slice D. Slice C du capstone | ~45 min | BETA |
| 05e | [Meal Planner Convergence at Scale](05e_Meal_Planner_Convergence_Scale.ipynb) | **Finale capstone (See #4617)** : le problème patient *fidèle* complet (`PlanificateurDeMenus.Create` — 7 menus × 5 créneaux × pools d'ordre × Min/Max par constituant) reste-t-il tractable à l'échelle réelle ? Sonde de coût naïf (disjonction-liée, O(M×P×C×K), ~63k clauses à 74 constituants, ~357k extrapolé) vs résolution **one-hot pseudo-booléenne** du 7×5 complet (build 0,03s + solve ~0,15s → SATISFIABLE). Leçon structurante : l'**encodage** décide seul de la tractabilité, la puissance brute du solveur ne compense pas un mauvais encodage | ~30 min | BETA |
...
| 07b | [Meal Planner Real Corpus Scaling](07b_Meal_Planner_Real_Corpus_Scaling.ipynb) | **Passage à l'échelle réelle** : fusion du **vrai corpus RecipeML** (XML, items anglais, sans nutrition) avec **Ciqual ANSES 2025** (`alim_nom_eng`, jointure lexicale à index inversé), puis **3 encodages SMT comparés** sur R≈961 recettes — disjonction naïve (explose à la construction), théorie des tableaux (`unknown` sur chaînes de `Store` symboliques), **one-hot pseudo-booléen** (`MkPBEq`/`MkPBLe`/`MkPBGe`, SAT ~1s) = le seul qui passe l'échelle. Les recettes sont des **contraintes énablantes** : plus de données = plus de solutions, l'explosion est purement un problème d'encodage. Sœur de 07 sur corpus réel | ~50 min | BETA |
...
6. **Notebooks 05 → 05e — le capstone MealPlanner (#4617)** : le planificateur de repas, fil rouge de la série, monte en cinq paliers. **05** pose le théorème hiérarchique (data fusion LINQ + mode `CollectionHandling.Constants` ressuscité) sur un corpus jouet ; **05b** le porte *sans modification* à l'échelle réelle (138 plats issus de la fusion Ciqual ANSES 2025 × Recettes). Puis le capstone *fidèle* au fork Z3.Linq se déploie en trois tranches : **05c** (Slice A — le préprocesseur de données `Dietetique.Load` : deux jointures LINQ + matcheur lexical flou français, **zéro raisonnement SMT**, il *construit* le corpus que les solveurs présupposaient) ; **05d** (Slice C — le **cœur solveur** `PlanificateurDeMenus.Create` : théorème à 4 niveaux Patient→Menus→Plats→Denrées, avec le **linking de composition** et les **restrictions patient Min/Max** que 05/05b n'avaient pas) ; **05e** (Slice D — la **convergence à l'échelle réelle**, où l'**encodage one-hot pseudo-booléen** seul reste tractable là où la disjonction-liée naïve explose à ~357k clauses). L'arc tout entier démontre la leçon structurante du solveur sous contraintes : *l'encodage décide seul de la tractabilité, la puissance brute ne compense pas un mauvais encodage*
...
- **Le débouché** — que le paradigme déclaratif se généralise au-delà des exemples jouets : le notebook 06b branche le solveur sur une **source de données externe** (corpus RecipeML parsé en LINQ-to-XML), prouvant que les mêmes contraintes s'appliquent à des données réelles ; et le notebook 06c montre que la **valeur d'un solveur se jauge aussi à la lisibilité de ses solutions**, rendues ici en cartes HTML color-codées et front de Pareto plutôt qu'en tableaux plats. C'est le pont entre la modélisation et la communication du résultat.
