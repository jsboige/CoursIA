...
L'API z3-py expose l'intégralité du solveur Z3 en Python — `Solver`, `Optimize`, tactiques, théories `BitVec`/`Array`/`String`. Série complète de **16 notebooks** (z3-solver + matplotlib), de la satisfaction de contraintes à l'optimisation avancée, à l'ordonnancement combinatoire, aux énigmes logiques (CSP), à l'arithmétique symbolique (cryptarithmes), à la coloration de graphe (NP-complet), à l'arithmétique réelle exacte (irrationnels algébriques), à l'explication de l'insatisfiabilité (UNSAT cores), à l'arithmétique machine bornée (bit-vectors, débordement), à la modélisation de grilles 2D (tableaux imbriqués, carrés latins et magiques), à l'optimisation réaliste d'un meal-planner hebdomadaire (matrice booléenne `jours × plats`) et au pont déclaratif avec la série sœur Z3.Linq (C#).
