...
7. **Notebook 01b** fait le pont avec la série sœur C# Z3.Linq : il montre que l'idiome déclaratif LINQ (`where`/`select`) et l'API impérative pyz3 (`s.add`) expriment la même intention, puis exploite le noyau d'insatisfiabilité (`unsat_core`) et la coloration de graphe (carte d'Australie) pour révéler où le déclaratif surpasse l'impératif
...
13. **Notebook 13** complète la triade `sat`/`unsat`/`unknown` par la **quatrième posture — expliquer** : `unsat` dit *qu'il n'y a pas* de solution, `unsat_core()` dit *pourquoi* (lesquelles contraintes conflictuent). Via `assert_and_track(formule, etiquette)`, Z3 renvoie le sous-ensemble **minimal** de contraintes responsables du conflit — sur une spécification à 5 contraintes, il isole les 2 coupables (`h = 12` vs `h ≠ 12`) en écartant les bornes compatibles et la redondance. L'ingénieur ne debug plus à l'aveugle un système insatisfiable : le solveur pointe chirurgicalement la contrainte à relâcher
14. **Notebook 14** passe à l'**arithmétique machine bornée** via la théorie `BitVec` (entiers modulo 2^n) — le domaine canonique de la vérification formelle de code. Là où `Int` (non borné) rend le débordement inexpressible, `BitVec` le rend **décidable** : preuve qu'il est **inévitable** (`x, y ≥ 2³¹` ⇒ `x + y` déborde, `unsat` par réfutation), preuve de **sécurité** (`p, q < 1000` ⇒ jamais de débordement), et raisonnement **bit-à-bit** (extraction de champ `Extract`, témoin produit par le solveur). Le contraste BV4-vs-Int (`a + b < a` possible sur 4 bits, impossible sur entiers) montre que la largeur fixe *est* la sémantique du débordement
15. **Notebook 15** étend la modélisation du **scalaire à la grille 2D** : un carré latin, un Sudoku 4×4 et un carré magique s'écrivent comme des listes de listes de `Int` contraintes par `Distinct` (lignes/colonnes/blocs) et `Sum` (constante magique 15). Le notebook confronte l'encodage déclaratif (idiomatique z3-py) à l'encodage brut de la théorie des tableaux (`Array` de `Array`, `Store`/`Select` imbriqués) — rendant visible ce que le binding C# Z3.Linq masque derrière son DSL `Theorem<Grid>`
16. **Notebook 16** applique la programmation par contraintes à l'**optimisation réaliste d'un meal-planner** : trois encodages d'un même problème (index, énumération, booléen) montrent que l'approche booléenne (`If(sel_i, val_i, 0)`) rend native la relation sélection-caractéristique. `Optimize.minimize` trouve le menu équilibré de coût minimal en un appel, puis une **matrice booléenne `jours × plats`** produit un plan hebdomadaire (fenêtre kcal/jour + variété globale) que ne sait pas énumérer une heuristique gloutonne — la combinatoire fenêtre × variété n'est pas compositionnelle. C'est le terrain où le solveur démontre sa valeur face à un algorithme impératif naïf. **Quatre compagnons (statut BETA)** prolongent ce module sur un corpus réel (Ciqual ANSES 2025 × RecipeML) : **16b** construit la couche de données (lecture en flux, appariement lexical flou), **16c** applique le capstone à un patient avec restrictions nutritionnelles, **16d** confronte les encodages à la tractabilité à l'échelle (le one-hot pseudo-booléen `PbEq`/`PbLe` l'emporte là où l'index+disjonction explose et la théorie des tableaux reste `unknown`), et **16e** passe du SAT à l'optimisation (`Optimize`, `add_soft` MaxSAT, multi-objectif Pareto/box).
17. **Notebook 17** formalise la **théorie des tableaux** de Z3 : un `Array(I, E)` n'est pas une structure mutable mais une **fonction totale** `Index → Élément`, manipulée via `Select` (lire) et `Store` (renvoyer un nouveau tableau modifié en un indice). Le notebook énonce et **vérifie les axiomes de McCarthy** (read-over-write : `Select(Store(A, i, v), i) = v` et `Select(Store(A, i, v), j) = Select(A, j)` pour `i ≠ j`) comme des théorèmes plutôt que des postulats, puis résout des contraintes sur les **contenus** d'un tableau (tableau trié, égalité de deux tableaux) — là où le notebook 15 n'effleurait la théorie qu'en encodage brut (`Array` de `Array`), le 17 la pose comme objet d'étude à part entière.
18. **Notebook 18** confronte **deux modes d'encodage** d'un même Sudoku 4×4 : un par `Int` constants (une variable scalaire par cellule, `c_0_0` … `c_3_3`), l'autre par `Array(Int, Int)` indexé par position. Les contraintes canoniques (domaine `{1..4}`, rangées / colonnes / blocs 2×2 à valeurs distinctes) s'expriment identiquement dans les deux modes, mais la **comparaison** révèle leurs compromis respectifs — granularité et lisibilité des scalaires d'un côté, uniformité fonctionnelle et extensibilité du tableau de l'autre. Cas d'école du **choix d'encodage** en SMT, ce notebook boucle la série sur la modélisation déclarative entamée au notebook 02 (le Sudoku comme premier CSP).
...
| **`Array`** | Tableau symbolique fonctionnel (théorie des tableaux : select/store) manipulé comme une valeur, pas comme un effet de bord. | 03, 15, 17, 18 |
