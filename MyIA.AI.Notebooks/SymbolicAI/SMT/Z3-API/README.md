...
15. **Notebook 15** étend la modélisation du **scalaire à la grille 2D** : un carré latin, un Sudoku 4×4 et un carré magique s'écrivent comme des listes de listes de `Int` contraintes par `Distinct` (lignes/colonnes/blocs) et `Sum` (constante magique 15). 

Le notebook confronte l'encodage déclaratif (idiomatique z3-py) à l'encodage brut de la théorie des tableaux (`Array` de `Array`, `Store`/`Select` imbriqués) — rendant visible ce que le binding C# Z3.Linq masque derrière son DSL `Theorem<Grid>`
