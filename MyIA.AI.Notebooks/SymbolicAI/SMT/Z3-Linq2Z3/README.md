...
Le bloc **B10** dans `11` (Job Shop, sur la même instance 3×2 que le bloc **B5**) ferme la série « deux stacks » sur l'**optimisation directe** : remplacer la recherche linéaire `for T = lowerBound..upperBound` (qui multiplie les appels `Check()`) par un seul `opt.MkMinimize(cmax) + opt.Check()` côté raw, et par `.OrderBy(t => t.Cmax)` (sucre LINQ sur `Optimize(Optimization.Minimize, lambda)`, `Theorem{T}.cs:142`/`:86`) côté DSL. Sur cette instance, les deux écritures produisent le **même `Cmax = 9h`** en une passe chacun — la boucle externe de la cellule 9 n'était qu'un *anti-pattern d'optimisation*, le solveur sait nativement le faire. La frontière DSL/raw devient alors une affaire de **sucre** (lisibilité du `.OrderBy` façon `orderby` SQL) plus que de couverture — y compris face à l'instance où l'optimum diverge de la borne basse, où `Optimize` est asymptotiquement imbattable.
...
8. **Notebook 11** passe à l'**optimisation** : un problème d'ordonnancement (Job Shop) NP-difficile — 3 jobs × 2 machines, chacun avec un itinéraire fixe. Les contraintes disjonctives de non-chevauchement expriment que deux opérations ne peuvent occuper la même machine au même instant, et le `Cmax` (durée totale) est minimisé par **recherche linéaire ascendante** (suite de requêtes SAT). Le glouton FIFO atteint 12 h ; le solveur prouve l'optimum à 9 h, égalant la borne inférieure (charge cumulée de la machine critique). C'est le terrain où le SMT *discrimine* face à une heuristique séquentielle aveugle. **Bloc B10 — DSL `.Optimize(Minimize, …)`** complète la cellule 9 : au lieu d'énumérer `T = 9..16` en `N+1` appels `Check()`, on déclare une seule fois l'objectif « minimiser `Cmax` » et le solveur trouve l'optimum en **un seul `Check()`** (raw `opt.MkMinimize(cmax)` ↔ DSL `.OrderBy(t => t.Cmax)` qui délègue à `Optimize(Optimization.Minimize, lambda)`). Même `Cmax = 9h` sur la même instance — l'anti-pattern de la boucle linéaire n'est pas une limitation de Z3, c'est un anti-pattern d'appel
...
| **NB-10 — fork Automata** | Le notebook 10 consomme un fork **distinct**, [MyIntelligenceAgency/Automata](https://github.com/MyIntelligenceAgency/Automata) (modernisation net8.0 de AutomataDotNet, gelé ~2020 ; ajoute les opérateurs de surface `&`/`~` et lève le cap du témoin #6), monté en sous-module sibling de `Z3.Linq/`. Exécuter **une fois** : [`scripts/environment/automata-build-deploy.ps1`](../../../../scripts/environment/automata-build-deploy.ps1) (Windows) ou [`.sh`](../../../../scripts/environment/automata-build-deploy.sh) (Linux/macOS) — compile le wrapper (~6 s) et rassemble `Microsoft.Automata.dll` + `System.CodeDom.dll` dans `.deploy/`. Sous Linux, le notebook 10 échoue encore à l'initialisation de `HighTimer` (appel Windows `QueryPerformanceCounter`), cf. [#17654](https://github.com/jsboige/CoursIA/issues/17654). |
...
| 10                          | Fork [Automata](https://github.com/MyIntelligenceAgency/Automata) | `#r "../Automata/.deploy/..."` | [`automata-build-deploy.ps1`](../../../../scripts/environment/automata-build-deploy.ps1) / [`.sh`](../../../../scripts/environment/automata-build-deploy.sh) |
...
| **Le notebook 10 échoue (`Microsoft.Automata.dll` manquant)** | Le notebook 10 consomme un fork **distinct** (Automata, pas Z3.Linq). Exécuter [`automata-build-deploy.ps1`](../../../../scripts/environment/automata-build-deploy.ps1) ou le [`.sh`](../../../../scripts/environment/automata-build-deploy.sh) une fois pour peupler son propre `.deploy/`. |
| **Linux : `DllNotFoundException: Unable to load shared library 'libz3'`** | Le `.deploy/` versionné dans le fork ne contient que le `libz3.dll` de Windows (le paquet NuGet `Microsoft.Z3` n'a pas d'actif natif Linux). Relancer [`z3-build-deploy.sh`](../../../../scripts/environment/z3-build-deploy.sh) : sous Linux, il ajoute le `libz3.so` du wheel `z3-solver` de même version. |
| **Linux : `TypeInitializationException` sur `Microsoft.Automata.Utilities.HighTimer`** | Le fork Automata appelle `QueryPerformanceCounter` de `Kernel32.dll`, propre à Windows. Correctif en attente sur le fork, cf. [#17654](https://github.com/jsboige/CoursIA/issues/17654). |
...
| **Une réexécution ne reproduit pas les sorties committées (`warning CS1701`, `System.Linq.Expressions, Version=…`)** | Le fork `Z3.Linq` est compilé contre `System.Linq.Expressions 8.0.0.0`, mais **chaque noyau résout cette référence sur son propre runtime** — et l'avis `CS1701` émis au chargement du DLL **inscrit l'identité résolue dans la sortie**. Les enregistrements committés ne sont donc pas tous sur le même runtime : `06`, `08` et `09` résolvent en `Version=10.0.0.0`, tandis que `10` résout en `Version=9.0.0.0` (la référence de `Microsoft.Automata` y est `System.Text.RegularExpressions`, même mécanisme). Le major de l'identité résolue suit le major du runtime du noyau — mesuré sur une même machine : `.NET 9.0.20` → `9.0.0.0` sous le défaut, `.NET 10.0.12` → `10.0.0.0` avec `DOTNET_ROLL_FORWARD=LatestMajor`. Conséquence pratique : une réexécution `papermill -k .net-csharp` **par défaut** rend `9.0.0.0` et ne reproduit donc pas les enregistrements de `06`/`08`/`09` ; il faut la variable d'environnement. Aucun organe ne garde ce point — le `Kernel drift guard` compare `metadata.language_info.version`, jamais l'identité d'assembly, et l'écart n'est visible que dans l'avis `CS1701`. |
...
- **Série sœur Z3-API** : [Z3-API](../Z3-API/README.md) expose l'API Z3 *complète* (tactiques, théorie des chaînes `Re`/`InRe`, quantificateurs, preuves) que le binding LINQ masque volontairement. C'est le prolongement naturel quand on veut pousser la modélisation au-delà des entiers et tableaux, notamment pour la **théorie native des chaînes** (génération de témoin depuis un regex, notebook Z3-API-04). La théorie des **bit-vectors**, elle, se découvre déjà en C# dans le [notebook 15 (BitVectors Overflow)](15_BitVectors_Overflow.ipynb) de cette série.
