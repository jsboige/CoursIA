...
> **Correction (2026-07-16, #5780) — traçage code source firsthand.** La ré-vérification vision + lecture du code générateur (`01-5-AnimateDiff-Introduction.ipynb` cell[10] : `indices = np.linspace(0, len(frames)-1, 8, dtype=int)` = [0,2,4,6,8,10,12,15], labels `f"Frame {idx + 1}/{len(frames)}"` avec `len(frames)=16`) établit que la figure `vid1-animatediff.png` porte les labels **Frame 1/3/5/7/9/11/13/16 sur 16** — et non « Frames 0/126/252/378/504/630/756/882 » comme l'indiquaient à tort le README (alt-text L73 + prose L76) et la ligne d'audit ci-dessous. Ces numéros fabriqués étaient internement incohérents (882 ≫ 16) et provenaient d'un enrichissement narratif (#6014) rédigé sans confronter la figure 500 px. Corrigés dans le README et la table d'audit ci-dessous. Les 5 autres figures sont **confirmées fidèles** (vision firsthand 2026-07-16, cohérente avec l'audit 2026-07-10 ; provenance des 6 cellules re-vérifiée par trace `nbformat` = inchangée).
...
| `vid1-animatediff.png` | Mosaïque 4×2 (8 frames) d'animation "A serene lake at sunset with mountains in the background", style painterly/pastel. Frames 1/3/5/7/9/11/13/16 sur 16 (labels "Frame N/16", indices `np.linspace(0,15,8)` = [0,2,4,6,8,10,12,15] ; corrigé 2026-07-16, cf note ci-dessus). | **VRAI** — animation AnimateDiff observable, conformité au prompt descriptif. |
