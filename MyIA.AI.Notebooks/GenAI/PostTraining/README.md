...
| **[RL — rlpt_*](../../RL/README.md)** | Le **pourquoi** de chaque algorithme | La sous-série `rlpt_*` montre la mécanique de chaque post-training en petit et vérifiable (rlpt_1 RLHF char-level, rlpt_2 GRPO nu, rlpt_3 anatomie du reward hacking, rlpt_4 DPO vs GRPO) — la présente série en est la **chaîne d'ingénierie** : mêmes algorithmes sur la pile `trl` complète, verifiers Z3/SymPy, observabilité `rewardspy`, RLVR multi-seed. |
