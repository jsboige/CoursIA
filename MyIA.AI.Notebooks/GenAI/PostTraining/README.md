...
| PT-11b | `PT_11_grpo_qwen_rlvr_on_verifiers.ipynb` | GRPO + RLVR + rewardspy.watch ONLINE — la pile complète sur petit modèle Qwen3.5-0.8B QLoRA, **Tier 1 SymPy (arithmétique) + Tier 2 Z3 (N-queens)**, détecteur Goodhart live — variante à verifiers complémentaires (sibling de PT-11a) | `trl.GRPOTrainer` + `rewardspy.watch` (composants `reward`, fenêtre 20) | Qwen3.5-0.8B (QLoRA 4-bit) | See #10289 |
