...
| **[RL — rlpt_*](../../RL/README.md)** | Mécanique des mêmes algorithmes, en petit | La sous-série `rlpt_*` de RL couvre la **mécanique** RLHF/GRPO/DPO dans son minimum vital (from-scratch char-level jusqu'au minimal sous `trl`, Qwen3.5-0.8B en pleine précision, tâches réduites) ; cette série déroule la **chaîne SOTA à l'échelle** (`trl` + QLoRA 4-bit + solveurs Z3/SymPy + rewardspy en ligne + multi-seed). Lire `rlpt_*` pour « comment ça marche dans son minimum vital », PostTraining pour « comment la chaîne s'exécute à l'échelle ». |
