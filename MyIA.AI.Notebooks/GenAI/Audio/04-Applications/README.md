...
**Hommage Munkres — musique et mathématiques** : [04-14](04-14-VoiceLeading-Affectation.ipynb) formule le voice leading chorale comme problème d'affectation linéaire, résolu par `scipy.optimize.linear_sum_assignment` (algorithme de Kuhn-Munkres) : démo mesurée greedy contre optimal, visualisation des trajectoires sur la ligne des quintes, post-traitement d'une progression générée, et audit du contrepoint de Fux (quintes parallèles) avec réparation CP-SAT — encodage issu des projets étudiants EPITA 2026 (H1 polyphonique, H1_V2 mélodique), honorés dans le notebook. Entièrement offline.
...
| # | Notebook | Contenu | Service | VRAM |
|---|----------|---------|---------|------|
| 14 | [04-14-VoiceLeading-Affectation](04-14-VoiceLeading-Affectation.ipynb) | Voice leading minimal par affectation (Kuhn-Munkres via scipy), greedy vs optimal, ligne des quintes, audit Fux + réparation CP-SAT | Aucun (offline) | 0 |
...
```text
Texte -> MusicGen -> Édition MIDI -> Voice leading optimal (04-14) -> Post-production -> Export
```
