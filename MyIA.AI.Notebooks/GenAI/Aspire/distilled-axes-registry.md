...
| **CSharpRepl — attaché à un process .NET vivant** (`#wrap` / `#replace`) | **Fermé sans livraison** (grain fantôme L1500-B) | [#10802](https://github.com/jsboige/CoursIA/issues/10802), [#10971](https://github.com/jsboige/CoursIA/issues/10971) — toutes deux CLOSED + `closedByPullRequestsReferences: []` | `#wrap`/`#replace` à chaud sur un process attaché vs `pdb`/`%autoreload` IPython | **À rouvrir** : notebook `01-CSharpRepl-Attache-Process-Vivant.ipynb` non créé sur disque. Le corps de #10971 spécifiait déjà la structure (init/list/connect + patches/revert) |
| **Roslyn — analyseurs statiques comme garde-fous** | **Livré** | [#11849](https://github.com/jsboige/CoursIA/issues/11849) → PR [#11866](https://github.com/jsboige/CoursIA/pull/11866) | Analyseurs dans la compilation vs `mypy`/`ruff` (hors compilation) | Itérations E1/E2 `AgentGuard.Analyzers`/`AgentGuard.Demo`/`AgentGuard.Verifier` à mesure des findings de couverture |
| **EF Core — requêtes vérifiées à la compilation** | **Livré** | [#12381](https://github.com/jsboige/CoursIA/issues/12381) → PR [#12393](https://github.com/jsboige/CoursIA/pull/12393) | ORM à l'exécution vs vérifié à la compilation — `EF.CompileQuery` (mesuré 2,78×), `ToQueryString()`, `FromSqlInterpolated` | Livré : notebook `GenAI/EFCore/01-EFCore-Requetes-Compilees.ipynb` ; densification continue possible (EPIC #10473) |
...
| EF Core requêtes vérifiées à la compilation (#12381) | [#12393](https://github.com/jsboige/CoursIA/pull/12393) | myia-po-2024:CoursIA | OPEN (c.28, 2026-08-23) | EF Core — requêtes vérifiées à la compilation | Notebook 01-EFCore-Requetes-Compilees + registre EF Core → Livré |
