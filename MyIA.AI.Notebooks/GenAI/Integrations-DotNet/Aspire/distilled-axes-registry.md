...
## Table des axes (état actualisé par la tranche #15780 au 2026-09-12)
...
| **Roslyn — analyseurs statiques comme garde-fous** | **Livré** (6 règles : AGENTGUARD001 à AGENTGUARD005b ; AGENTGUARD003 couvre `Task.Run`, `Task.Factory.StartNew` et `Task<TResult>.Factory.StartNew` nus) | Base [#11849](https://github.com/jsboige/CoursIA/issues/11849) → PR [#11866](https://github.com/jsboige/CoursIA/pull/11866) ; `CancellationToken` [#13696](https://github.com/jsboige/CoursIA/pull/13696) ; `GetAwaiter().GetResult()` [#13819](https://github.com/jsboige/CoursIA/pull/13819) ; variante `ConfigureAwait` [#13885](https://github.com/jsboige/CoursIA/pull/13885) ; `Task.Factory.StartNew` [#15780](https://github.com/jsboige/CoursIA/issues/15780) | Analyseurs dans la compilation vs `mypy`/`ruff` (hors compilation) ; résolution sémantique des symboles Roslyn plutôt que recherche textuelle | E3 absorbée par #15780 ; prochains grains uniquement sur défauts de couverture mesurés |
...
| **Channels / Orleans** | **Livré** (notebook [`Orleans/01-Orleans-Grains-Agents.ipynb`](../Orleans/01-Orleans-Grains-Agents.ipynb), 16 cellules : 8 md + 8 code exécution réelle ; lab `OrleansAgentLab/` .NET 10 silo co-hosté `Microsoft.Orleans.Server` 10.3.1) | [#10473](https://github.com/jsboige/CoursIA/issues/10473) (reste OPEN), claim [#16436](https://github.com/jsboige/CoursIA/issues/16436) → PR [#16439](https://github.com/jsboige/CoursIA/pull/16439) (c.1208 lane `myia-po-2027:CoursIA-2`, repair commit `990ac290f699`) | Modèle d'acteurs (grains single-threaded turn-based, identité = clé `(type, key)`) avec concurrence sérialisante sans `lock`/`Interlocked` vs `asyncio`/`ray` où la discipline de concurrence reste au codeur. Concurrence mesurée : 50 appels simultanés de `RecordUsage(10)` sur le même grain → total exact `2125` (somme attendue) sans verrou | ~~co-host Aspire AppHost + `IGrainWithGuidKey`~~ **livré** par [`Orleans/02-Orleans-Aspire-CoHost.ipynb`](../Orleans/02-Orleans-Aspire-CoHost.ipynb) (lab `OrleansAspireLab/` : AppHost file-based `Aspire.Hosting.Orleans` 13.4.6 + CLI `aspire run`, silo ressource orchestrée, identités générées, reprise par Guid cross-process). Suite éventuelle : brancher `WithGrainStorage` sur un provider réel (persistance) |
