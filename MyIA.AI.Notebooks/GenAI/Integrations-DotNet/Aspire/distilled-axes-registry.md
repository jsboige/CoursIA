...
| **Channels / Orleans** | **Livré** (notebook [`Orleans/01-Orleans-Grains-Agents.ipynb`](../Orleans/01-Orleans-Grains-Agents.ipynb), 15 cellules : 8 md + 7 code exécution réelle ; lab `OrleansAgentLab/` .NET 10 silo co-hosté `Microsoft.Orleans.Server` 10.3.1) | [#10473](https://github.com/jsboige/CoursIA/issues/10473) (reste OPEN), claim [#16436](https://github.com/jsboige/CoursIA/issues/16436) → PR [#16439](https://github.com/jsboige/CoursIA/pull/16439) (c.1208 lane `myia-po-2027:CoursIA-2`, head exact `8351cb1a5fe9`) | Modèle d'acteurs (grains single-threaded turn-based, identité = clé `(type, key)`) avec concurrence sérialisante sans `lock`/`Interlocked` vs `asyncio`/`ray` où la discipline de concurrence reste au codeur. Concurrence mesurée : 50 appels simultanés de `RecordUsage(10)` sur le même grain → total exact `2125` (somme attendue) sans verrou | Densification future côté Orleans (axe successeur) : co-host Aspire AppHost + `IGrainWithGuidKey` (identités générées) |
