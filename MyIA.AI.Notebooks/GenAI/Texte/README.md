...
<p align="center">
  <img src="assets/readme/texte-scaling-passk.png" alt="Line plot matplotlib « Scaling du test-time compute (BoN) par difficulte » (axe X Budget d'echantillons k = 1/2/4/6, axe Y pass@k 0.0→1.0) ; 3 courbes (Facile vert + Moyen bleu en plateau saturé 1.0 dès k=1 ; Difficile rouge en montée monotone, du run committé 0.58→0.66→0.79→0.89, sans atteindre 1.0) — révèle que seules les questions difficiles bénéficient du budget k." width="420"/><br>
...
**BoN vs Réflexion : seul le bucket difficile tranche.** Comparer, à budget de tokens égal, le Best-of-N parallèle (essais indépendants) à la Réflexion séquentielle (un essai qui se corrige) ne discrimine que sur les problèmes durs : les buckets *facile* et *moyen* saturent au plafond pour les deux stratégies, donc n'apprennent rien, tandis que sur le *difficile* le parallélisme BoN passe devant la Réflexion — la *frontière compute-optimale* de Snell se lit là où le single-shot échoue. Le run committé (échantillonnage non seedé) donne BoN 0.79 contre Réflexion 0.50 ; un run antérieur donnait l'égalité 0.50/0.50 — « BoN n'est jamais battu » tient sur les deux, la marge est ce qui varie.

<p align="center">
  <img src="assets/readme/texte-bon-vs-reflex.png" alt="Grouped barplot matplotlib « Compute-optimal : strategie gagnante selon la difficulte » (axe Y Taux de succes 0.0→1.0, axe X 3 buckets discret facile/moyen/difficile) ; 6 barres par paires (BoN orange + Reflexion violette) : facile et moyen = egalite a 1.00 pour les deux strategies, mais difficile = BoN 0.79 bat Reflexion 0.50 — seul le régime difficile discrimine les deux stratégies." width="420"/><br>
  <em><b>BoN vs Réflexion par difficulté</b> ([16](16_Scaling_Test_Time_Compute.ipynb)) — diagramme en barres groupées (taux de succès Y) : le bucket difficile est le seul où les deux stratégies se séparent, à budget de tokens égal.</em>
</p>

**Raisonnement natif vs scaling hand-rolled : le raisonnement gagne le bucket dur, mais de peu.** La comparaison cost-normalisée — en *tokens dépensés* plutôt qu'en échantillons — oppose le BoN « fait maison » (courbes) au raisonnement natif de deepseek-r1 (croix, un seul essai). Sur les buckets *facile* et *moyen*, les deux stratégies saturent à 1.00 et le graphique ne discrimine rien. Sur le *difficile*, r1 atteint 1.00 pour ~1186 tokens (dont ~840 de raisonnement interne) quand le BoN plafonne à 0.95 après 405 tokens cumulés : il ne franchit jamais le dernier point. La direction du verdict tient donc, mais **la marge est instable** — un run antérieur du même notebook donnait un BoN difficile bloqué à ~0.49, contre 0.95 ici, et r1 y coûtait ~735 tokens au lieu de ~1186. Les deux mesures sont non seedées et portent sur n=2 problèmes par bucket : ce que la figure démontre solidement est la **méthode** cost-normalisée (tokens de raisonnement inclus), pas l'ampleur de l'écart.

<p align="center">
  <img src="assets/readme/texte-reason-vs-scale.png" alt="Plot matplotlib « Raisonnement natif vs scaling hand-rolled (cost-normalise) » (axe X Tokens depenses ~55→1186, axe Y Taux de succes 0.0→1.0) ; 6 séries : BoN facile/moyen (vert/bleu cercles) plateau 1.0 sur x≈55→381, r1 facile (X vert) a 1.0 pour 257 tokens + r1 moyen (X bleu) a 1.0 pour 450 tokens, BoN difficile (rouge cercles) en montee 0.62→0.95 sur x=100→405, et r1 difficile (X rouge) qui atteint 1.0 a x≈1186 — sur le bucket difficile le raisonnement natif franchit seul le plafond 1.0, mais a ~3x le cout token du meilleur point BoN (Snell 2024)." width="500"/><br>
  <em><b>Raisonnement natif vs scaling hand-rolled</b> ([17](17_Native_Reasoning_vs_Scaling.ipynb)) — courbes (BoN) et croix (r1) du taux de succès vs <b>tokens dépensés</b> (comparaison cost-normalisée) : sur le bucket difficile la croix atteint 1.00 là où la courbe BoN plafonne à 0.95, et sur facile/moyen les deux saturent — le graphique ne discrimine que le régime dur.</em>
</p>

Provenance et poids de chaque figure : [`assets/readme/MANIFEST.md`](assets/readme/MANIFEST.md).

Ces trois PNG ne sont **pas** des copies manuelles : les notebooks 16 et 17 les écrivent eux-mêmes dans `assets/readme/` quand la variable d'environnement `COURSIA_README_ASSETS=1` est armée (désarmée par défaut, donc une exécution ordinaire n'écrit rien sur disque). Le fichier écrit est **byte-identique** à l'image affichée dans le notebook — les deux sortent du même tampon PNG. Pour régénérer les figures, ré-exécuter les deux notebooks avec ce drapeau :

```bash
cd MyIA.AI.Notebooks/GenAI/Texte
COURSIA_README_ASSETS=1 python -m papermill 16_Scaling_Test_Time_Compute.ipynb /tmp/nb16.ipynb -k python3
COURSIA_README_ASSETS=1 python -m papermill 17_Native_Reasoning_vs_Scaling.ipynb /tmp/nb17.ipynb -k python3
```

Les mesures de ces notebooks sont **stochastiques** (échantillonnage LLM non seedé) : les valeurs citées ci-dessus sont celles du run committé, et un nouveau run les fera bouger. C'est précisément pourquoi les figures sont désormais produites par le notebook plutôt que recopiées — la figure et les sorties qui la justifient ne peuvent plus diverger.
