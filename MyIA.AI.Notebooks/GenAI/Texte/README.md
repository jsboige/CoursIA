...
**BoN vs Réflexion : la discrimination attendue n'apparaît pas à ce n.** Comparer Best-of-N parallèle (plusieurs essais indépendants) à la Réflexion séquentielle (un essai qui se corrige) est la comparaison compute-optimale de Snell. Sur le run committé (6 échantillons par problème, K=4, `llama-3.3-70b-instruct`), les deux stratégies rendent **le même taux sur les trois buckets** — 1.00 en *facile* et *moyen*, 0.50 en *difficile*. Snell prédit que le séquentiel devient compute-optimal quand la difficulté monte ; le mesurer demande un n et un modèle plus grands, et le notebook assume ce résultat négatif en section 4 (limites honnêtes, G.2) plutôt que de l'habiller.

<p align="center">
  <img src="assets/readme/texte-bon-vs-reflex.png" alt="Grouped barplot matplotlib « Compute-optimal : strategie gagnante selon la difficulte » (axe Y Taux de succes 0.0→1.0, axe X 3 buckets discret facile/moyen/difficile) ; 6 barres par paires (BoN orange + Reflexion violette) : facile et moyen = egalite a 1.00 pour les deux strategies, et difficile = BoN 0.50 contre Reflexion 0.50 — egalite stricte sur les trois buckets a ce n." width="420"/><br>
  <em><b>BoN vs Réflexion par difficulté</b> ([16](16_Scaling_Test_Time_Compute.ipynb)) — diagramme en barres groupées (taux de succès Y) : sur ce run les deux stratégies sont à égalité sur chaque bucket, le résultat négatif que le notebook assume (G.2).</em>
</p>
