...
| [2.11-Regression-Grande-Dimension](2.11-Regression-Grande-Dimension.ipynb) | Régression en grande dimension : OLS non identifiable, ridge, PCR, PLS | **La variance n'est pas la prédictivité** : la composante à 38 % de variance est muette (corr ≈ 0), le signal vit sur une composante à 2 % (corr 0.85) | synthétique à facteurs latents (p = 200, n = 60) |
...
> **Le régime extrême.** Le notebook [2.11](2.11-Regression-Grande-Dimension.ipynb) pousse la régression là où le socle ne va pas : p = 200 variables pour n = 60 observations. Le critère OLS y devient **non identifiable** — deux solutions exactes, indiscernables sur le train (RMSE 0.0000 l'une comme l'autre), divergentes en test (1.54 contre 2.13) — avant que le ridge (α choisi par CV), la PCR et la PLS ne réorganisent le problème. Le concept-phare s'y lit dans un seul tableau : la première composante principale porte 38 % de la variance et ne prédit rien (|corr| ≈ 0.04), celle qui porte le signal en porte 2 % (|corr| = 0.85) — d'où la PLS, qui construit ses composantes en regardant `y`. Verdict multi-graines à protocole constant : OLS dernier dans les deux régimes, et l'écart PCR/PLS dépend de l'alignement du signal sur la variance.
...
flowchart TD
    ...
    L["2.11 - Régression en grande dimension<br/>p >> n : OLS non identifiable<br/>ridge, PCR, PLS ; variance ≠ prédictivité"]
    C -. "quand p dépasse n" .-> L
    F -. "les composantes deviennent régresseurs" .-> L
```
...
11. **Régresser en grande dimension (p >> n)** : diagnostiquer la non-identifiabilité d'OLS (rang, noyau, deux solutions exactes divergentes en test), régulariser (ridge, α par validation croisée), régresser sur des composantes non supervisées (PCR) ou supervisées (PLS), et lire la différence entre direction de grande variance et direction prédictive (2.11).
