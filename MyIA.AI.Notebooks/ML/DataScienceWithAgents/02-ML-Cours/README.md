...
Cette série comble ce socle manquant. Elle pose, **à la main et de façon canonique**, les chapitres fondamentaux du machine learning supervisé et non supervisé — le référent qui rend *jugeable* ce qu'un agent produira ensuite. L'arc pédagogique suit la progression classique : le **workflow** d'ensemble, puis on ouvre les boîtes noires (**descente de gradient**, **fonction de lien**), on élargit la famille de modèles (**régression linéaire/logistique**, **naive Bayes génératif**, **arbres et ensembles**, **SVM à noyau et k plus proches voisins**), on formalise l'évaluation (**biais-variance, validation croisée, ROC, calibration des probabilités, équité par sous-groupe**), puis l'on bascule en **non supervisé** (**clustering, ACP**), avant de clore par le **cadre théorique** (**théorie PAC, dimension VC**). Chaque notebook rend visible un concept-phare — le surapprentissage, la divergence d'un learning rate, la frontière de décision, la réduction de variance, le coût d'un seuil, le kernel trick, la structure retrouvée sans étiquettes, et le nombre d'exemples suffisant pour généraliser.
...
| [2.3b-Naive-Bayes-Generatif](2.3b-Naive-Bayes-Generatif.ipynb) | Naive Bayes génératif vs régression logistique discriminative | **Génératif vs discriminatif** : modéliser `P(x\|y)P(y)` ou directement `P(y\|x)` | synthétique gaussien |
...
| [2.5c-Equite-Par-Sous-Groupe](2.5c-Equite-Par-Sous-Groupe.ipynb) | Équité par sous-groupe : trois critères, leurs incompatibilités, deux mitigations | **Ce que la métrique globale masque** : parité / égalité des chances / equalized odds — dont certains **mathématiquement incompatibles** | synthétique contrôlé (2 groupes, base rates ≠) |
...
> **L'équité par sous-groupe.** Le notebook [2.5c](2.5c-Equite-Par-Sous-Groupe.ipynb) prolonge le bloc d'évaluation (2.5, 2.5b, 2.13) au-delà de la métrique globale : trois critères d'équité vérifiables à la main sur une table de 20 lignes, un écart de TPR de 0,32 masqué par une AUC globale honorable, la démonstration empirique du théorème de Chouldechova (calibration par groupe + base rates différents ⟹ equalized odds **impossible**, même par post-traitement), et deux mitigations comparées sur le même split — reweighting (l'attribut à l'entraînement) contre thresholding par groupe (l'attribut à l'inférence) — avec la frontière performance–équité qui rend le compromis visible. Aucune métrique n'y est présentée comme décision normative : le choix appartient au contexte d'usage.

> **Épilogue — au-delà du socle scikit-learn.** Les chapitres ci-dessus posent le socle canonique. Le notebook [2.9](2.9-Grokking-Generalisation.ipynb) fait un pas de côté vers le **deep learning** : il entraîne le premier réseau de neurones de la série (PyTorch, quelques minutes sur CPU) pour montrer le **grokking** — la généralisation qui surgit longtemps *après* la mémorisation parfaite, un phénomène que la borne PAC (2.8) ne laissait pas prévoir — et réutilise l'ACP du 2.6 pour révéler la structure apprise.
...
**[2.5c — Le compromis performance–équité, rendu visible.](2.5c-Equite-Par-Sous-Groupe.ipynb)** Deux groupes, deux base rates, une accuracy globale de 0,80 qui ne veut rien dire : le groupe B a un TPR de 0,50 là où le groupe A atteint 0,82 — l'écart vit dans les faux négatifs, invisible de l'AUC. La figure balaie le seuil global et trace la **frontière performance–équité** : chaque point est un modèle, aucun ne domine les autres, et les deux mitigations (reweighting, thresholding par groupe) s'y placent honnêtement — l'une adoucit l'écart sans le fermer, l'autre égalise les chances au prix d'un arbitrage que le théorème de Chouldechova rend inévitable.

<p align="center"><a href="2.5c-Equite-Par-Sous-Groupe.ipynb"><img src="assets/readme/ml25c-frontiere-equite.png" width="540" alt="Frontière performance–équité : accuracy globale vs écart de TPR entre groupes (courbe du balayage de seuil + trois modèles annotés : baseline, reweighting, thresholding par groupe)."></a></p>
...
Le fil rouge de la série : on pose le **workflow**, on ouvre les **boîtes noires** (descente de gradient, fonction de lien), on élargit la **famille de modèles** (linéaire/logistique, naive Bayes, arbres, ensembles, SVM à noyau et k-NN), on formalise l'**évaluation** (biais-variance, validation croisée, ROC, calibration des probabilités, équité par sous-groupe), puis l'on bascule en **non supervisé** (clustering, ACP), avant de clore par le **cadre théorique** (théorie PAC, dimension VC). Chaque notebook rend visible un concept-phare distinct.
...
flowchart TD
    subgraph SUP["Apprentissage supervisé (2.1 à 2.5, 2.7)"]
      ...
      E3["2.5c - Équité par sous-groupe<br/>parité / equal opportunity / equalized odds<br/>incompatibilités, frontière performance–équité"]
...
11. **Évaluer l'équité par sous-groupe** : distinguer demographic parity, equal opportunity et equalized odds, les vérifier sur une table contrôlée, comprendre pourquoi certains sont **mathématiquement incompatibles** (Chouldechova, Kleinberg), comparer des mitigations (reweighting vs thresholding) sur le même split en traçant la frontière performance–équité, et situer où l'attribut sensible entre dans le pipeline (2.5c).
