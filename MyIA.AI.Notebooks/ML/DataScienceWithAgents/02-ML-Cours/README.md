...
| [2.7c-Proximal-Operators-From-Scratch](2.7c-Proximal-Operators-From-Scratch.ipynb) | *Accrétion de 2.7/2.7b* — opérateurs proximaux : ISTA (forward-backward splitting) et FISTA (inertie de Nesterov, `O(1/k²)`) pour le Lasso ; sparse recovery synthétique `n=200, p=500, k=30` | **Trois solveurs convergent vers la même solution** : ISTA, FISTA, `sklearn.Lasso` (coord descent) et `cvxpy` CLARABEL (SOCP) donnent tous `‖x−x*‖₂ ≈ 0.13` ; FISTA gagne ~k fois en itérations vs ISTA | synthétique (signal sparse gaussien, mesures bruitées) |
