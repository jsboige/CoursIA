...
| [3.6c-Modeles-Generatifs-Diffusion-from-scratch](3.6c-Modeles-Generatifs-Diffusion-from-scratch.ipynb) | Le DDPM démonté pièce par pièce : forward en forme fermée, réseau ε_θ, loss MSE sur le bruit, sampler ancestral — puis la comparaison mesurée des schedules linéaire et cosinus sur une architecture identique | **Ce que `DDPMScheduler` cachait** : chaque pièce du pipeline écrite à la main, puis mise en face de son équivalent `diffusers` | ᾱ final 4,0e-5 (linéaire) vs 2,4e-9 (cosinus) ; ᾱ sous 0,5 au pas 259 vs 496 ; loss finale 0,0564 vs 0,1014 ; MMD 0,00735 vs 0,00869 (plancher vrai/vrai 0,00324) — le classement s'inverse à σ = 0,5 : bande passante du noyau, verdict non tranché ; échantillonnage 5,3-5,7 s (3 tirages, écart-type 0,24 s) |
| [3.6d-Modeles-Generatifs-Score-SDE-from-scratch](3.6d-Modeles-Generatifs-Score-SDE-from-scratch.ipynb) | Le passage à la limite continue du `3.6c` : β(t) fonction au lieu de tableau, le réseau apprend le score ∇_x log p_t(x), et trois intégrateurs comparés — Euler-Maruyama, Langevin recuit, prédicteur-correcteur | **La vue continue** : la même mécanique décrite par une SDE, où le signe de la dérive — invisible à la relecture — est vérifié par la mesure | contrôle Euler-Maruyama à score exact : MMD 0,00025 (plancher 0,00016) ; avec réseau : Euler-Maruyama 0,00156 (5/5 modes, 1,0 s) < prédicteur-correcteur 0,00259 (1,8 s) < Langevin recuit 0,00894 (12,6 s) ; l'écart ×6,14 au contrôle entièrement attribuable au réseau ; entraînement 49,2 s (8 000 pas) |
| [3.6e-Modeles-Generatifs-Conditionnels-from-scratch](3.6e-Modeles-Generatifs-Conditionnels-from-scratch.ipynb) | La génération conditionnelle et le *classifier-free guidance* : conditionnement par étiquette (temb(t) + emb_classe(c)), token nul ∅, et le poids w entre bruit conditionnel et inconditionnel | **L'arbitrage fidélité/diversité au poids w** : l'adhérence monte de la chance (0,089) à 0,989 pendant que la diversité descend — un curseur continu et mesuré | plancher vrai/vrai 0,01151 (n = 256 contre 256) ; plafond du classifieur 0,824 ; w=0 : adhérence 0,089 / diversité 3,379 (l'étiquette est ignorée) ; w=1 : 0,782 / 2,711 ; w=2 : 0,970 / 2,372 ; w=3 : 0,989 / 2,213 ; ~12,5 s pour 256 échantillons par classe |
