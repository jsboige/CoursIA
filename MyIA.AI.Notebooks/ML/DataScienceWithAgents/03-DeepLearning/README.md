...
**Kernel** : Python 3 (`coursia-ml-training` pour 3.7) · **Bibliothèques** : NumPy (implémentations from scratch), matplotlib, torch (3.6c, 3.6d, 3.6e, 3.7, 3.9a) · **Niveau** : intermédiaire (post socle ML) · **CPU** : oui (exception 3.9a : entraînement ResNet-20 sur GPU, ~8 min)
...
| [3.6c-Modeles-Generatifs-Diffusion-from-scratch](3.6c-Modeles-Generatifs-Diffusion-from-scratch.ipynb) | Le DDPM écrit à la main (schedule β linéaire **et** cosinus, forward fermé `q_sample`, cible ε, boucle reverse, variance a posteriori) : deux modèles entraînés identiques à une exception — le schedule — pour départager linéaire vs cosinus sur MNIST 8×8 | **La loss est une régression sur le bruit, pas sur l'image** — et elle ne se compare pas entre schedules : changer β_t change la distribution des tâches, seul le plancher MMD compare équitablement | MMD noyau RBF (1024 échantillons, plancher vrai/vrai mesuré à effectif égal 0,00324, biais 2/n exposé) : schedule linéaire 0,00735, cosinus 0,00869 — et le rang s'inverse selon la bande passante σ (σ=0,5 : cosinus devant ; σ=1,0 : linéaire devant) ; latence mesurée 3× ; loss 0,056 vs 0,101 explicitement déclarée non comparable ; table de correspondance pièce par pièce avec `diffusers` ; 3 exercices |
| [3.6d-Modeles-Generatifs-Score-SDE-from-scratch](3.6d-Modeles-Generatifs-Score-SDE-from-scratch.ipynb) | La vue continue de la diffusion (VP-SDE) : dérive et diffusion, score exact d'un mélange gaussien, conversion ε→score, trois samplers (Euler-Maruyama, Langevin recuit, prédicteur-correcteur) sur une cible 2D à 5 modes | **Le signe de la dérive** : quatre combinaisons de signes testées, une seule échantillonne — le mauvais signe diverge avec une variance qui explose à 10¹², pas un simple « ça marche moins bien » | signe retenu MMD 0,00079, 5/5 modes ; mauvais signe MMD 0,680, 1/5 mode, variance 2,0 → 10¹² le long de t ; Euler-Maruyama 0,00156 en 1,0 s contre Langevin recuit 0,00894 en 12,6 s et prédicteur-correcteur 0,00259 en 1,8 s ; contrôle au score exact 0,00025, plancher vrai/vrai 0,00016 (N = 2048) ; variance empirique vs attendue vérifiée à chaque t (1,96 contre 2,00 à t = 1) ; table de correspondance `diffusers` ; 3 exercices |
| [3.6e-Modeles-Generatifs-Conditionnels-from-scratch](3.6e-Modeles-Generatifs-Conditionnels-from-scratch.ipynb) | La génération conditionnelle et le classifier-free guidance écrits à la main : conditionnement par étiquette (`temb(t) + emb_classe(c)`), token nul ∅, abandon d'étiquette pendant l'entraînement, combinaison `eps_u + w·(eps_c − eps_u)` à l'échantillonnage | **La guidance est un arbitrage fidélité/variété, pas un réglage de qualité** : le curseur w déplace l'échantillon entre « respecter la classe » et « garder la diversité » — mesuré sur trois axes, pas un seul | balayage w ∈ {0, 1, 2, 3, 5} : MMD par classe 0,055 → 0,021 (minimum à w = 1) → 0,161 ; adhérence à la classe 0,089 → 0,991 ; diversité 3,38 → 2,10 — les trois axes racontent un déplacement, pas une montée ; test nul (sans guidance, la génération doit ignorer la classe) ; 3 exercices |
...
# notebooks 3.6c, 3.6d, 3.6e, 3.7 et 3.9a (torch + torchvision, kernel coursia-ml-training pour 3.7) :
