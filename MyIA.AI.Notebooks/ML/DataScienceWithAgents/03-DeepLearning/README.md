...
**Kernel** : Python 3 (`coursia-ml-training` pour 3.7) · **Bibliothèques** : NumPy (implémentations from scratch), matplotlib, torch (moteur principal de 3.6b à 3.10 ; parité ailleurs), torchvision, diffusers (3.10) · **Niveau** : intermédiaire (post socle ML) · **CPU** : oui (exceptions : famille compression 3.9a/3.9e/3.9f, recette complète ResNet-20 sur GPU ~8 min — recette CPU 6 époques sinon ; 3.6c ~19 min CPU)
...
Tous les notebooks tournent sur CPU — la plupart en moins de dix minutes. Exceptions : la famille compression ([3.9a](3.9a-Compression-Quantization-INT8.ipynb), [3.9e](3.9e-Compression-Quantization-SOTA.ipynb), [3.9f](3.9f-Compression-Pruning-SOTA.ipynb)) entraîne ResNet-20 sur CIFAR-10 — la recette complète (40 époques, ~8 min sur RTX 3090) exige un GPU ; sur CPU ces notebooks tournent une recette réduite de 6 époques (~9-15 min), le témoin GPU de 3.9a (0,9019) servant de référence croisée. Et [3.6c](3.6c-Modeles-Generatifs-Diffusion-from-scratch.ipynb) demande ~19 min CPU pour ses 16 entraînements.
