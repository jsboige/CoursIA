...
- **B.5 (livré)** - WS-01 : débruitage SOTA (BayesShrink/SureShrink/VisuShrink) confronté au seuillage from scratch.
- **Bloc B (restant)** - kymatio (scattering optimisé, WS-02 en PR), tableau comparatif final from scratch vs SOTA.

## Prérequis

Python 3.10+, `numpy`, `matplotlib` (figures des notebooks), `scipy` (DCT 8×8 servant de baseline en compression, WS-00b), `scikit-learn` (régression logistique du WS-00c), `scikit-image` (débruitage SOTA du bloc B, WS-01). Dans les notebooks from scratch (WS-00a/WS-00b/WS-00c), `pywt` (PyWavelets) et `kymatio` ne sont utilisés que pour la validation croisée, jamais pour calculer ; le bloc B (WS-01) inverse délibérément le cap — les libs calculent, le from scratch devient la baseline. Les bases d'algèbre linéaire (orthonormalité, adjoint) et de traitement du signal (FFT) suffisent ; `02-ML-Cours` (2.6, clustering/PCA) et `03-DeepLearning` (3.0, théorie de l'information) fournissent les ponts utiles.
