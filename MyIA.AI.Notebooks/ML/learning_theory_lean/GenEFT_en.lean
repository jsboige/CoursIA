...
theorem competition_invariant (ηx ηA : ℝ) (a₂ c : ℝ → ℝ)
  ...
  · -- ηx = 0: dc/dt = 0, the quantity reduces to -2 ηA c², zero derivative
    -- (rfl substituted ηx := 0 everywhere: the types below use the literal)
  ...
  · -- ηA = 0: da₂/dt = 0, the quantity reduces to ηx a₂², zero derivative
    ...
    simp
  -- generic case (ηx ≠ 0, ηA ≠ 0): delegation to the organ — the
  -- normalized invariant C = a₂²/(2ηA) − c²/ηx lives in
  -- EffectiveTheory.Repons; our form is just the multiple 2 ηA ηx · C.
  -- the organ expects the unparenthesized form -2 * ηA * ..., ours is -(2 * ηA) * ... (same term up to ring)
