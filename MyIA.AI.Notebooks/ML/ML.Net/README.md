...
### Fonctionnalités avancées (ML-5 à ML-11)
...
| 11 | [ML-11-MatrixProfile-Multidim](ML-11-MatrixProfile-Multidim-Python.ipynb) | **Matrix Profile multidimensionnel** : détection sans apprentissage (`stumpy`), condensation `min`/`max`, mSTAMP, l'anomalie répétée que `k=1` cesse de voir | *standalone Python (stumpy — la réponse constructive à ML-10)* | 45-60 min |
...
### Phase 2 : Fonctionnalités avancées (ML-5 à ML-11, ~4h30)
...
### ML-11-MatrixProfile-Multidim-Python

| Section | Contenu |
|---------|---------|
| Matrix Profile | Distance à la sous-séquence la plus proche (`stumpy.stump`) — aucune phase d'entraînement |
| Anomalie conjointe | Trois capteurs dont les **marginales ne bougent pas** : la carte `M+3σ` de ML-10 et l'ACP de ML-9 restent aveugles |
| Condensation | `min` (ET logique) vs `max` (OU) sur les profils univariés |
| mSTAMP | Profil à `K` dimensions (`stumpy.mstump`) — `K=3` redevient aveugle : plus de dimensions peut être pire |
| Effondrement à `k=1` | L'anomalie répétée devient sa propre plus proche voisine ; `k>=2` la restaure |
| Décalage de `m` points | Le discord tombe *avant* l'anomalie — le piège 3 de ML-10, vu de l'intérieur |
...
ML-10-TSAD-Benchmark-Flaws (introspection des benchmarks)
    |
ML-11-MatrixProfile-Multidim (Matrix Profile, détection sans apprentissage)
```
