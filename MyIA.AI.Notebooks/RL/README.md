...
> **Note éditoriale (counts)** : Le marqueur `CATALOG-STATUS` ci-dessus est autoritatif pour le compte agrégé (26 notebooks pédagogiques). Pour la **décomposition langagière par kernel** (`metadata.kernelspec.language`), ce README reste autoritatif car la granularité kernel n'est pas dans le marqueur agrégé ; elle est documentée ici par lecture directe des kernelspecs au 16/09/2026 :
>
> **Python 100% = 31/31 fichiers mono-langage ✓** (31 fichiers `*.ipynb` sur disque hors `_output.ipynb` — 28 kernels `python3` + 3 kernels `coursia-ml-training`, tous `language: python` ; l'écart avec le `pedagogical_count: 26` du marqueur correspond aux fichiers non catalogués ; exécution via MCP Jupyter `nbconvert --execute --to notebook` ou `jupyter nbconvert --execute --inplace`).
>
