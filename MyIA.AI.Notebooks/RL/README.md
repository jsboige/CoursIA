...
| pt-0d | [rlpt_0d_reward_trainer_sota](rlpt_0d_reward_trainer_sota.ipynb) | Le **même** problème que `rlpt_0`, traité par le harnais industriel `trl.RewardTrainer` : monde, juge, architecture (5 441 paramètres) et seeds identiques, une seule variable — le harnais. Documente les **trois exigences de contrat** non devinables (`num_labels=1`, `processing_class`, `gradient_checkpointing=False`) et la traduction `y` → `chosen_ids`/`rejected_ids` ; verdict mesuré : les deux harnais sont **statistiquement indistinguables** (0.656 ± 0.018 vs 0.658 ± 0.008, écart 0.3σ) pour un coût en temps de **~14×** sur ce modèle minuscule, et l'évaluation honnête reste **entièrement à ré-écrire** — `trl` n'évalue pas pour vous | 35-40 min |
...
| « Le harnais SOTA (`trl`) fait-il mieux que ma boucle from scratch — et que me coûte-t-il ? » | [`rlpt_0d`](rlpt_0d_reward_trainer_sota.ipynb) | `trl.RewardTrainer` sur le **même** monde que `rlpt_0`, à architecture identique : les deux sont statistiquement indistinguables (0.3σ), le harnais coûte ~14× le temps sur un modèle minuscule, impose trois réglages de contrat, et n'évalue pas pour vous. |
