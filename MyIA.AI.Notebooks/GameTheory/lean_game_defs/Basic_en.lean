/-
  ...
  Convention i18n Lean #4980 (ratified 2026-07-04): English mirror of
  `Basic.lean` (FR-first canonical, namespace-less — declarations at root).
  This twin wraps its declarations in `namespace LeanGameDefs_en` to avoid
  name collisions with the FR canonical when both are built in lib
  `LeanGameDefs` (`NormalFormGame` at root ↔ `LeanGameDefs_en.NormalFormGame`).
  Only docstrings/comments differ between the two files; the code body —
  signatures, structures, defs — stays byte-identical.
-/

namespace LeanGameDefs_en
