/-
  ...
  ---
  i18n sibling: this module is the EN mirror of the FR canonical `Bayesian.lean`
  (sibling-pair Pattern A, ratified by user on 2026-07-04, EPIC #4980). The FR
  canonical lives in `Bayesian.lean` (top-level declarations, no namespace).
  This EN mirror uses the *reuse-FR-names* variant (like `Nash_en.lean`): it
  `import Basic` (the FR canonical base module, matching the FR file) and
  redeclares its own declarations inside `namespace BayesianEn` to avoid name
  collisions with the FR `Bayesian` module when both live in the same lake. The
  body of definitions, signatures, structures, inductives, instances and
  tactics is byte-identical between the two files; only the docstrings
  `/-! ... -/` (section headings) and `/-- ... -/` (field and declaration
  docstrings) differ — EN in this file, FR in `Bayesian.lean`.
-/

import Basic

namespace BayesianEn
...
def auctionWinner {n : Nat} (h_n : n ≥ 1) (bids : Fin n → Nat) : Fin n :=
  ...
  -- The first player who bid the maximum wins (breaks ties)
...
-- no `deriving Repr`: `cards` is a functional type, not derivable. Manual instance:
...
def kuhnPayoff (cards : Fin 2 → KuhnCard) (history : List KuhnAction) : Int :=
  ...
  | [KuhnAction.check, KuhnAction.bet, KuhnAction.fold] => 1  -- P1 wins the pot
  | [KuhnAction.check, KuhnAction.bet, KuhnAction.call] =>
    if cards 0 > cards 1 then 2 else -2  -- pot = 2
  | [KuhnAction.bet, KuhnAction.fold] => 1   -- P2 folds, P1 wins the pot
