/-
  ...
  `Combinatorial.lean` (sibling-pair Pattern A, ratified by user on 2026-07-04,
  EPIC #4980). The FR canonical lives in `Combinatorial.lean` (top-level
  declarations, no namespace). This EN mirror uses the *redeclare-in-namespace*
  variant: it redeclares the entire self-contained module inside
  `namespace CombinatorialEn` (the module has no `import`, so no FR dependency).
  The body of definitions, signatures, structures, inductives and tactics is
  byte-identical between the two files; only the docstrings `/-! ... -/`
  (section headings) and `/-- ... -/` (field and declaration docstrings) differ
  — EN in this file, FR in `Combinatorial.lean`. The `_en` namespace suffix
  prevents name collisions with the FR `Combinatorial` module when both live in
  the same lake.
-/

namespace CombinatorialEn
...
def minimax : GameTree → Bool → Int
  | GameTree.leaf w, _ => w
  | GameTree.node [], _ => 0  -- No move = draw
