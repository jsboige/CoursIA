/-
  ...
  canonique `Basic.lean`). Aucune library externe n'importe ce module,
  les noms `SocialChoice_en.P`, `SocialChoice_en.I`, etc. restent locaux
  au sibling.

  Source EN verbatim : aaaf0c52a (parent du commit c.218 FR-first).
...
lemma same_order_iff_same_order' {R R' : σ → σ → Prop} {x y : σ}
  ...
  · -- R y x → R' y x: via ¬P R x y → ¬P R' y x
