/-
  Valeur de Shapley — Caractérisation axiomatique
  =================================================

  Ce fichier formalise l'approche axiomatique de Shapley pour la division équitable :
  - les quatre axiomes de Shapley (Efficacité, Symétrie, Joueur nul, Additivité)
...
/-! ## Solutions et axiomes -/

/-- Un concept de solution assigne à chaque jeu un vecteur de paiements -/
...
/-! ## Les quatre axiomes de Shapley -/

/-- Axiome 1 : Efficacité
    Les paiements somment à la valeur de la grande coalition -/
def Efficiency : Prop :=
  ∀ G : TUGame N, ∑ i : N, φ G i = G.v Finset.univ

/-- Deux joueurs sont symétriques dans G s'ils apportent la même contribution -/
def SymmetricPlayers (G : TUGame N) (i j : N) : Prop :=
  ∀ S : Finset N, i ∉ S → j ∉ S →
    G.v (S ∪ {i}) = G.v (S ∪ {j})

/-- Axiome 2 : Symétrie
    Les joueurs symétriques reçoivent des paiements égaux -/
def Symmetry : Prop :=
  ∀ G : TUGame N, ∀ i j : N,
    SymmetricPlayers G i j → φ G i = φ G j

/-- Un joueur i est nul s'il n'apporte aucune valeur à aucune coalition -/
...
/-- Axiome 4 : Additivité
    La solution d'une somme de jeux vaut la somme des solutions -/
def Additivity : Prop :=
  ∀ G H : TUGame N, ∀ i : N,
...
/-- La valeur de Shapley : moyenne des contributions marginales sur tous les ordonnancements -/
...
/-! ## Caractérisation par jeux d'unanimité (toute solution axiomatique) -/

/-- Toute solution efficace, symétrique et respectant le joueur nul donne 1/|T|
    aux joueurs dans T et 0 aux joueurs hors de T dans le jeu d'unanimité u_T.
    Utilise directement les trois axiomes (efficacité, symétrie, joueur nul). -/
private theorem phi_unanimity (φ : Solution N)
  ...
  · -- Case i ∈ T: by symmetry all j∈T get same value, by efficiency sum=1
    ...
    have h_null_out : ∀ j, j ∉ T → φ (unanimityGame T hT) j = 0 := by
      ...
      · rfl
    -- Étape 3 : efficacité, somme = v(univ) = 1
...
/-- La valeur de Shapley satisfait l'axiome du joueur nul -/
...
private theorem shapleyCoef_shift (n s : ℕ) (hs : s + 2 ≤ n) :
  ...
  -- Convertir ↑(n - s - 2) en ↑n - ↑s - 2 à l'aide des lemmes de coercition Nat
...
private theorem shapleyCoef_top (n : ℕ) (hn : 0 < n) :
  ...
  -- But : ↑n * (↑(n-1).factorial / ↑n.factorial) = 1
...
private theorem pos_term_eq (G : TUGame N) :
  ...
  classical
  -- Étape 1 : déplacer le coefficient à l'intérieur de la somme interne du LHS
  ...
  -- Étape 3 : réécrire le RHS à l'aide de hT ponctuellement
  rw [Finset.sum_congr rfl (fun T _ => hT T)]
  -- Étape 4 : aplanir les deux côtés en sommes sigma, puis appliquer la bijection
  ...
  -- f dans l'image : p.2 ∈ p.1 ∪ {p.2}
  · intro p hp
    simp only [Finset.mem_sigma] at hp ⊢
    exact ⟨Finset.mem_univ _, by
      rw [Finset.union_comm]
      exact Finset.mem_insert_self _ _⟩
  -- g dans l'image : p.2 ∈ univ \ (p.1 \ {p.2}) (c-à-d p.2 ∉ p.1 \ {p.2})
  · intro p hp
    simp only [Finset.mem_sigma] at hp ⊢
  ...
  · intro p hp
    ...
    · rfl
  -- valeurs en accord : c(|S|) * v(S∪{i}) = c(|S∪{i}|-1) * v(S∪{i})
  · intro p hp
    simp only [Finset.mem_sigma] at hp
    have hni : p.2 ∉ p.1 := (Finset.mem_sdiff.mp hp.2).2
...
/-- La valeur de Shapley satisfait l'efficacité.
    ESQUISSE DE PREUVE :
    ∑ᵢ φᵢ(G) = ∑ᵢ ∑_{S:i∉S} c(|S|,n) · [v(S∪{i}) - v(S)]
    Échangeons l'ordre de sommation :
    = ∑_S ∑_{i∉S} c(|S|,n) · [v(S∪{i}) - v(S)]
    = ∑_S c(|S|,n) · ∑_{i∉S} [v(S∪{i}) - v(S)]
    Chaque i ∉ S contribue sa valeur marginale à S.
    Télescopage : ∑_{i∉S} v(S∪{i}) - (n-|S|)·v(S)
    Après réarrangement sur tous les S, les termes s'annulent pour laisser v(univ) - v(∅) = v(univ).
    L'identité clé : chaque v(S) apparaît comme +v(S) avec coefficient c(|S|-1,n)
...
theorem shapley_efficient (G : TUGame N) :
  ...
  simp only [← Finset.mul_sum]
  -- Découper la soustraction dans les sommes internes : ∑ (a - b) = ∑ a - ∑ b
  ...
  rw [pos_term_eq]
  -- Combiner en une somme unique de différences
  rw [← Finset.sum_sub_distrib]
  ...
  -- Terme positif : n * c(n,n-1) * v(univ) = v(univ) puisque n * c(n,n-1) = 1
...
/-- La valeur de Shapley satisfait la symétrie.
    ESQUISSE DE PREUVE (bijection de swap) :
    Définir f : {S | i ∉ S} → {T | j ∉ T} par :
    - f(S) = S                    si j ∉ S (i et j tous deux absents)
    - f(S) = (S\{j})∪{i}        si j ∈ S (swap j contre i)
    ...
    (3) f est une bijection (inverse : swap i↔j)
...
theorem shapley_symmetric (G : TUGame N) (i j : N)
  ...
  by_cases heq : i = j
  · subst heq; rfl
  -- Preuve par bijection de swap à l'aide de Finset.sum_bij
  -- La fonction directe envoie {S | i ∉ S} → {T | j ∉ T} par swap j↔i
  unfold shapleyValue TUGame.marginalContribution
  ...
  -- Définir la directe et l'inverse comme des definitions séparées pour qu'elles réduisent correctement
  ...
  case fwd_mem =>
    ...
    rw [hmem_src] at hS
    rw [hmem_tgt]
    -- But : j ∉ fwd S hS✝ où fwd S _ = if j ∈ S then (S.erase j) ∪ {i} else S
    simp only [fwd]
    split_ifs with hSj
...
/-- Valeur de Shapley sur les jeux d'unanimité : chaque joueur dans T obtient 1/|T| -/
theorem shapley_unanimity (T : Finset N) (hT : T.Nonempty) (i : N) :
    shapleyValue (TUGame.unanimityGame T hT) i =
    if i ∈ T then (1 : ℝ) / T.card else 0 := by
  classical
  split_ifs with hiT
  · -- Cas i ∈ T : par symétrie tous les j∈T ont la même valeur, par efficacité somme=1
...
/-- La valeur de Shapley satisfait l'additivité -/
...
/-! ## Multiplication scalaire et sommes finies de jeux -/

namespace Solution

/-- Multiplication scalaire d'un jeu TU par un réel -/
...
/-- Insérer dans finsetSumGames donne AddGames -/
...
/-! ## Additivité finie pour la valeur de Shapley -/

namespace ShapleyValue

/-- shapleyValue est homogène : shapleyValue (SmulGame c G) i = c * shapleyValue G i -/
...
/-- La valeur de Shapley est additive -/
...
/-- La reconstruction de Mobius de G : G = Σ_{T≠∅} a_T • u_T -/
...
/-- Lemme auxiliaire : pour R ⊂ S, la somme alternée sur les sur-ensembles T de R dans S est nulle.
...
private theorem mobius_inner_sum_zero (S R : Finset N) (hR : R ⊆ S) (hne : R ≠ S) :
  ...
  have h_eq :
    ...
    -- hi : l'application directe atterrit dans la cible
    · intro T hT
      simp only [Finset.mem_filter, Finset.mem_univ, true_and] at hT
      obtain ⟨hRT, hTS⟩ := hT
      rw [Finset.mem_powerset]
      exact Finset.sdiff_subset_sdiff hTS (Finset.Subset.refl R)
    -- hj : l'application inverse atterrit dans la source
    · intro m hm
    ...
    -- h : égalité des sommands via card_sdiff_of_subset
    · intro T hT
      simp only [Finset.mem_filter, Finset.mem_univ, true_and] at hT
      obtain ⟨hRT, _⟩ := hT
      congr 1
  ...
  rw [h_eq]
  -- Réécrire : ∑ m ∈ powerset, (-1 : ℝ)^m.card = ↑(∑ m, (-1 : ℤ)^m.card)
...
/-- Lemme auxiliaire : pour R = S, la somme interne vaut 1 (le singleton {S} contribue (-1)^0 = 1). -/
...
/-- Inversion de Mobius : v(S) = Σ_{∅≠T⊆S} a_T
    Tout jeu se décompose de manière unique en jeux d'unanimité pondérés (dividendes de Harsanyi).
...
private theorem mobius_decomposition_axiom (G : TUGame N) (S : Finset N) :
  ...
  rw [h_comm]
  -- Montrer maintenant que chaque R contribue soit G.v R (si R = S), soit 0 (sinon)
  ...
  · rw [if_pos hRS, hRS]
    by_cases hSe : S = ∅
    -- S = ∅ : le filtre est vide, somme = 0 = G.v ∅
    · rw [hSe]
  ...
  · rw [if_neg hRS]
    ...
    -- Sous-cas R ⊄ S : aucun T ne vérifie R ⊆ T ⊆ S
...
/- Théorème d'unicité de Shapley :
    ...
    Stratégie de preuve : montrer que toute solution axiomatique φ coincide avec
...
/-- φ coincide avec shapleyValue sur les jeux d'unanimité -/
...
/-- Symétrie des joueurs dans T au sein de SmulGame c u_T -/
...
/-- Joueurs nuls hors de T dans SmulGame c u_T -/
...
/-- φ sur un jeu d'unanimité pondéré : φ(SmulGame c u_T, i) = c * φ(u_T, i).
    Prouvé directement à partir des axiomes sans multiplication scalaire générale. -/
private theorem phi_weightedUnanimity (φ : Solution N)
  ...
  split_ifs with hiT
  -- Cas i ∈ T
  · -- Efficacité : Σ_j φ(H, j) = H.v(univ) = c
    ...
    -- Les joueurs hors de T contribuent 0
    ...
    have h_sum_T : ∑ j ∈ T, φ (Solution.SmulGame c (TUGame.unanimityGame T hT)) j = c := by
      ...
      linarith
    -- Tous les joueurs dans T obtiennent la même valeur
    have h_eq : ∀ j ∈ T,
...
/-- Toute φ axiomatique coincide avec shapleyValue sur SmulGame c u_T -/
...
/-- φ se distribue sur les sommes finies (à partir de l'additivité binaire par induction) -/
...
/-- La valeur de Shapley se distribue sur les sommes finies -/
...
/-- Le jeu G vaut le finsetSumGames de ses termes de décomposition de Mobius.
...
private theorem game_eq_mobius_sum (G : TUGame N) :
  ...
  classical
  -- Étape 1 : pour T ∈ filter Nonempty, f(T).v S = mobiusCoeff G T * (if T⊆S then 1 else 0)
...
/-- Unicité de la valeur de Shapley : toute solution axiomatique vaut la valeur de Shapley.
    Stratégie : décomposer G = ∑_{T≠∅} a_T · u_T via Mobius, puis φ et shapleyValue
    se distribuent sur la somme et coïncident sur chaque terme par phi_eq_shapley_weighted. -/
