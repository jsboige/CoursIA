...
def SymmetricPlayers (G : TUGame_en N) (i j : N) : Prop :=
  ∀ S : Finset N, i ∉ S → j ∉ S →
    G.v (S ∪ {i}) = G.v (S ∪ {j})
...
def NullPlayer (G : TUGame_en N) (i : N) : Prop :=
  ∀ S : Finset N, i ∉ S →
    G.v (S ∪ {i}) = G.v S
...
noncomputable def shapleyValue (G : TUGame_en N) (i : N) : ℝ :=
...
private theorem phi_unanimity (φ : Solution N)
    ...
    φ (TUGame_en.unanimityGame T hT) i =
...
theorem shapley_null_player (G : TUGame_en N) (i : N)
  ...
  have hmc : G.marginalContribution i S = 0 := by
    unfold TUGame_en.marginalContribution
    rw [h S hS]
    ring
  simp [hmc]
...
private theorem pos_term_eq (G : TUGame_en N) :
...
-- Linearity of summation
  unfold shapleyValue Solution.AddGames TUGame_en.marginalContribution
  simp only
...
def SmulGame (c : ℝ) (G : TUGame_en N) : TUGame_en N where
  v := fun S => c * G.v S
  empty_zero := by simp [G.empty_zero]

/-- The zero TU game -/
def zeroGame : TUGame_en N where
  v := fun _ => 0
  empty_zero := rfl
...
noncomputable def finsetSumGames {ι : Type*} (s : Finset ι) (f : ι → TUGame_en N) : TUGame_en N where
  v := fun S => ∑ j ∈ s, (f j).v S
...
theorem shapley_smulGame (c : ℝ) (G : TUGame_en N) (i : N) :
...
theorem shapley_addGames (G H : TUGame_en N) (i : N) :
...
noncomputable def weightedUnanimity (c : ℝ) (T : Finset N) (hT : T.Nonempty) : TUGame_en N where
...
noncomputable def mobiusReconstruction (G : TUGame_en N) : TUGame_en N where
...
private theorem phi_eq_shapley (φ : Solution N)
    ...
    (h_null : φ.NullPlayerAxiom)
    (T : Finset N) (hT : T.Nonempty) (i : N) :
    φ (TUGame_en.unanimityGame T hT) i = shapleyValue (TUGame_en.unanimityGame T hT) i := by
...
private theorem sym_in_smulUnanimity (φ : Solution N)
    ...
    φ (Solution.SmulGame c (TUGame_en.unanimityGame T hT)) i =
    φ (Solution.SmulGame c (TUGame_en.unanimityGame T hT)) j := by
  apply h_sym
  intro S hiS hjS
...
private theorem null_outside_smulUnanimity (φ : Solution N)
    ...
    φ (Solution.SmulGame c (TUGame_en.unanimityGame T hT)) k = 0 :=
  h_null (Solution.SmulGame c (TUGame_en.unanimityGame T hT)) k
    (fun S _ => by
      simp only [Solution.SmulGame, TUGame_en.unanimityGame]
...
private theorem phi_weightedUnanimity (φ : Solution N)
    ...
    φ (Solution.SmulGame c (TUGame_en.unanimityGame T hT)) i = c * φ (TUGame_en.unanimityGame T hT) i := by
  classical
  rw [phi_unanimity φ h_eff h_sym h_null T hT i]
  split_ifs with hiT
  -- Cas i ∈ T
  · -- Efficience : Σ_j φ(H, j) = H.v(univ) = c
    have h_eff_T : ∑ j : N, φ (Solution.SmulGame c (TUGame_en.unanimityGame T hT)) j = c := by
      ...
      exact (h_eff (Solution.SmulGame c (TUGame_en.unanimityGame T hT))).trans h_val
-- Players outside T contribute 0
    have h_null_out : ∀ j, j ∉ T →
        φ (Solution.SmulGame c (TUGame_en.unanimityGame T hT)) j = 0 :=
      fun j hj => null_outside_smulUnanimity φ h_null c T hT j hj
-- The sum over the complement of T is 0
    have h_out_sum : ∑ j ∈ Tᶜ, φ (Solution.SmulGame c (TUGame_en.unanimityGame T hT)) j = 0 :=
      Finset.sum_eq_zero (fun j hj => by
        simp only [Finset.mem_compl] at hj
        exact h_null_out j hj)
    -- ∑_{j∈T} φ(j) = c
    have h_sum_T : ∑ j ∈ T, φ (Solution.SmulGame c (TUGame_en.unanimityGame T hT)) j = c := by
      have h_split := Finset.sum_add_sum_compl T
          (fun j => φ (Solution.SmulGame c (TUGame_en.unanimityGame T hT)) j)
      linarith
-- All players in T get the same value
    have h_eq : ∀ j ∈ T,
        φ (Solution.SmulGame c (TUGame_en.unanimityGame T hT)) j =
        φ (Solution.SmulGame c (TUGame_en.unanimityGame T hT)) i := by
      intro j hjT
      by_cases hij : j = i; · subst hij; rfl
      exact (sym_in_smulUnanimity φ h_sym c T hT i j hiT hjT (Ne.symm hij)).symm
-- T.card * phi(H, i) = c
    have h_card : (T.card : ℝ) * φ (Solution.SmulGame c (TUGame_en.unanimityGame T hT)) i = c := by
      have : ∑ _ ∈ T, φ (Solution.SmulGame c (TUGame_en.unanimityGame T hT)) i =
          (T.card : ℝ) * φ (Solution.SmulGame c (TUGame_en.unanimityGame T hT)) i := by
        rw [Finset.sum_const, nsmul_eq_mul]
      rw [← this]
      exact (Finset.sum_congr rfl (fun j hj => (h_eq j hj).symm)).trans h_sum_T
    have hT0 : (T.card : ℝ) ≠ 0 := by
...
private theorem phi_eq_shapley_weighted (φ : Solution N)
    ...
    (c : ℝ) (T : Finset N) (hT : T.Nonempty) (i : N) :
    φ (Solution.SmulGame c (TUGame_en.unanimityGame T hT)) i =
    shapleyValue (Solution.SmulGame c (TUGame_en.unanimityGame T hT)) i := by
  rw [phi_weightedUnanimity φ h_eff h_sym h_null c T hT i,
      ShapleyValue.shapley_smulGame c (TUGame_en.unanimityGame T hT) i,
...
private theorem phi_finsetSumGames (φ : Solution N)
    ...
    {ι : Type*} [DecidableEq ι] (s : Finset ι) (f : ι → TUGame_en N) (i : N) :
...
private theorem shapley_finsetSumGames {ι : Type*} [DecidableEq ι]
    (s : Finset ι) (f : ι → TUGame_en N) (i : N) :
...
private theorem game_eq_mobius_sum (G : TUGame_en N) :
  ...
  have h_term (T : Finset N) (hT : T ∈ Finset.univ.filter Finset.Nonempty) :
      (if hT' : T.Nonempty then
        Solution.SmulGame (Mobius.mobiusCoeff G T) (TUGame_en.unanimityGame T hT')
      else (Solution.zeroGame : TUGame_en N)).v S =
...
theorem shapley_uniqueness (φ : Solution N)
    ...
    (h_null : φ.NullPlayerAxiom)
    (h_add : φ.Additivity) :
    ∀ G : TUGame_en N, ∀ i : N, φ G i = shapleyValue G i := by
  intro G i
  classical
  let idx : Finset (Finset N) := Finset.univ.filter Finset.Nonempty
  let g : Finset N → TUGame_en N := fun T =>
    if hT : T.Nonempty then
      Solution.SmulGame (Mobius.mobiusCoeff G T) (TUGame_en.unanimityGame T hT)
...
noncomputable def WeightedVotingGame (weights : N → ℝ) (quota : ℝ) (hquota : 0 < quota) : TUGame_en N where
...
theorem eq_zero_of_ne_one {G : TUGame_en N} (hG : SimpleGame G) (S : Finset N)
    (hne : G.v S ≠ 1) : G.v S = 0 :=
...
theorem complement_wins {G : TUGame_en N} (hG : StrongGame G) {S : Finset N}
...
theorem le_of_subset {G : TUGame_en N} (hG : MonotoneGame G) {S T : Finset N}
    (h : S ⊆ T) : G.v S ≤ G.v T :=
  hG h
...
theorem complement_loses {G : TUGame_en N} (hG : ProperGame G) {S : Finset N}
...
theorem proper (G : TUGame_en N) (hG : SelfDualGame G) : ProperGame G := hG.1
...
def VetoPlayer (G : TUGame_en N) (i : N) : Prop :=
...
def DummyPlayer (G : TUGame_en N) (i : N) : Prop :=
