/-
  ...
  Ce fichier formalise l'approche axiomatique de Shapley pour le partage équitable :
  - Les quatre axiomes de Shapley (Efficience, Symétrie, Joueur nul, Additivité)
  ...
  le code Lean tactique (noms de symboles, références Mathlib, énoncés de lemmes,
  preuves) reste en anglais pour préserver la compatibilité Mathlib.
-/
...
/-- Deux joueurs sont symétriques dans G s'ils apportent des contributions égales -/
...
/-- La valeur de Shapley : contribution marginale moyenne sur tous les ordres -/
...
/-! ## Caractérisation par les jeux d'unanimité (toute solution axiomatique) -/
...
/-- La valeur de Shapley vérifie l'axiome du joueur nul -/
...
/-- Multiplication scalaire d'un jeu TU par un nombre réel -/
...
/-- φ du jeu nul vaut 0 (chaque joueur est nul) -/
...
/-- Lemme : pour R = S, la somme interne vaut 1 (le singleton {S} contribue (-1)^0 = 1). -/
...
/-- φ coïncide avec shapleyValue sur les jeux d'unanimité -/
...
private theorem phi_weightedUnanimity (φ : Solution N)
  ...
  · -- Efficience : Σ_j φ(H, j) = H.v(univ) = c
    ...
    -- La somme sur le complément de T vaut 0
    ...
    have h_sum_T : ∑ j ∈ T, φ (Solution.SmulGame c (TUGame.unanimityGame T hT)) j = c := by
      ...
      linarith
    -- Tous les joueurs de T obtiennent la même valeur
    have h_eq : ∀ j ∈ T,
...
/-- Toute φ axiomatique coïncide avec shapleyValue sur SmulGame c u_T -/
...
/-- Un jeu de vote pondéré [q; w₁, ..., wₙ] avec quota positif -/
...
/-- Dans un jeu simple, une valeur différente de `1` doit être `0`. Lecture symétrique. -/
...
/-- Dans un jeu fort, le complément d'une coalition perdante est gagnant. -/
...
/-- Dans un jeu monotone, une coalition plus grande a une valeur au moins aussi grande. -/
...
/-- Dans un jeu propre, le complément d'une coalition gagnante est perdant. -/
...
/-- Le joueur i est critique dans la coalition S si son retrait fait perdre S -/
...
/-- Indice brut de Banzhaf : nombre de coalitions où i est critique. -/
...
/-- Joueur avec pouvoir de veto -/
def VetoPlayer (G : TUGame N) (i : N) : Prop :=
  ∀ S : Finset N, G.v S = 1 → i ∈ S

/-- Dictateur : peut gagner seul et possède le veto -/
...
theorem dictator_unique (G : TUGame N) (i j : N) (hi : Dictator G i) (hj : Dictator G j) :
  ...
  -- (`v {i} = 1`, première conjonction), donc la propriété de veto force `j ∈ {i}`, i.e. `j = i`.
...
theorem critical_of_minimal_winning {G : TUGame N} {S : Finset N}
  ...
  -- `S.erase i` est un sous-ensemble propre de `S` puisque `i ∈ S`, donc la minimalité le rend perdant.
...
theorem veto_shapley_pos (G : TUGame N) (hG : SimpleGame G) (i : N)
  ...
  -- La grande coalition privée de i est un ensemble précédent de i dont la contribution
  -- marginale vaut `1`.
  ...
  -- Chaque terme de la somme de Shapley est non négatif : `v S = 0` (veto, `i ∉ S`),
  ...
  -- Le terme indexé par `T` est strictement positif : marginale `= 1`, coefficient `> 0`.
  ...
  -- La somme sur tout le filtre est au moins ce terme positif unique indexé par `T`.
  have hle :=
...
/-- Joueur muet : n'ajoute aucune valeur. -/
...
theorem dummy_banzhaf_raw_zero (G : TUGame N) (i : N) (h : DummyPlayer G i) :
  ...
  -- et `v (S.erase i) = 0`, mais `DummyPlayer` donne `v S = v (S.erase i)`.
  have hneq : ∀ S, Critical G i S → False := by
    ...
    rw [hdummy] at hone
    linarith
  -- `BanzhafRaw` = cardinalité du filtre des coalitions critiques (instance désormais
  -- cohérente via `criticalDecidable`) ; le filtre est vide car `hneq` réfute toute criticalité.
  simp only [BanzhafRaw]
...
theorem veto_not_dummy (G : TUGame N) (hG : SimpleGame G) (i : N) (hv : VetoPlayer G i)
  ...
  intro hd
  -- Un muet ne change jamais la valeur d'une coalition ; appliqué à `univ.erase i`
  -- (qui exclut `i`) :
  have hni : i ∉ Finset.univ.erase i := by simp [Finset.mem_erase]
  have hdummy := hd (Finset.univ.erase i) hni
  -- `univ.erase i ∪ {i} = univ` (l'absence puis l'addition du joueur restaurent l'univers).
  have huniv : Finset.univ.erase i ∪ ({i} : Finset N) = Finset.univ := by
    ext j; simp [Finset.mem_univ]
  rw [huniv] at hdummy
  -- Un joueur veto force `v (univ.erase i) = 0` (il est absent de cette coalition).
  rw [veto_losing_without hG hv hni] at hdummy
  -- `hdummy : v univ = 0` contredit `hwin : v univ = 1`.
  rw [hdummy] at hwin
  linarith
...
theorem dictator_not_dummy (G : TUGame N) (i : N) (h : Dictator G i) :
    ¬ DummyPlayer G i := by
  intro hd
  -- L'hypothèse de muet à la coalition vide donne `v {i} = v ∅ = 0`.
...
theorem banzhaf_raw_symmetric (G : TUGame N) (i j : N)
  ...
  -- Comportement de `banzhafSwap i j` dans chacun des quatre cas d'appartenance.
  ...
  -- (2) Invariance de la valeur : `G.v (banzhafSwap i j S) = G.v S`.
  ...
  -- `hkey` : quand i, j sont tous deux dans S, la symétrie sur S \ {i,j} identifie les
  -- deux valeurs effacées.
  have hkey : ∀ S : Finset N, i ∈ S → j ∈ S →
      G.v (S.erase j) = G.v (S.erase i) := by
    intro S hSi hSj
    -- Effacer i puis j équivaut à effacer j puis i.
  ...
  -- (3) « Critique pour i » ↔ « critique pour j (après l'échange) ».
  have hiff : ∀ S : Finset N, Critical G i S ↔ Critical G j (banzhafSwap i j S) := by
    ...
    · -- i dehors, j dedans : σ S = (S.erase j) ∪ {i}. Les deux côtés sont faux.
      have hσ : banzhafSwap i j S = (S.erase j) ∪ {i} := swap_j_only hSi hSj
      rw [hσ]
      -- j ∉ (S.erase j) ∪ {i} (j a été effacé, j ≠ i), donc `Critical G j (σ S)` est faux ;
      -- `Critical G i S` est aussi faux puisque i ∉ S.
  ...
  -- (4) L'échange est une involution donc injectif ; le filtre j-critique est exactement
  -- l'image du filtre i-critique par l'échange, donc les deux cardinaux coïncident.
...
theorem dictator_banzhaf_index_eq_one (G : TUGame N) (i : N)
  ...
  have hn : 0 < Fintype.card N := Fintype.card_pos_iff.mpr ⟨i⟩
  have hdenom : 0 < (2 : ℝ) ^ (Fintype.card N - 1) := pow_pos (by norm_num) _
  -- Pour un dictateur, i est critique dans S ⟺ i ∈ S.
  ...
  -- D'où le filtre critique égale le filtre d'appartenance.
  have hfilter_eq :
      (Finset.univ.filter fun S => Critical G i S) =
  ...
  -- Le filtre d'appartenance a pour cardinal 2^(|N|-1) (bijection insertion/effacement).
  have hmemcard :
      ((Finset.univ : Finset (Finset N)).filter (fun S => i ∈ S)).card =
        2 ^ (Fintype.card N - 1) := by
    have heq :
