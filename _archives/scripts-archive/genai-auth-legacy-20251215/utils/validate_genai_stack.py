...
Usage:
    python scripts/genai-auth/utils/validate_genai_stack.py [--output report.json]

Auteur: Roo Code Mode (Phase 34) - Consolidé Phase 36
Date: 2025-12-12
"""
            ...
            if success:
                ...
                if images_output:
                    ...
                    if img_resp.status_code == 200:
                        ...
                        is_valid, msg, stats = analyze_image_content(str(out_path))
                        if is_valid:
                            logger.info(f"✅ Qualité Image OK: {msg}")
                            self.results["generations"][model_name] = {
                                "status": "SUCCESS",
                                "duration": duration,
                                "image": str(out_path),
                                "quality": "PASS",
                            ...
                            logger.error(f"❌ Qualité Image ÉCHOUÉE: {msg}")
                            self.results["generations"][model_name] = {
                                "status": "QUALITY_FAIL",
                                "duration": duration,
                                "image": str(out_path),
                            ...
                            return False # Échec du test même si généré
                    else:
