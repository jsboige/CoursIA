...
La progression suit cette logique : formaliser le raisonnement (Tweety), représenter la connaissance (Semantic Web), la prouver mécaniquement (Lean), l'appliquer à des problèmes concrets (Planners, SMT, Smart Contracts), puis la ponter vers le neuronal (Argument Analysis, Symbolic Learning). Chaque sous-série est autonome, mais ensemble elles dessinent une vision cohérente de l'IA symbolique moderne.
...
**[SMT](MyIA.AI.Notebooks/SymbolicAI/SMT/README.md) -- décrire les contraintes, laisser le solveur décider** -- Au lieu d'écrire l'algorithme de résolution, on déclare les contraintes et on laisse le solveur SMT (*Satisfiability Modulo Theories*) trouver la solution ou prouver l'impossibilité. Z3, le solveur de référence (Microsoft Research), raisonne directement sur des théories -- arithmétique linéaire, tableaux, vecteurs de bits, chaînes -- plutôt que sur des booléens nus, ce qui rend l'encodage lisible et naturel. La sous-série l'aborde sous deux angles complémentaires : une approche C# déclarative bornée via Z3.Linq (LINQ-to-Z3, requêtes bornées par les types), et une approche Python impérative complète via z3-py, complétées par les automates symboliques à prédicats. On y retrouve le Sudoku et le planificateur de repas -- des problèmes déjà croisés en Search et Sudoku -- désormais exprimés dans le langage de leur théorie naturelle. Le pont avec le reste de la série est direct : la vérification formelle d'un programme par solveur SMT est la même opération que la preuve d'un théorème en Lean.
...
CoursIA/
  MyIA.AI.Notebooks/          Notebooks interactifs, organisés par série
    ...
    SymbolicAI/                IA symbolique (Python, Lean 4, C#)
      Tweety/ SemanticWeb/ Lean/ Planners/ SMT/ SmartContracts/ Argument_Analysis/ SymbolicLearning/
  ...
  tools/                       Outils dédiés (semantic-fleet-radix)
  slides/                      Supports de cours (Marp)
  docker-configurations/       Infrastructure Docker GPU
  docs/                        Documentation projet (règles, procédures, guides)
  translations/                Traductions générées (EPIC multilingue)
