...
CoursIA est une collection de notebooks Jupyter interactifs couvrant l'ensemble du spectre de l'IA : algorithmes de recherche, résolution par contraintes, raisonnement formel et preuve mécanisée, théorie des jeux, programmation probabiliste et théorie de la décision, machine learning, apprentissage par renforcement jusqu'au post-training des LLMs, IA générative multimodale et trading algorithmique -- jusqu'à une série de recherche vivante qui mesure l'émergence causale dans les représentations internes d'un modèle.
...
Au 17 août 2026, le dépôt versionne **1 024 notebooks** sous `MyIA.AI.Notebooks/`, **22 lakes Lean 4** compilés contre Mathlib et un corpus de traduction de **33 CSV / 24 470 cellules** prêt pour un rendu multilingue. Le [catalogue régénéré chaque jour](COURSE_CATALOG.generated.md) fait foi sur le détail par série.
...
- **Preuves formelles** : au-delà des exercices, les séries Lean et GameTheory mécanisent des théorèmes phares en Lean 4 -- théorème de sensibilité, théorème de Kochen-Specker, théorème du libre arbitre de Conway-Kochen, formalisations des théorèmes d'Arrow et de Sen -- avec des hommages aux mathématiques de Grothendieck et de Conway. L'ensemble forme une couche de **22 lakes Lean 4 / Mathlib**, avec un théorème-phare par famille branché sur les notebooks qui l'enseignent. La dette formelle y est *mesurée*, pas déclarée : **18 `sorry` de code** subsistent sur l'ensemble des lakes, dont 14 concentrés sur le seul chantier `knot_lean` -- le reste des familles est démontré ou à un `sorry` près.
- **Une série de recherche vivante** : le banc ICT (extension de la théorie de l'information intégrée) mesure des gains d'émergence causale, de surprise et de compression sur des substrats de complexité croissante -- algorithmes de tri, réaction-diffusion (Gray-Scott), morphodynamique stratégique, features SAE d'un vrai LLM, le Jeu de la Vie (seul substrat au moteur de calcul prouvé en Lean, pont `hashlife_correct`, #5726), puis le discours lui-même comme substrat. Cinquante-cinq notebooks, une trentaine ajoutés au seul mois d'août : c'est le laboratoire du dépôt, avec ce que cela suppose de résultats négatifs assumés.
...
**[Recherche heuristique avancée](MyIA.AI.Notebooks/Search/Part3-Advanced/README.md)** -- La montée en gamme sur la recherche informée : bases de données de motifs (pattern databases, factorisation de l'espace d'états pour A*), recherche à divergence limitée (Limited Discrepancy Search, où l'on accepte quelques écarts à l'heuristique pour débloquer un chemin bloqué), A* pondéré (Weighted A*, compromis temps/optimalité réglable par un facteur ε). Chaque algorithme vient en jumeau Python/C# et se mesure sur des instances du taquin et du Rubik's cube où l'heuristique admissible standard atteint ses limites -- un pont direct vers l'analyse de performance de la Partie 4.

**[Métaheuristiques composables](MyIA.AI.Notebooks/Search/Part4-Metaheuristics/README.md)** -- Un side-track .NET 9 (GeneticSharp) qui **reconstruit et compose** les métaheuristiques plutôt que d'importer une boîte noire : MGS-1 à 7 bâtissent le moteur et la grammaire de composition (jusqu'au TSP), MGS-7b/7c/7d projettent les paysages en dimension N≥5, MGS-8 à 14 visualisent les paysages et mesurent la robustesse aux biais (décalage, rotation, synergie d'îles), MGS-15 à 18 referment sur l'analyse quantitative (No-Free-Lunch, contrôle de paramètres), MGS-19 démonte le recuit simulé pour éprouver l'opérateur de Metropolis seul. Prolonge Search-5 (algorithmes génétiques) et Search-11 (métaheuristiques).
...
Le plus mûr des deux est [2025-Epita-Intelligence-Symbolique](https://github.com/jsboigeEpita/2025-Epita-Intelligence-Symbolique), le *moteur* : plus de **quarante agents spécialistes** -- chacun tenant un axe d'analyse, un formalisme ou une étape de vérification -- répartis sur **plusieurs systèmes d'orchestration** concurrents, adossés à une suite d'intégration de plus de quatorze mille tests. La difficulté propre à ce projet n'est pas d'ajouter un agent de plus : c'est de **consolider en vol** cette diversité d'orchestrateurs sans perdre ce que chacun sait faire -- un problème d'architecture agentique à part entière, et le chantier le plus avancé du dépôt sur l'argumentation. C'est de là que vient `argumentation_lib`.
...
**[Théorie de la décision](MyIA.AI.Notebooks/Probas/DecisionTheory/README.md)** -- Une troisième sous-série porte l'arc décision *en miroir* des deux écosystèmes : 10 notebooks C# dans [`DecisionTheory/DecInfer/`](MyIA.AI.Notebooks/Probas/DecisionTheory/DecInfer/README.md) doublés de 7 notebooks PyMC dans [`DecisionTheory/PyMC/`](MyIA.AI.Notebooks/Probas/DecisionTheory/PyMC/README.md), plus un lake compagnon Lean 4 ([`decision_theory_lean`](MyIA.AI.Notebooks/Probas/decision_theory_lean/)) où les identités d'escompte géométrique de l'indice de Gittins sont entièrement prouvées (`Discount.lean` 0 sorry, jalon `GittinsTheorem` à 2 sorry sur le théorème d'optimalité faute de formalisation MDP/Bellman dans Mathlib), et un [notebook-pont do-calculus](MyIA.AI.Notebooks/Probas/DecisionTheory/Causal-Bridges/Do-Calculus-Bridge.ipynb) qui fédère les traitements de la causalité (Tweety, Infer.NET, PyMC, PyPhi) autour de [`dowhy`](https://www.pywhy.org/dowhy/).
...
**Simuler, puis prouver -- la dualité Python/Lean** -- À côté du fil principal court un fil de formalisation : les grands théorèmes ne sont pas seulement illustrés, ils sont démontrés dans l'assistant de preuve Lean 4. L'existence d'un équilibre de Nash y est établie via les points fixes de Brouwer et Kakutani ; les jeux combinatoires reposent sur la structure PGame de Mathlib ; la valeur de Shapley est reconstruite à partir de ses axiomes. Ce travail est porté par cinq projets Lake, dont un lake principal qui a absorbé les modules autrefois épars -- et la frontière du certifié y a nettement reculé. La valeur de Shapley et le théorème d'Arrow étaient déjà démontrés sans aucun `sorry` ; s'y ajoutent désormais la **condition de Bondareva-Shapley**, longtemps laissée ouverte et obtenue par la route de Farkas -- séparation par hyperplan, cône conique fermé, décodage du témoin -- et la **structure de treillis des mariages stables** de Knuth, où l'on prouve que le sup et l'inf de deux mariages stables le restent, et que l'exécution de Gale-Shapley côté proposants en occupe le sommet. Il subsiste exactement **un** `sorry` sur tout le lake : la direction difficile du théorème *Folk* escompté, assumée et documentée comme telle plutôt que masquée. Cette frontière explicite entre ce qui est certifié et ce qui ne l'est pas encore est exactement le fil rouge de vérification formelle partagé avec la série Lean de SymbolicAI.
...
**[FineTuning](MyIA.AI.Notebooks/GenAI/FineTuning/README.md) -- adapter sans tout ré-entraîner** -- La boîte à outils pratique de la spécialisation : LoRA et QLoRA pour n'entraîner qu'une poignée de paramètres, SFT pour apprendre un format de réponse, DPO pour aligner sur des préférences, et le *model merging* pour combiner plusieurs spécialisations en un seul modèle. Cinq notebooks, orientés « ce qui marche sur le GPU dont vous disposez ».

**[PostTraining](MyIA.AI.Notebooks/GenAI/PostTraining/README.md) -- la chaîne complète, du SFT au RL à récompense vérifiable** -- C'est ici que se fabrique concrètement un assistant, et c'est l'une des séries les plus denses du dépôt. Elle remonte toute la chaîne -- SFT, puis RLHF, puis DPO, puis GRPO, puis RLVR -- en expliquant à chaque étape *la formule du loss avant le code*, parce que le point pédagogique n'est pas qu'un `trainer.train()` converge, mais qu'on sache pourquoi DPO se passe d'un reward model et pourquoi GRPO se passe d'un critique. Trois notebooks réimplémentent la famille « sans critique » **depuis zéro** sur un environnement jouet CPU -- GRPO, RLOO et GAE côte à côte, où l'on voit l'arbitrage biais-variance à l'œuvre et où le « collapse » observé à un pas se révèle une propriété du banc d'essai plutôt que de l'algorithme, une fois le crédit rendu véritablement différé. Deux autres sortent du jouet et entraînent un **vrai modèle** (Qwen3.5-0.8B en QLoRA 4-bit, sur un GPU de 8 Go) contre des récompenses *vérifiées par un solveur* -- Z3 sur des contraintes arithmétiques, SymPy et un N-reines -- avec un détecteur de *reward hacking* branché en ligne pendant l'entraînement, et une validation multi-seed. La discipline d'évaluation du dépôt s'y applique à la lettre : un gain se déclare sur plusieurs graines ou ne se déclare pas.

**[Plateformes conversationnelles](MyIA.AI.Notebooks/GenAI/Plateformes-Conversationnelles/README.md) -- quand la GenAI devient un produit que quelqu'un utilise** -- Un modèle qui répond n'est pas encore un service : il lui faut une interface, des comptes, une mémoire, des outils, et quelqu'un pour en répondre. Cette sous-série -- une vingtaine de notebooks, l'un des chantiers les plus actifs du dépôt -- est délibérément nommée par sa **fonction** et non par un produit, parce qu'elle en présente **deux, en sœurs**, et que le choix entre elles est justement l'exercice.

*Open WebUI*, d'abord : la plateforme auto-hébergée, avec un tour guidé fonctionnalité par fonctionnalité, puis une série d'**assurance qualité automatisée** (Playwright) qui écrit de vrais tests de bout en bout -- authentification, streaming de réponses, RAG, outils MCP, multi-tenant et intégration continue. *AI-Engine*, ensuite : l'extension GenAI d'un site **WordPress** existant, terrain radicalement différent où l'IA doit s'insérer dans un CMS déjà en production, avec son contenu, ses formulaires et ses visiteurs. Quatorze notebooks y explorent la plateforme **par son API**, sous la forme d'une veine systématique et toujours en cours : un grain par fonctionnalité, chacun démontré par un notebook réellement exécuté contre une **instance jetable** -- une installation WordPress éphémère, montée puis détruite, ce qui rend la série reproductible par un étudiant sans rien casser. Le fil déplie les faces successives de l'API : le socle et la présentation, la configuration des chatbots, les formulaires, l'exposition d'un serveur MCP, les environnements multi-fournisseurs, et -- grain en cours -- la face du **visiteur anonyme**, avec son espace de nommage distinct et son amorçage de session. Autour de cette colonne, les notebooks abordent les questions que seul un déploiement réel pose : ingérer un corpus long en RAG, **séparer les environnements de vecteurs** pour ne pas mélanger recette et production, mesurer la **dérive d'un copilot** dans le temps, auditer un formulaire conditionnel ou la conformité visuelle d'un rendu. Le MCP y est traité dans les deux sens -- *consommer* un serveur d'outils, mais aussi *exposer* WordPress lui-même comme serveur, jusqu'à le piloter par ce canal -- et un notebook d'audit vient vérifier ce qu'un serveur MCP annonce réellement. Un comparatif au niveau de la catégorie, et un notebook qui apprend à *différencier les assistants*, referment le choix : non pas quelle plateforme est la meilleure, mais laquelle convient à quel terrain.

Côté **mémoire des agents**, la sous-série [RAG et Mémoire Sémantique](MyIA.AI.Notebooks/GenAI/RAG-et-Memoire-Semantique/README.md) documente le backend Qdrant qui ancre les agents de codage dans des faits vérifiables (historique des conversations d'agents, code des dépôts) — la couche sous les front-ends Claude Code / Roo Code / Claw-Systems.

**[Aspire](MyIA.AI.Notebooks/GenAI/Aspire/README.md) -- la pile IA inattendue : C#/.NET** -- Une sous-série à contre-courant, qui prend au sérieux une question rarement posée : et si l'orchestration d'une pile GenAI se faisait en .NET plutôt qu'en Python ? .NET Aspire décrit l'application entière -- modèles servis, bases vectorielles, services annexes, variables d'environnement, santé et télémétrie -- dans un **AppHost** compilé, typé, versionné, au lieu d'un assemblage de fichiers Compose et de scripts shell. Deux notebooks .NET Interactive, tous deux branchés sur l'infrastructure GPU réelle du dépôt plutôt que sur une maquette. Le premier orchestre un service de transcription conteneurisé et démontre l'isolation de ports : deux instances de la même pile tournent simultanément, ports randomisés et conteneurs suffixés, là où l'équivalent en Compose se règle à la main. Le second modélise la pile **multi-machines** dans un seul AppHost -- ComfyUI et le serveur vLLM y sont *référencés* comme services externes, jamais recréés -- et fait un appel traversant authentifié à chacun. L'intérêt pédagogique dépasse le langage : voir la même architecture décrite deux fois, une fois en scripts et une fois en code typé, fait apparaître ce que chaque approche rend facile -- et ce qu'elle laisse tacite.

**[Vibe-Coding](MyIA.AI.Notebooks/GenAI/Vibe-Coding/README.md)** -- Le développement assisté par agents IA, devenu compétence centrale : ateliers progressifs sur Claude Code et Roo Code, de la découverte à l'automatisation avancée (skills, sous-agents, MCP, hooks), plus deux modules d'infrastructure : les agents autonomes conteneurisés (Claw-Systems — les bots reviewers qui auditent réellement ce dépôt) et le proxy multi-provider Claudish. Deux chantiers récents y posent la question qui suit naturellement — *si un agent écrit le code, qui le relit ?* Le premier est un **analyseur Roslyn de garde-fous** (`MyIA.AgentSafetyAnalyzer`) : une série de règles de sécurité compilées dans le build lui-même, jusqu'au secret codé en dur détecté à la frappe et corrigé par un *code fix*, avec son registre de suppressions et ses tests — un filet qui s'applique au code produit par la machine sans dépendre de la vigilance de qui relit. Le second branche un **REPL C# sur un processus .NET vivant** pour le corriger à chaud, sans l'arrêter. Les deux relèvent d'un même fil, celui d'une pile IA en C#/.NET là où on n'attend que du Python — le même fil que la sous-série Aspire plus haut.
...
**Ce que le manuel escamote -- six impasses classiques, traitées de front** -- La série ne s'arrête pas au socle. Six notebooks prennent chacun un endroit précis où le RL de tutoriel se casse la figure. Le **model-based** (Dyna-Q) apprend un modèle du monde pour planifier au lieu de ne faire qu'agir, et divise d'autant le nombre d'interactions nécessaires. Le **RL offline** affronte l'erreur d'extrapolation -- apprendre d'un jeu de données figé, sans jamais pouvoir tester ses idées, exactement la situation d'un système entraîné sur des logs. Le **reward shaping** et le curriculum montrent comment une récompense mal formée enseigne autre chose que ce qu'on croyait demander. Le **POMDP** retire à l'agent l'accès à l'état vrai et le force à entretenir une croyance. Le **distributional RL** (C51 depuis zéro) remplace l'espérance de retour par sa *distribution* -- deux stratégies de même moyenne n'ont pas le même risque. L'**exploration par curiosité** (Random Network Distillation) fabrique une récompense intrinsèque là où l'environnement n'en donne aucune.

**Post-training des LLMs -- le RL là où il compte aujourd'hui** -- Un dernier arc, `rlpt_*`, branche tout ce qui précède sur l'alignement des modèles de langage, qui est devenu le principal débouché industriel du renforcement. On y aligne un petit modèle par **PPO façon RLHF**, puis par **GRPO** sur une récompense *vérifiable* (la variante sans critique popularisée par DeepSeek-R1) ; on y dissèque une anatomie de **reward hacking** -- un hack qui, ici, ne prend pas, et l'on regarde pourquoi ; on y oppose enfin **DPO hors ligne et RL en ligne** sur la même tâche à budget égal. Cet arc est délibérément *petit* -- from scratch, au niveau du caractère, sur CPU ou sur un budget de quelques dizaines de pas : il sert à voir la mécanique, pas à produire un assistant. C'est le pont ; il **atterrit** dans la série [GenAI / PostTraining](MyIA.AI.Notebooks/GenAI/PostTraining/README.md), qui reprend les mêmes techniques sur de vrais modèles, avec des récompenses vérifiées par solveur et des verdicts multi-seed. Lire les deux dans cet ordre, c'est passer de « comment ça marche » à « ce que ça coûte de le faire marcher pour de bon ».
...
La confrontation empirique des deux grandes théories rivales est en place : détecter dans le même substrat les signatures du Global Workspace (broadcast, ignition) et vérifier si elles co-localisent avec les pics de complexité intégrée. Un résultat négatif -- une dissociation -- y vaut autant qu'une convergence, et la série en publie plusieurs : c'est un laboratoire, pas une démonstration.
...
CoursIA/
  ...
  translations/                Corpus de traduction des notebooks (33 CSV, 24 470 cellules)
