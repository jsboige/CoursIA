...
**Post-training des LLMs -- le RL là où il compte aujourd'hui** -- Un dernier arc, `rlpt_*`, branche tout ce qui précède sur l'alignement des modèles de langage, qui est devenu le principal débouché industriel du renforcement. On y aligne un petit modèle par **PPO façon RLHF**, puis par **GRPO** sur une récompense *vérifiable* (la variante sans critique popularisée par DeepSeek-R1) ; on y dissèque une anatomie de **reward hacking** -- un hack qui, ici, ne prend pas, et l'on regarde pourquoi ; on y oppose enfin **DPO hors ligne et RL en ligne** sur la même tâche à budget égal. C'est la jonction la plus concrète du dépôt entre l'algorithmique du renforcement et la fabrication réelle des assistants.
