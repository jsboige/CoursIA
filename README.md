...
**[Théorie de la décision](MyIA.AI.Notebooks/Probas/DecisionTheory/README.md)** -- Une troisième sous-série porte l'arc décision en miroir d'Infer.NET et de PyMC, prolongé par le lake Lean 4 [`decision_theory_lean`](MyIA.AI.Notebooks/Probas/decision_theory_lean/) et par deux notebooks-ponts de causalité : do-calculus quand l'intervention est identifiable, méthodes quasi-expérimentales quand `do()` est impossible. Le versant actuariel traite crédibilité, prime pure et chargement, ruine de Lundberg, valeur de l'information en souscription et fréquence-sévérité hiérarchique. Un banc cross-engine soumet en outre le même contrat JSON à Infer.NET et PyMC, pour comparer des décisions plutôt que juxtaposer des syntaxes. Le lake prouve les identités d'escompte de Gittins et la cohérence tarifaire : un barème non additif expose un Dutch Book exploitable ; la frontière restante du théorème d'optimalité est documentée plutôt que masquée.
...
Deux portes d'entrée sont proposées :

- **[Ouvrir une issue](https://github.com/jsboige/CoursIA/issues/new?template=content-feedback.yml)** pour signaler une erreur ou une coquille, suggérer une amélioration, ou proposer une nouvelle source ou un nouveau contenu à distiller. Une Pull Request associée est facultative et bienvenue.
- **Soumettre une correction d'exercice par Pull Request** : après validation, la solution devient un **exemple guidé crédité** et un nouvel exercice non résolu de difficulté comparable prend sa place.

Le premier triage et certaines réponses peuvent être assurés par les agents qui maintiennent le dépôt. Ils répondent avec bienveillance, indiquent ce qu'ils ont vérifié et escaladent vers un humain ou un spécialiste lorsque nécessaire.

Le parcours détaillé, les conventions et le protocole de crédit sont décrits dans le [guide de contribution](CONTRIBUTING.md).
