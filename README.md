...
**[Recherche heuristique avancée](MyIA.AI.Notebooks/Search/Part3-Advanced/README.md)** -- Que faire quand l'heuristique admissible reste trop faible pour résoudre à l'optimum un problème vraiment difficile ? Cette troisième partie repousse le plafond avec les **bases de données de motifs** (Pattern Databases) : l'heuristique n'est plus calculée mais précalculée une fois pour toutes par retournement du problème, puis stockée dans une table consultée en temps constant. Le fil rouge est le **15-puzzle** (taquin 4×4), banc d'essai historique depuis Korf (1985) -- on y voit que l'heuristique de Manhattan, bien qu'admissible, explore des centaines de milliards de nœuds sur les instances difficiles, et que les Pattern Databases additives de Korf & Felner (2002) y gagnent plusieurs ordres de grandeur sans violer l'admissibilité. La leçon dépasse le cas : pour la recherche à l'optimum, une heuristique *informée* (proche du vrai coût) vaut mieux qu'une heuristique *admissible mais myope*, et la mémoire devient une ressource algorithmique à doser.

**[Métaheuristiques composables](MyIA.AI.Notebooks/Search/Part4-Metaheuristics/README.md)** -- Un side-track C# .NET 9.0 autour de [MetaGeneticSharp](https://github.com/jsboige/MetaGeneticSharp), une bibliothèque qui reconstruit les métaheuristiques publiées (Whale Optimization, Equilibrium Optimizer, Differential Evolution, Bare-Bones PSO, modèle insulaire) à partir de **primitives réutilisables** plutôt que comme des monolithes opaques. Vingt-deux notebooks .NET Interactive en sont le fil conducteur : un algorithme doit pouvoir s'énoncer en quelques lignes déclaratives, chaque brique (sélection, croisement, mutation, réinsertion) étant interceptable et recomposable. L'arc culmine sur le **No-Free-Lunch** -- mesurer le paysage de fitness, sélectionner l'algorithme, contrôler le paramètre pendant la course, éprouver le tout sur le banc CEC shift+rotate -- avant un démontage qui isole un composant pour éprouver sa contribution. C'est le pont entre la série pédagogique Python (OR-Tools, DEAP, mealpy) et une vraie bibliothèque industrialisable.
