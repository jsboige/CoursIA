> **ARCHIVED 2026-07-22** — c.728y+46 docs-hygiene triage #7422 (`docs/archive/` racine, 1 fichier). Snapshot T22.12 du 2026-05-28 (issue #1628) = classification 4-types (a/b/c/d) + inventaire exhaustif `MyIA.AI.Notebooks/QuantConnect/` (~192 notebooks, 95 projets) consolidé cycle 91 depuis issue #29. **Verdict KEEP-AS-ARCHIVE (SUPERSEDED)** : **15+ inbound refs actifs** depuis `docs/qc/qc-strategies-status.md` (canonique vivant, 830 lignes, 137 KB, alimenté quotidiennement par catalog-cron + tr.1-23) **ET** depuis 4 stale cross-refs vers l'archive (à dispatcher c.728y+48 ref-quality, hors scope c.187) : `MyIA.AI.Notebooks/QuantConnect/ML-Training-Pipeline/README.md:67`+`README.en.md:16` (2×) + `Python/README.md:24`+`:28` (2×) — devraient pointer vers `docs/qc/qc-strategies-status.md` canonique. Snapshot préservé pour traçabilité historique (cycles 5, 91, dedup 50 projets audit #1627) ; SUPERSEDED-BY canonique = `docs/qc/qc-strategies-status.md` (single source of truth pour les classifications de stratégies QuantConnect). Source triage : issuecomment à venir (#7422).
