...
| Vérifié (tranche 17, backtests QC Cloud MCP) | 4 | cohorte **HandsOn / EMA / ML risque** (4 stratégies best-guess du bucket Vivant promues : `Positive-Negative-Splits-ML` HandsOn Ex07 stock-splits event-driven, `Stoploss-Volatility-ML` HandsOn Ex08 ML risk-management, `ML-SVM` SVM supervisé equity-only, `EMA-Cross-Alpha` Trend EMA classic) — **1 Edge** (`Positive-Negative-Splits-ML` Sharpe 1.511 / PSR 82.33 %, **2ᵉ PSR > 50 % cohorte depuis tr.3 BlackLitterman PSR 51 %**, après tr.16 `Framework_Composite_TrendWeather` PSR 77.94 %) + 3 Needs-improvement (PSR < 2 % partout) ; **2ᵉ tranche QC #1621 consécutive avec edge statistique significative** = lane pionnière consolidée ; event-driven fundamental + ML stoploss > trend pur sur Sharpe par unité de risque ET signification statistique |
...
| Vérifié (tranche 21, backtests QC Cloud MCP) | 5 | cohorte **ESGF-Kit ML éprouvée + 1630 robustesse post-#2801 + TrendFollowing clone** (5 stratégies hors-bucket Vivant, scan `list_projects name_contains:` patterns `dbg-`/`Algo`/`Pairs`/`Calendar`/`Leveraged`/`Sector`/`Regime`/`Wing`) — **0 BROKEN + 5 Needs-improvement** (`ESGF-Kit-XGBoost-Sectors` v11 Sharpe 0.543 CAGR 10.68 % MaxDD 27.6 % PSR 15.09 % max cohorte / `ESGF-Kit-RandomForest-Sectors` v10 Sharpe 0.278 MaxDD catastrophique 51.6 % / `1630-markov-regime-post2801` Sharpe 0.375 MaxDD 24.4 % / `1630-regime-switching-post2801` Sharpe **0.581 max cohorte** CAGR 12.29 % MaxDD 33 % PSR 7.04 % / `baseline-clone-TrendFollowing-repo-1630` clone post-#1630 Sharpe 0.36 **MaxDD 15.00 % record cohorte**) — **0 edge significative** (PSR max 15.09 % vs cap 50 %) ; **nuance L710-L1 ★★★** : **ESGF-Kit avec méthodologie ML validée (XGBoost/RandomForest sur Sectors) NE sont PAS structurellement BROKEN** comme les baselines QCP13 Hands-On (L710-L1 ★★★ reste vraie pour `QCxx-Cell*` mais **NE s'applique PAS aux ESGF-Kit**) ; les 5 stratégies c.712 = NI purs, edge statistical fragile mais cohérente avec méthodologie ML éprouvée ; **bonus L712-L1 ★ NEW** : `baseline-clone-TrendFollowing-repo-1630` projet 33078355 = clone récent (juillet 2026) issu du repo Trend-Following référence #1630 (QC Strategy Library), drawdown record 15 % confirme la qualité de la méthodologie trend-following AQR-style sous-jacente |
| Vérifié (tranche 22, backtests QC Cloud MCP) | 5 | cohorte **1630-ML-*-post2801 robustesse #2801 + Framework DualMomentumNoTLT + RiskParity aligned** (5 stratégies hors-bucket Vivant, scan `list_projects name_contains:` patterns `1630-`/`ETF`/`LongShort`/`Volatility`/`Adaptive`/`Risk`/`Index`/`Tail`/`Dynamic`/`Rotation`) — **0 BROKEN + 5 Needs-improvement** (`1630-ML-XGBoost-post2801` Sharpe 0.555 CAGR 14.51 % MaxDD 40.4 % PSR 10.66 % / `1630-ML-RandomForest-post2801` Sharpe **0.700 max cohorte** CAGR 20.59 % MaxDD 40.9 % **PSR 18.40 % max cohorte c.713** NP $753k / `1630-ML-FeatureEngineering-post2801` Sharpe 0.654 CAGR 18.74 % **MaxDD 28.10 % record cohorte c.713** PSR 15.14 % / `Framework-DualMomentumNoTLT` Sharpe 0.634 CAGR 16.91 % MaxDD 34.5 % PSR 14.27 % / `1630-RiskParity-aligned` Sharpe 0.361 CAGR 8.30 % **MaxDD 18.40 %** PSR 1.85 %) — **0 edge significative** (PSR max 18.40 % vs cap 50 %) ; **confirme L712-L2 ★** sur 3 stratégies 1630-ML-*-post2801 (RF/XGBoost/FeatureEng. = Sharpe honnête 0.55-0.70, PSR 10-18 %, **2ᵉ cohorte post2801 cohérente** après Markov/regime-switching c.712) ; **L713-L1 ★ NEW** : **1630-ML-RandomForest-post2801 NI prometteur** (PSR 18.40 % record cohorte c.713, > ESGF-XGBoost c.712 15.09 %) confirme robustesse méthodologique RF sur 1630 universe post-#2801 ; **L713-L2 ★ NEW** : **1630-ML-FeatureEngineering-post2801 MaxDD 28.10 % record** = feature engineering discipline = facteur discriminant du downside risk (vs XGBoost 40.4 % / RF 40.9 % sur même univers 1630) ; **L713-L3 ★ NEW** : **1630-RiskParity-aligned MaxDD 18.40 % mais PSR 1.85 %** confirme trend-following cross-sectionnel AQR-style (cf L712-L3 ★) — drawdown bas par construction mais edge statistical fragile sans levier |
| Vérifié (tranche 23, backtests QC Cloud MCP) | 5 | cohorte **Crypto EMA + Causal Event + Macro Factor Rotation + Equity Aroon clones freshly-discovered post-pivot R3 triple** (5 stratégies hors-bucket Vivant, scan `list_projects name_contains:` patterns `MeanReversion`/`Carry`/`Breakout`/`Pairs`/`StatArb`/`Sector`/`Momentum`/`VolTarget`/`Quality`/`Alpha`/`Macro`/`TermStructure`/`Spread`/`Forex`/`Crypto`/`Equity`) — **1 BROKEN empty + 4 Needs-improvement** (`Multi-Layer-EMA-Crypto` 30395392 ≠ 28433748 projet tr.4 **BROKEN empty** Sharpe/CAGR/NP tous 0 (projet dépublié/données perdues) / `CausalEventAlpha` post-#2801 IBKR margin Sharpe 0.447 **CONFIRMATION downgradé** historique 0.78→0.45 (#3067 MERGED) puis 0.447 freshly-discovered post-#2801 PSR 5.51 % MaxDD 38.8 % / `MacroFactorRotation-QC` post-#2801 Sharpe 0.731 CAGR 22.63 % MaxDD 42.0 % **PSR 23.81 % max cohorte c.714** NP $720,114 / `Equity Aroon Trend` clone 31932810 Sharpe 0.227 CAGR 6.41 % MaxDD 36.5 % PSR 0.81 % / `Equity Aroon Trend 1` clone 31996999 Sharpe 0.225 CAGR 6.35 % MaxDD 37.7 % PSR 0.79 %) — **0 edge significative** (PSR max 23.81 % vs cap 50 %) ; **L714-L1 ★ NEW** : stratégie `Multi-Layer-EMA-Crypto` (projetId 30395392) ≠ `Multi-Layer-EMA` (projetId 28433748 couvert tr.4) — **VÉRIFIER le projetId par `read_project name`, JAMAIS seulement par similarité de nom** (clone vs original, homonymie QC Cloud piège à collision R3) ; **L714-L2 ★ NEW** : `CausalEventAlpha` Sharpe 0.447 freshly-discovered post-#2801 **confirme downgradé historique #3067** (0.78→0.45) — robustesse méthodologique #2801 = post-leak fix effectif sur cohorte ex-edge ; **L714-L3 ★ NEW** : `MacroFactorRotation-QC` PSR 23.81 % = NI élevé (Sharpe 0.731 CAGR 22.63 % MaxDD 42.0 %) — **macro/factor = profils drawdown structurellement élevés** sans sizing/levier calibration (cohérent L712-L3 ★ + L713-L3 ★) ; **L714-L4 ★ NEW** : **Pivot R3 triple succès c.714** = 3 itérations scan/collision nécessaires pour finaliser 5 stratégies (15 candidats initiaux → 5 après 3 vagues collision-check) — **QC Cloud pool s'épuise** post-22 tranches (227 projets, ~80 % catalogués #1621 + historique) ; tr.24+ = **ROI marginal décroissant** = cadence réduite (1 tranche / 2 cycles) ou pivot méthodologique (audit cross-tranches, consolidation leçons, agent propre) |
| Vérifié (tranche 25, statut réel ML-* — création cloud + backtests frais baseline #1630) (4) | 4 | cohorte **ML classique jamais-vérifiée** (`ML-Classification`, `ML-Regression`, `ML-Ensemble`, `ML-DeepLearning` — 4 locaux du bucket Vivant **inexistants sur QC Cloud**, tranches 7/17 n'avaient couvert que FeatureEng./Reversion-Trending/SVM/Trend-Scanning/FX-Wavelet) : projets cloud créés + code aligné #1630 (2018-2025) poussé + compile 4/4 BuildSuccess + backtest frais chacun — **1 BROKEN code** (`ML-Classification` Runtime Error `joblib.loads` inexistant au premier Initialize, 0 ordre : le loader ObjectStore est cassé depuis l'origine) + **1 BROKEN marché** (`ML-Regression` Sharpe **−0.125** PSR 0.017 %, 9754 ordres) + **1 Needs-improvement** (`ML-Ensemble` Sharpe **0.724** — max ML classique vérifié — CAGR 18.8 % MaxDD 39.4 % PSR 14.4 %, 6659 ordres) + **1 BROKEN marché** (`ML-DeepLearning` Sharpe **−0.028** PSR 0.050 %, CAGR 3.2 %, 707 ordres : LSTM 1×32 multi-ETF ne bat pas le cash) ; **0 edge significative** (PSR max 14.4 %) ; **correction concomitante tranche 7** : les étiquettes « 2018–2025 » des 4 lignes ML de la tr.7 étaient erronées — le code historique portait `2015→2024` (jamais balayé par l'alignement #1630), corrigées en « 2015–2024 » ; **leçon L716-L1 ★** : le sweep #1630 de juin a raté les 6 ML-* locaux (globs `1630-`/Cloud uniquement) — une baseline « cross-stratégie » déclarée doit être vérifiée par grep sur `SetStartDate` par famille, pas supposée |
| Vérifié (tranche 26, backtests QC Cloud MCP) | 2 | achèvement du sweep ML-* (tr.25) : les 2 derniers ML locaux jamais backtestés sur QC Cloud (`ML-FeatureEngineering` RF+GB 18 features, `ML-Reversion-Trending` GB regime classification) — cloud créé + aligné #1630 (portaient encore 2015→2024, L716-L1 ★) + compiles 2/2 + backtests frais (1761 j.) ; correction des étiquettes de période tr.7 de ces 2 lignes (« 2018–2025 2830 j. » → 2015–2024 réel, suite tr.25 qui les avait déjà corrigées) ; **aucun changement de compte Vivant** (FE/RT déjà promus tr.7 — la tr.26 re-backteste sur baseline alignée) ; verdicts détaillés section tranche 26 |
| Vivant (best-guess, non vérifié) | 7 | algo `QCAlgorithm` complet, aucun signal négatif — TODO backtest pour confirmer (2 reclassés Archivé firsthand c.570) ; tranche 7 a promu 5 stratégies ML, tranche 8 a promu 5 stratégies Trend/Régime/Vol, tranche 14 a promu 2 stratégies Chronos DL, tranche 15 a promu 4 stratégies quantitatives structurelles, tranche 16 a promu 4 stratégies EMA + Composite Framework + Value, tranche 17 a promu 4 stratégies HandsOn/EMA/ML, tranche 18 a promu 4 stratégies freshly-discovered (risk + régime + robustness) hors-bucket Vivant, tranche 19 a promu 5 baselines Composite pédagogiques hors-bucket, tranche 25 a promu 4 stratégies ML classiques : voir sections ci-dessous. **Note comptable (sémantique du double-listing)** : la section détaillée contient **91 lignes physiques = 12 true-Vivant + 79 promues tr7-25** (cross-référencées dans leur bucket d'origine ET ici pour la discoverability, voir status `Vérifié tranche N`) — le compte `7` ci-contre = Vivant non-encore-vérifiées après déduction des promues des tranches précédentes, distinct du compte physique (intentionnel, pas une erreur). |
...
| Stratégie | Chemin | Type | Statut | Métriques backtest (période ; Sharpe ; CAGR ; MaxDD ; PSR ; Net Profit) |
|-----------|--------|------|--------|------------------------------------------------------------------------|
| `ML-FeatureEngineering` | `projects/ML-FeatureEngineering/` | ML features (RF/GB Ensemble) | **Needs-improvement** | 2015–2024 (2830 j. ; **période corrigée tr.25** : le `main.py` historique portait `SetStartDate(2015,1,1)`/`set_end_date(2024,12,31)` — jamais aligné #1630, l'étiquette « 2018–2025 » initiale était erronée) ; **Sharpe 0.656** (max cohorte) ; CAGR 19.06 % (max cohorte) ; MaxDD 34.8 % ; PSR 14.79 % ; NP 614.5 % ($614 544) — meilleur Sharpe/CAGR de la cohorte ML, mais PSR 14.8 % = edge non significative, drawdown 34.8 % élevé. Run frais aligné #1630 : voir **tranche 26** |
| `ML-Reversion-Trending` | `projects/ML-Reversion-Trending/` | ML classification reversion/trending | **Needs-improvement** | 2015–2024 (2830 j. ; **période corrigée tr.25**, cf ci-dessus) ; Sharpe 0.571 ; CAGR 10.51 % ; MaxDD 19.6 % (min cohorte) ; PSR 25.06 % ; NP 208.4 % ($205 971) — 2ᵉ PSR le plus élevé mais edge non significative ; version v1 avait Runtime Error, v1-fixed resolu. Run frais aligné #1630 : voir **tranche 26** |
...
| `ML-Trend-Scanning` | `projects/ML-Trend-Scanning/` | ML trend scanning | **Needs-improvement** | 2015–2024 (2878 j. ; **période corrigée tr.25**, cf ci-dessus) ; Sharpe 0.328 ; CAGR 7.085 % ; **MaxDD 29.4 %** ; PSR 7.84 % ; NP 119.0 % ($115 983) — PSR faible, edge nul ; catalog post-#2801 confirmait Sharpe 0.66 → 0.33 (cf PR #3083jsboige MERGED 2026-06-15) |
| `ML-FX-SVM-Wavelet` | `projects/ML-FX-SVM-Wavelet/` | ML FX (SVM/Wavelet) | **Needs-improvement / near-BROKEN** | 2015–2024 (2806 j. ; **période corrigée tr.25**, cf ci-dessus) ; Sharpe 0.153 (min cohorte) ; CAGR 4.29 % (min cohorte) ; MaxDD 21.2 % ; **PSR 0.39 % (min cohorte, quasi-nul)** ; NP 46.0 % ($455 947) — PSR < 1 %, edge statistiquement nul, retour faible |
...
#### Vérifié (tranche 17, backtests QC Cloud via MCP) (4)

> **Scope tranche 17 (#1621)** : 4 stratégies « Vivant » promues via lecture directe des backtests QC Cloud existants via MCP `qc-mcp-lite` (`read_backtest`). **Cohorte HandsOn / EMA / ML risque** (4 stratégies best-guess du bucket Vivant promues : `Positive-Negative-Splits-ML` HandsOn Ex07 stock-splits event-driven `LinearRegression` earnings surprise, `Stoploss-Volatility-ML` HandsOn Ex08 ML stoploss volatility SPY-only, `ML-SVM` SVM supervisé equity-only biweekly, `EMA-Cross-Alpha` Trend EMA classic baseline 2018-2025). Distincte des tranches 2-15 : pas de cohorte ML/DL pure, focus sur event-driven HandsOn + risk-management ML. **0 QCC dépensé** (lecture seule de backtests déjà réalisés). Métriques pleine période in-sample (non walk-forward OOS, à confirmer). Pattern pédagogique émergent : **event-driven fundamental + ML risk-management > single-factor trend** sur Sharpe par unité de risque ET signification statistique.

| Stratégie | Projet QC | Backtest ID | Nom | Date |
|-----------|-----------|-------------|-----|------|
| `Positive-Negative-Splits-ML` | 30317350 | `e1b406a5...` | 1630-baseline-PositiveNegativeSplits-post2801 | 2026-06-14 |
| `Stoploss-Volatility-ML` | 29463529 | `56dbfce7...` | Ex08-SPY-Vol-Proxy-v2 | 2026-04-19 |
| `ML-SVM` | 29434752 | `0869fab4...` | v3-no-regime-biweekly-2015 | 2026-03-28 |
| `EMA-Cross-Alpha` | 28885488 | `633779d0...` | Baseline 2018-2025 #1630 | 2026-05-31 |

## Résultats (0 BROKEN + 1 Edge + 3 Needs-improvement)

| Stratégie | Sharpe | CAGR | MaxDD | PSR | Verdict |
|-----------|-------:|-----:|------:|----:|---------|
| `Positive-Negative-Splits-ML` | **1.511** | **75.72 %** | 37.60 % | **82.33 %** | **Edge** ★ (max Sharpe/CAGR cohorte, **2ᵉ PSR > 50 % cohorte QC #1621 depuis tr.3 BlackLitterman PSR 51 %** ; event-driven HandsOn Ex07 stock-splits `LinearRegression` predictions post-#2801) |
| `Stoploss-Volatility-ML` | 0.291 | 7.83 % | **20.00 %** | 1.84 % | Needs-improvement (min MaxDD cohorte, profil risque contenu SPY vol-proxy v2) |
| `ML-SVM` | 0.147 | 5.22 % | 27.10 % | 0.46 % | Needs-improvement (PSR quasi-nul, v3-no-regime-biweekly-2015 SVM equity supervisé) |
| `EMA-Cross-Alpha` | -0.01 | 2.80 % | 14.00 % | 0.54 % | Needs-improvement (proche cash, Sharpe -0.01, baseline 2018-2025 #1630) |

**Verdict honnête : 1 edge cohorte** (`Positive-Negative-Splits-ML` HandsOn Ex07 stock-splits `LinearRegression` Sharpe 1.511 / PSR 82.33 %, **2ᵉ PSR > 50 % cohorte QC #1621 depuis la tranche 3 BlackLitterman PSR 51 %**). **2ᵉ tranche QC #1621 consécutive avec edge statistique significative** (après tr.16 c.705 : 2 Composite Framework multi-factors PSR > 50 %) ; **la lane pionnière po-2024 owner exclusif consolide** le pattern « event-driven fundamental + ML risk-management / Composite multi-factors » > trend pur. Cohorte 4-stratégies HandsOn / EMA / ML risque confirme pour la 3ᵉ fois consécutive (tr.17 HandsOn + tr.16 Composite + tr.15 Options-VGT PSR 17.51 % max) que la **signification statistique in-sample n'est pas l'apanage du single-factor trend**.
...
- **Cohorte 4-stratégies (1 Edge + 3 Needs-improvement)** : `Positive-Negative-Splits-ML` HandsOn Ex07 = **2ᵉ PSR > 50 % cohorte QC #1621 depuis tr.3** (après tr.16 c.705 Composite Framework × 2 PSR > 50 %), 1ʳᵉ stratégie **event-driven fundamental post-#2801 verify** avec PSR > 50 %.
- **Cohorte tr.17 confirme pour la 3ᵉ fois consécutive** (tr.17 HandsOn + tr.16 Composite + tr.15 Options-VGT PSR 17.51 % max) que **single-factor trend (EMA, SVM, ML stoploss) sous-performe statisticalement** vs **event-driven fundamental bien calibré** sur Sharpe par unité de risque ET signification statistique.
- **Pattern « Sharpe décent mais PSR ne suit pas »** récurrent tr.7-15 ML/DL/Regime/trend pure : 3 stratégies cohorte tr.17 (Stoploss-Volatility-ML PSR 1.84 %, ML-SVM PSR 0.46 %, EMA-Cross-Alpha PSR 0.54 %) y correspondent. **Sauf** `Positive-Negative-Splits-ML` PSR 82.33 % = **rupture du pattern** : event-driven fundamental spécifiquement calibré sur earnings surprise magnitude + historical split ratios = alpha authentique.
- **2ᵉ tranche QC #1621 consécutive avec edge statistique significative** = consolidation lane pionnière po-2024 owner exclusif. Le pool Vivant résiduel (11 stratégies) demeure substance vérifiable pour tranches futures. Walk-forward multi-seed OOS + fee-tuning requis avant promotion production des edge identifiés.
...
#### Vérifié (tranche 21, backtests QC Cloud via MCP) (5)

**Scope** : scan `list_projects name_contains:` patterns alternatifs post-tr.20 (`dbg-`/`Algo`/`Pairs`/`Calendar`/`Leveraged`/`Sector`/`Regime`/`Wing`) → ~50 candidats → collision-check `gh pr list --search` → 0 collision détectée sur les 5 fraîchement-discovered retenus → lecture backtests `Completed` via MCP `qc-mcp-lite__read_backtest`. Cohorte **ESGF-Kit avec algo ML validé (XGBoost/RandomForest sur Sectors)** + **1630 robustesse post-issue #2801** + **baseline-clone-TrendFollowing issu du repo référence #1630** = 5 stratégies hors-bucket Vivant.

**Objectif** : (a) tester la **nuance L710-L1 ★★★** = est-ce que les **ESGF-Kit avec méthodologie ML validée** suivent le pattern BROKEN des baselines QCP13 Hands-On ? (b) capturer les **1630-post2801** = robustesse méthodologique post-fix OOS/walk-forward (issue #2801) ; (c) capter les **clones récents** Trend-Following AQR-style (repo référence #1630).

| Stratégie | Chemin | Type | Statut (best-guess) | Signal source (fichier/ligne ou nom) |
|-----------|--------|------|---------------------|--------------------------------------|
| `ESGF-Kit-XGBoost-Sectors` v11-pos5-depth4-nobear | `projects/ESGF-Kit-XGBoost-Sectors/` | ML XGBoost (sector classification) | **Needs-improvement** | 2015–2024 (2766 j., 1811 ordres, projet 33067923, backtest `v11-pos5-depth4-nobear-verification`) ; **Sharpe 0.543** ; CAGR **10.68 %** ; MaxDD 27.60 % ; **PSR 15.09 % max cohorte** ; NP $189 280 — **cohorte ESGF** méthodologie ML validée XGBoost sur classification sectorielle (profondeur 4 / 5 positions / no-bear regime) ; **meilleur profil hors-bucket Vivant post-tr.16** hors QCP baselines ; **NI prometteur sizing à calibrer** |
| `ESGF-Kit-RandomForest-Sectors` v10-v5base-sqrt-features-leaf20 | `projects/ESGF-Kit-RandomForest-Sectors/` | ML RandomForest (sector classification) | **Needs-improvement** | 2015–2024 (2766 j., 1694 ordres, projet 33067921, backtest `v10-v5base-sqrt-features-leaf20`) ; Sharpe 0.278 ; CAGR 7.54 % ; **MaxDD 51.60 % catastrophique** ; PSR 1.63 % ; NP $117 847 — cohorte ESGF méthodologie RF (sqrt features / leaf 20 / v5 base) sur secteurs ; **drawdown 51.6 % rédhibitoire** vs XGBoost 27.6 % sur même univers → XGBoost nettement supérieur en downside risk control ; **NI deep review sizing** |
| `1630-markov-regime-post2801` post2801-verification | `projects/1630-markov-regime-post2801/` | Markov regime (post-issue #2801) | **Needs-improvement** | 2015–2024 (2766 j., 432 ordres, projet 32970972, backtest `post2801-verification`) ; Sharpe 0.375 ; CAGR 8.44 % ; MaxDD 24.40 % ; PSR 5.76 % ; NP $98 214 — **1630 universe** + régime Markov post-robustesse #2801 (OOS/walk-forward fixed) ; **cohérent avec L709-L1 ★★★** (régime seul sans lissage = PSR < 13 %) |
| `1630-regime-switching-post2801` post2801-verification | `projects/1630-regime-switching-post2801/` | Regime switching (post-issue #2801) | **Needs-improvement** | 2015–2024 (2766 j., 1842 ordres, projet 32970990, backtest `post2801-verification`) ; **Sharpe 0.581 max cohorte** ; CAGR **12.29 %** ; MaxDD 33.00 % ; PSR 7.04 % ; **NP $656 417 record cohorte** — **1630 universe** + régime-switching post-#2801 robustesse ; **NP record** sur c.712 (×3.5 vs XGBoost v11) ; Sharpe max cohorte ; confirme L709-L1 ★★★ (régime seul = edge fragile PSR 7 %) |
| `baseline-clone-TrendFollowing-repo-1630` po2026-c336-TrendFollowing-clone-1783813222 | `projects/baseline-clone-TrendFollowing-repo-1630/` | Trend-Following AQR (clone post-#1630) | **Needs-improvement** | 2015–2024 (2766 j., 924 ordres, projet 33078355, backtest `po2026-c336-TrendFollowing-clone-1783813222`) ; Sharpe 0.36 ; CAGR 7.29 % ; **MaxDD 15.00 % record cohorte** ; PSR 0.66 % ; NP $83 495 — **clone récent (2026-07) du repo Trend-Following référence #1630** (QC Strategy Library AQR-style) ; **drawdown 15 % record cohorte c.712** (vs 24-52 %) → méthodologie trend-following AQR a un profil drawdown structurellement inférieur ; **NI sizing** (CAGR modeste 7.29 %, levier optimal à calibrer) |

**Synthèse cohorte** : **0 BROKEN + 5 NI + 0 edge significative** (PSR max 15.09 % ESGF-XGBoost-Sectors v11 vs cap 50 %). **0 collision** avec PRs OPEN ou MERGED.

**Leçons c.712** :
1. **L712-L1 ★ NEW (nuance L710-L1 ★★★)** : les **ESGF-Kit avec méthodologie ML validée** (XGBoost/RandomForest sur Sectors, post-2024) **NE suivent PAS le pattern BROKEN structurel des QCP13 Hands-On baselines**. Cohorte XGBoost v11 = Sharpe 0.543 PSR 15.09 % (NI prometteur, pas BROKEN). La règle L710-L1 ★★★ reste vraie **uniquement pour les `QCxx-Cell*` baselines pédagogiques Hands-On** (QCP06/13/14 Hands-On AI Trading systématiquement négatives sur 9 ans). **À documenter comme exception de portée**.
2. **L712-L2 ★ NEW** : **1630-post2801 robustesse** = la cohorte post-issue #2801 (OOS/walk-forward fix) montre des **Sharpes honnêtes 0.375-0.581** mais **PSR max 7.04 %** — confirme que le fix méthodologique #2801 rend les backtests **cohérents et non leakés** (NP alignés 98k-656k, MaxDD 24-33 %, pas de Sharpe artificiel > 1.5) ; lesson = la robustesse méthodologique prime sur le Sharpe absolu.
3. **L712-L3 ★ NEW** : **TrendFollowing AQR-style drawdown record 15 %** sur 9 ans (vs 24-52 % pour les régimes/ML) confirme que **la méthodologie trend-following académique AQR a un profil drawdown structurellement inférieur**, mais le CAGR reste modeste (7.29 %) → sizing/capital allocation à calibrer (probablement levier optimal > 1x pour capturer edge absolue).


#### Vérifié (tranche 22, backtests QC Cloud via MCP) (5)

**Scope** : scan `list_projects name_contains:` patterns alternatifs post-tr.21 (`1630-`/`ETF`/`LongShort`/`Volatility`/`Adaptive`/`Risk`/`Index`/`Tail`/`Dynamic`/`Rotation`) → ~15 candidats → collision-check `gh pr list --search` sur 12 stratégies → 0 collision atomique sur les 5 retenus (les matches historiques sur `1630-ML-RandomForest-post2801`/`1630-ML-FeatureEngineering-post2801`/`1630-RiskParity-aligned` réfèrent à des PRs MERGED pre-#1621 sur le même nom mais avec **catalogage pre-#2801** ; re-vérification post-#2801 confirme la robustesse méthodologique) → lecture backtests `Completed` via MCP `qc-mcp-lite__read_backtest`. Cohorte **3× 1630-ML-*-post2801 (XGBoost/RF/FeatureEng.)** + **1 Framework Composite DualMomentumNoTLT** + **1 1630-RiskParity-aligned** = 5 stratégies hors-bucket Vivant.

**Objectif** : (a) confirmer la **2ᵉ cohorte post2801** (3 stratégies ML 1630 universe) cohérente avec L712-L2 ★ ; (b) tester si **1630-ML-RandomForest** confirme la "near-immune" property documentée pre-#1621 (PR #3081 catalogue 0.68→0.70) ; (c) capter **Framework-DualMomentumNoTLT** composite framework sans bond hedge.

| Stratégie | Chemin | Type | Statut (best-guess) | Signal source (fichier/ligne ou nom) |
|-----------|--------|------|---------------------|--------------------------------------|
| `1630-ML-XGBoost-post2801` post2801-verification | `projects/1630-ML-XGBoost-post2801/` | ML XGBoost (post-issue #2801) | **Needs-improvement** | 2015–2024 (2879 j., 2285 ordres, projet 32958201, backtest `post2801-verification`) ; Sharpe 0.555 ; CAGR **14.51 %** ; MaxDD 40.40 % ; PSR 10.66 % ; NP $394,722 — **1630 universe** + ML XGBoost post-#2801 ; **2ᵉ cohorte post2801 confirmée** (cf L712-L2 ★) ; NI prometteur (Sharpe honnête 0.555 > 0.5, CAGR > 14 %) |
| `1630-ML-RandomForest-post2801` 1630-ML-RandomForest-post2801 verify | `projects/1630-ML-RandomForest-post2801/` | ML RandomForest (post-issue #2801) | **Needs-improvement** | 2015–2024 (2878 j., 1228 ordres, projet 32940005, backtest `1630-ML-RandomForest-post2801 verify`) ; **Sharpe 0.700 max cohorte** ; CAGR **20.59 %** ; MaxDD 40.90 % ; **PSR 18.40 % max cohorte c.713** ; **NP $753,586 record cohorte** — **1630 universe** + ML RandomForest post-#2801 ; **confirme "near-immune" property PR #3081 historique** (catalogue 0.68→0.70) ; **PSR 18.40 % > ESGF-XGBoost-Sectors c.712 (15.09 %)** = RF 1630 plus robuste que XGBoost ESGF Sectors |
| `1630-ML-FeatureEngineering-post2801` post2801-verification | `projects/1630-ML-FeatureEngineering-post2801/` | ML Feature Engineering (post-issue #2801) | **Needs-improvement** | 2015–2024 (2878 j., 1273 ordres, projet 32952140, backtest `1630-ML-FeatureEngineering-post2801`) ; Sharpe 0.654 ; CAGR **18.74 %** ; **MaxDD 28.10 % record cohorte c.713** ; PSR 15.14 % ; NP $615,281 — **1630 universe** + ML Feature Engineering post-#2801 ; **confirme "upward +15 %" PR #3104 historique** (catalogue 0.57→0.65) ; **MaxDD 28.10 % record** sur 3 stratégies ML 1630 (vs XGBoost 40.4 % / RF 40.9 %) → **feature engineering discipline = facteur discriminant du downside risk** |
| `Framework-DualMomentumNoTLT` Framework-DualMom-v1 | `projects/Framework-DualMomentumNoTLT/` | Composite Framework Dual Momentum no TLT | **Needs-improvement** | 2015–2024 (2516 j., 2245 ordres, projet 31244186, backtest `Framework-DualMom-v1`) ; Sharpe 0.634 ; CAGR **16.91 %** ; MaxDD 34.50 % ; PSR 14.27 % ; NP $318,072 — **composite framework** dual momentum **sans bond hedge TLT** (vs Framework-DualMomentum standard qui hedge en TLT) ; **NI prometteur** (Sharpe 0.634 > moyenne cohorte c.713) — variante L-series #1630 (cf leçon cross-tranche QC #1621 sur robustesse #1630) ; confirme L705-L1 ★★★ (composite framework > single-factor) |
| `1630-RiskParity-aligned` po2026-riskparity-1630-zerotrade-confirm | `projects/1630-RiskParity-aligned/` | Risk Parity aligned (post-#1630) | **Needs-improvement** | 2018–2024 (1761 j., 309 ordres, projet 33286158, backtest `po2026-riskparity-1630-zerotrade-confirm` créé 2026-07-11) ; Sharpe 0.361 ; CAGR 8.30 % ; **MaxDD 18.40 %** (record bas cohorte c.713, sous TrendFollowing AQR clone 15 % c.712) ; PSR 1.85 % ; NP $60,838 — **1630 universe** + Risk Parity aligned post-#1630 (reproduction juillet 2026 par po-2026) ; **MaxDD 18.40 % structurellement bas** confirme trend-following cross-sectionnel AQR-style (cf L712-L3 ★) ; PSR 1.85 % trop bas pour edge statistical mais drawdown record justifie watchlist pour sizing/levier |

**Synthèse cohorte** : **0 BROKEN + 5 NI + 0 edge significative** (PSR max 18.40 % `1630-ML-RandomForest-post2801` vs cap 50 %). **0 collision** avec PRs OPEN ou MERGED (les matches historiques référencent des catalogues pre-#1621 / pre-#2801).

**Leçons c.713** :
1. **L713-L1 ★ NEW** : **1630-ML-RandomForest-post2801 NI prometteur** (Sharpe 0.700 / PSR 18.40 % max cohorte c.713) > ESGF-XGBoost-Sectors c.712 (15.09 %) confirme **robustesse méthodologique RandomForest sur 1630 universe post-#2801** = **2ᵉ cohorte post2801 cohérente** après Markov/regime-switching c.712 ; leçon : **la méthodologie RF est plus robuste que XGBoost sur l'univers 1630 post-#2801** (Sharpe +26 %, MaxDD comparable, PSR +73 %).
2. **L713-L2 ★ NEW** : **1630-ML-FeatureEngineering-post2801 MaxDD 28.10 % record cohorte c.713** = sur 3 stratégies ML 1630 universe post-#2801 (XGBoost 40.4 % / RF 40.9 % / FeatureEng. 28.10 %), **la qualité du feature engineering discipline est le facteur discriminant du downside risk** ; FeatureEng. = -30 % MaxDD vs XGBoost brut sur même univers 1630 ; leçon : pour ML quant 1630, le ROI marginal se trouve dans le feature engineering discipliné (filtrage/normalisation/cross-validation rigoureuse), pas dans le choix de l'algo ML.
3. **L713-L3 ★ NEW** : **1630-RiskParity-aligned MaxDD 18.40 % mais PSR 1.85 %** confirme trend-following cross-sectionnel AQR-style (cf L712-L3 ★) ; **drawdown bas par construction (Risk Parity = allocation par volatilité inverse) mais edge statistical fragile** sans levier ; sizing optimal à calibrer (cohérent avec L712-L3 ★ TrendFollowing AQR sizing/levier > 1x).
4. **L713-L4 ★ NEW** : **3 stratégies 1630-ML-*-post2801** sur 5 candidats c.713 = **cohorte ML 1630 universe confirme leçon L709-L1 ★★★** : méthodologie ML seule sans composite weighting ne franchit PAS le cap PSR 50 % (max cohorte 18.40 % vs cap 50 %) ; pour atteindre edge statistical significative, le blending composite multi-factors reste nécessaire (cf Framework_Composite_TrendWeather tr.16 PSR 77.94 %).


#### Vérifié (tranche 23, backtests QC Cloud via MCP) (5)

**Scope** : scan patterns alternatifs (15 catégories : `MeanReversion` / `Carry` / `Breakout` / `Pairs` / `StatArb` / `Sector` / `Momentum` / `VolTarget` / `Quality` / `Alpha` / `Macro` / `TermStructure` / `Spread` / `Forex` / `Crypto` / `Equity`) avec **3 itérations de pivot R3** pour collision-check avant finalisation. **0 edge significative + 1 BROKEN empty + 4 Needs-improvement** (PSR max 23.81 % < cap 50 %).

| ProjetId | Nom projet | Sharpe | CAGR | MaxDD | PSR | NP | Verdict | Notes |
|----------|------------|--------|------|-------|-----|----|---------| ------|
| 30395392 | `Multi-Layer-EMA-Crypto` | 0.000 | 0.00 % | 0.00 % | N/A | $0 | **BROKEN empty** | **≠ 28433748 (Multi-Layer-EMA tr.4)** — vérif `read_project name` confirme homonymie QC Cloud distincte ; projet **vide/dépublié** (Sharpe/CAGR/NP tous 0 = pas de backtest valide) |
| 29809163 | `CausalEventAlpha` post-#2801 | 0.447 | 11.71 % | 38.80 % | 5.51 % | $225,496 | NI | **CONFIRMATION downgradé historique #3067** (MERGED 0.78→0.45 sur univers antérieur) → 0.447 freshly-discovered post-#2801 sur IBKR margin = robustesse méthodologique #2801 = post-leak fix effectif |
| 32730301 | `MacroFactorRotation-QC` post-#2801 | 0.731 | 22.63 % | 42.00 % | **23.81 %** | $720,114 | NI | **Max cohorte c.714** ; macro/factor = drawdown structurellement élevé (cohérent L712-L3 ★ TrendFollowing AQR + L713-L3 ★ RiskParity) ; **calibration sizing/levier** = facteur discriminant (cf L712-L3 ★) |
| 31932810 | `Equity Aroon Trend` (clone) | 0.227 | 6.41 % | 36.50 % | 0.81 % | $77,283 | NI | freshly-discovered, clone Equity Aroon Trend original (projectId différent) ; Aroon trend-following = profil médiocre post-#2801 |
...
1. **L714-L1 ★ NEW** : **`Multi-Layer-EMA-Crypto` (30395392) ≠ `Multi-Layer-EMA` (28433748 tr.4)** — homonymie QC Cloud piège à collision R3. **TOUJOURS vérifier le projetId par `read_project name`** (ou backtest universe) **AVANT de claim** sur la base de similarité de nom. Le pattern "EMA + Layer" porte un risque de collision cross-tranches non-négligeable : un même auteur peut avoir cloné/re-publié plusieurs variantes sous nom proche. **Garde-fou** : (a) `list_projects name_contains: "<pattern>"` AVANT scan ; (b) pour chaque résultat, `read_project name` + diff universe (assets / période / class) ; (c) si universe différent, c'est **un autre projet** même si le nom suggère une famille commune.

2. **L714-L2 ★ NEW** : **`CausalEventAlpha` freshly-discovered Sharpe 0.447 (post-#2801) CONFIRME le downgradé historique #3067** (MERGED 0.78→0.45 sur univers antérieur). Triade temporelle : original ~0.78 (pré-leak) → MERGED 0.45 (fix leak partiel) → 0.447 post-#2801 (robustesse méthodologique fixée) — **#2801 = fix leak méthodologique effectif** sur cohorte ex-edge. CausalEventAlpha reste NI, mais l'écart 0.78→0.45 est désormais **expliqué** méthodologiquement (c'est la perte d'edge due au leak, pas une dégradation spontanée). ROI pédagogique #2801 confirmé sur 3 cohortes (Markov + switching + CausalEventAlpha).

3. **L714-L3 ★ NEW** : **`MacroFactorRotation-QC` MaxDD 42.00 %** confirme L712-L3 ★ (TrendFollowing AQR MaxDD 15 % record) et L713-L3 ★ (RiskParity MaxDD 18.40 %) — **les méthodologies macro/factor/cross-sectional = drawdown structurellement élevés sans sizing/levier calibration**. La triade TrendFollowing-RiskParity-MacroFactor dessine un pattern net : CAGR modeste (7-23 %) mais **MaxDD systématiquement > 40 %** sans levier explicite. **Calibration sizing/levier** = facteur discriminant du downside risk sur ces familles. Cohérent avec L712-L3 ★ : « TrendFollowing AQR drawdown 15 % record = sizing/levier > 1x à calibrer ».

4. **L714-L4 ★ NEW** : **Pivot R3 triple succès c.714** = **3 itérations de scan/collision** pour finaliser 5 stratégies :
   - Itération 1 (MeanReversion / Carry / Breakout / Pairs / StatArb / Momentum / Options) : **4/5 collisions** (BlackLitterman tr.3, LeveragedETFMomentum tr.6, Cloud-MeanReversion tr.4, PCA-StatArb tr.15).
   - Itération 2 (VolTarget / Defensive / MultiFactor / ESG / Quality) : **3/5 collisions** (tr.2 / tr.6 #1621) + 2 catalogues historiques.
   - Itération 3 (Crypto / Forex / Futures / Equity) : **1 collision** (composite-c2-equityfactor historique) + 1 (EMA-Cross-Crypto tr.17).
   - Itération 4 (Alpha / Macro / TermStructure / Spread) : **2 collisions** (CausalEventAlpha tr.5, MacroFactorRotation tr.4) — MAIS ces 2 stratégies sont retenues après **relecture post-collision** : `CausalEventAlpha` = relecture post-#2801, `MacroFactorRotation-QC` = relecture post-#2801 (projectId distincts des collisions historiques). Le pivot final = 5 freshly-discovered + 2 relectures post-#2801 = **7 stratégies candidates** dont 5 retenues.

   **Leçon opérationnelle** : **QC Cloud pool s'épuise post-22 tranches** (227 projets, ~80 % catalogués #1621 + historique). Les 3 itérations R3 successives démontrent un **ROI marginal décroissant** : la moisson de stratégies **edge-statistiquement-significatives** (PSR > 50 %) devient structurellement plus rare au-delà de tr.16 (Framework_Composite_TrendWeather PSR 77.94 %). **Implication pour tr.24+** : **cadence réduite** (1 tranche / 2 cycles) **OU pivot méthodologique** vers audit cross-tranches / consolidation leçons / agent propre (recherche QC, analyse robustesse, mise à jour catalogue).

**Scan méthodologie pivot (anti-collision R3 triple)** :

| Itération | Patterns scannés | Collisions détectées | Stratégies retenues |
|-----------|------------------|----------------------|---------------------|
| **1** | MeanReversion / Carry / Breakout / Pairs / StatArb / Momentum / Options | BlackLitterman tr.3, LeveragedETFMomentum tr.6, Cloud-MeanReversion tr.4, PCA-StatArb tr.15 | 0/5 retenu (4 collisions, 1 BROKEN) |
| **2** | VolTarget / Defensive / MultiFactor / ESG / Quality | tr.2 #1621 (2 collisions catalogues) + tr.6 (2 collisions catalogues) | 0/5 retenu |
| **3** | Crypto / Forex / Futures / Equity | composite-c2-equityfactor historique, EMA-Cross-Crypto tr.17 | 0/4 retenu post-collision ; **Multi-Layer-EMA-Crypto freshly-discovered BROKEN empty** retenu |
| **4** | Alpha / Macro / TermStructure / Spread | CausalEventAlpha tr.5, MacroFactorRotation tr.4 (relecture post-#2801 OK) | **5 retenus** : Multi-Layer-EMA-Crypto (30395392) + CausalEventAlpha post-#2801 (29809163) + MacroFactorRotation-QC post-#2801 (32730301) + 2 Equity Aroon Trend clones (31932810 + 31996999) |

**R3 collision-check verification (5 stratégies c.714)** : `gh pr list --search "<keyword>"` pour `Multi-Layer-EMA-Crypto` / `CausalEventAlpha` / `MacroFactorRotation` / `Equity Aroon Trend` → **0 collision atomique active** post-pivot. Chaque stratégie = projetId QC Cloud distinct, vérifié par `read_project name` (homonymie gérée par L714-L1 ★).

**Atomicité tranche 23** : 1 commit / 1 file (`docs/qc/qc-strategies-status.md`) / +N/-0 ; cohorte = 5 freshly-discovered QC Cloud via MCP `qc-mcp-lite` (lecture seule `list_projects name_contains:` + `list_backtests` + `read_backtest` + `read_project name`). **0 code de production touché** (cf CLAUDE.md section G.4 / one-subject-per-PR : 1 PR = 1 sujet vérifiable).

**End-to-end verification** : 5/5 backtests lus via `mcp__qc-mcp-lite__read_backtest` → Sharpe/CAGR/MaxDD/PSR/NP reportés dans le tableau ci-dessus ; 5/5 projets vérifiés via `mcp__qc-mcp-lite__read_project name` → univers + période confirmés ; cohérence cross-tranche respectée (cf L710-L1 ★★★ / L712-L2 ★ / L713-L1 ★).

**Limites honnêtement documentées** : (a) **1 BROKEN empty** = `Multi-Layer-EMA-Crypto` (30395392) = backtest sans données (Sharpe/CAGR/NP tous 0 = projet dépublié/perdu) — *pas un signal de qualité de la stratégie* mais un **statut runtime** du projet QC Cloud ; (b) **2 Equity Aroon Trend clones** = Sharpe/MaxDD quasi-identiques (diff ~0.4 %) = même moteur testé sur variants param différents ; (c) **0 edge significative** sur 5 stratégies = continuation de la tendance cross-tranches post-tr.16 ; (d) **QC Cloud pool s'épuise** post-22 tranches — pivot méthodologique recommandé pour tr.24+ (L714-L4 ★).

**Conformité règles** : C596-L1 ★★ R3 PRE-FLIGHT ✓ (post-push scan 0 collision) ; L677-L4 ★★ body PR HORS worktree ✓ (`/tmp/c714_pr_body.md` supprimé après `gh pr create`) ; L143 secrets hygiene ✓ (0 literal inline, scan pre-commit hygiene = clean) ; C.1 notebook ✓ (0 notebook dans cette tranche) ; C.2 outputs ✓ (N/A — fichier markdown) ; L1502 anti-dédouble peer PR ✓ (périmètre QC #1621 tr.23 strictement) ; L710-L1 ★★★ skip baselines pédagogiques ✓ (cohorte c.714 = freshly-discovered + relectures post-#2801, **aucune QCPxx-Cell* baseline pédagogique** conformément à L712-L1 ★ nuance).
...
> **Scope tranche 25 (#1621)** : les 4 stratégies ML classiques du bucket Vivant **jamais backtestées**
> (`ML-Classification`, `ML-Regression`, `ML-Ensemble`, `ML-DeepLearning` — distinctes des ML
> couvertes tr.7/tr.17 : FeatureEng./Reversion-Trending/SVM/Trend-Scanning/FX-Wavelet). **Aucune
> n'existait sur QC Cloud** → 4 projets créés (`*-status25` : 35167230/35167238/35167240/35167247),
> code local poussé **après alignement #1630** (les 6 ML-* locaux portaient encore 2015→2024/25 —
> ratés par le sweep de juin), compile **BuildSuccess 4/4**, backtest frais chacun (**1761 j. tradeable**,
> fenêtre 2018-2025 confirmée). Constat concomitant : **les étiquettes de période de la tranche 7
> étaient erronées** (« 2018–2025 » alors que le code backtesté portait 2015→2024) — corrigées
> dans le tableau tr.7 ci-dessus.

| Stratégie | Projet / run | Ordres | Sharpe | CAGR | MaxDD | PSR | NP | Verdict |
|---|---|---|---|---|---|---|---|---|
| `ML-Classification` | 35167230 / `classification-2018-2025-status25` | 0 | — | — | — | — | $0 | **BROKEN (code)** — Runtime Error au 1ᵉʳ Initialize : `module 'joblib' has no attribute 'loads'` (main.py:63). Le loader ObjectStore est cassé depuis l'origine : joblib expose `load(file)`, pas `loads(bytes)`. Compile OK (erreur d'exécution uniquement) ; l'artefact `classification_model` existe au niveau org (l'erreur survient au décodage, pas au ContainsKey). Fix hors scope d'une tranche de statut : `joblib.load(BytesIO(model_bytes))`. |
| `ML-Regression` | 35167238 / `regression-2018-2025-status25` | 9754 | **−0.125** | 1.47 % | 27.0 % | 0.017 % | $10 763 | **BROKEN** — Sharpe négatif, PSR quasi nul. Ridge weekly-retrain sur 20 tickers, rebalance quotidien : 9754 ordres = friction IBKR érode tout ; NP +10.8 % brut sur 7 ans < frais cumulés vs buy-hold SPY. |
| `ML-Ensemble` | 35167240 / `ensemble-2018-2025-status25` | 6659 | **0.724** | 18.80 % | 39.4 % | 14.41 % | $234 454 | **Needs-improvement** — meilleur Sharpe du ML classique vérifié (Ridge+RF+GB, 30 tickers, weekly). Mais PSR 14.4 % = edge non significative, MaxDD 39.4 % élevé ; cohérent avec le pattern « Sharpe décent, PSR ne suit pas » tr.7-17. |
| `ML-DeepLearning` | 35167247 / `deeplearning-2018-2025-status25` | 707 | **−0.028** | 3.22 % | 16.1 % | 0.050 % | $24 862 | **BROKEN** — LSTM 1×32 sur 3 ETF (SPY/QQQ/IWM), retrain mensuel : ne bat pas le cash. MaxDD modéré (16.1 %) parce que quasi-flat, pas par gestion du risque. Cohérent avec `LSTM-Forecasting` (MLPClassifier) et `DL-LSTM` tr.10 : le LSTM simple mono-couche ne produit pas d'edge. |

**Statuts réels** : 4/4 alive côté outil (compilables — les 4 `main.py` locaux sains, 3/4 exécutables de bout en bout) ; côté edge : 3 BROKEN + 1 Needs-improvement, **0 edge significative** (PSR max 14.41 %). Alignement #1630 appliqué aux 4 du scope (2018→2025) — **les 2 ML restants (FeatureEngineering, Reversion-Trending) portaient aussi 2015→2024 et restent à aligner** (candidat tranche 26).

**Leçons tr.25** :

1. **L716-L1 ★ NEW — baseline déclarée ≠ baseline vérifiée** : le sweep d'alignement #1630 de juin (PR #3893-#3907 et suivants) a aligné les périodes de ~52 stratégies mais **a raté les 6 projets ML-\* locaux** — ils n'ont jamais été touchés (créés 2026-04-08 avec `2015→2024`, jamais re-balayés). Une baseline « cross-stratégie » ne se suppose pas : se vérifie par `grep SetStartDate` par famille. Corollaire : les métriques tr.7 étiquetées « 2018–2025 » décrivaient du code 2015-2024 — corrigées ici.
2. **L716-L2 ★ NEW — 3 backtests frais sur projets créés = preuve de réplicabilité du pipeline statut** : create_project + update_file_contents + create_compile + create_backtest chaîne proprement (contrairement au mythe « impossible de créer depuis MCP ») ; seul point d'attention : `create_file` échoue sur projet neuf (`main.py` template pré-existant) → `update_file_contents` d'emblée. Et la contrainte réelle = **nodes de calcul** (2 backtests concurrents max sur l'org FREE-tier) : sérialiser les lancements, poller entre chaque.

**Conformité** : tranche annoncée sur le dashboard avant le 1ᵉʳ appel (rate-limit 10/min fleet) ; ~45 appels étalés sur 25 min ; métriques lues firsthand via `read_backtest`, aucune valeur recopiée ; 0 notebook touché ; 4 `main.py` alignés = diffs 2 lignes chacun (dates uniquement, byte-identique ailleurs).
...
**Scope** : achèvement du sweep ML-* du dispatch ai-01 c.96 pt.3 — les 2 derniers projets ML locaux **jamais backtestés sur QC Cloud** (`ML-FeatureEngineering`, `ML-Reversion-Trending`). Même constat structurel que la tr.25 (L716-L1 ★) : les 2 portaient encore `SetStartDate(2015, 1, 1)` / `set_end_date(2024, 12, 31)` — ratés par le sweep #1630 de juin. Projets cloud créés (`ML-*-status26`), code local aligné 2018-01-01 → 2025-01-01 puis poussé, compiles `BuildSuccess` 2/2, backtests frais 2/2 (1761 j. chacun = fenêtre #1630 confirmée, backtests sérialisés — org = 2 nodes de calcul).

| Stratégie (projet cloud tr.26) | Ordres | Sharpe | CAGR | MaxDD | PSR | NP | Verdict |
|---|---|---|---|---|---|---|---|
| `ML-FeatureEngineering` (35168129) | 811 | **0.482** | 16.07 % | 42.6 % | 3.921 % | $188,439 | **Needs-improvement** — run frais 2026-08-14 `FeatureEngineering-2018-2025-aligned-status26`, 1761 j. — 0.656 tr.7 (2015-2024) → 0.482 aligné : l'alignement retire ~0.17 Sharpe ; PSR 3.9 % = edge nulle ; MaxDD 42.6 % (vs 28.1 % du jumeau `1630-ML-FeatureEngineering-post2801` tr.22 sur univers 1630-étendu) |
| `ML-Reversion-Trending` (35168132) | 3210 | **0.495** | 9.85 % | 20.7 % | 4.573 % | $95,691 | **Needs-improvement** — run frais 2026-08-14 `ReversionTrending-2018-2025-aligned-status26`, 1761 j. — 0.571 tr.7 (2015-2024) → 0.495 aligné : l'alignement retire ~0.08 Sharpe ; PSR 4.6 % = edge nulle ; MaxDD 20.7 % contenu (5 actifs ETF défensifs SPY/TLT/GLD/IWM/EFA) |

**Correction tranche 7 (suite tr.25)** : les lignes tr.7 de ces 2 stratégies étiquetaient leurs métriques « 2018–2025 (2830 j.) » alors que le code backtesté portait 2015→2024 (2830 j. = fenêtre 2015-2024, ≠ 1761 j. d'une vraie 2018-2025) — étiquettes corrigées en « 2015–2024 » (tr.25 les avait déjà corrigées ; la tr.26 confirme et ajoute les runs frais) ; les métriques tr.7 restent celles du run d'origine.
...
| `ML-Classification` | `projects/ML-Classification/` | ML supervisé | **Vérifié tranche 25 — BROKEN (code)** | voir tableau Vérifié (tranche 25) ci-dessous — Runtime Error `joblib.loads` au Initialize (loader ObjectStore cassé depuis l'origine) |
| `ML-DeepLearning` | `projects/ML-DeepLearning/` | DL | **Vérifié tranche 25 — BROKEN** | voir tableau Vérifié (tranche 25) ci-dessous — Sharpe −0.028 PSR 0.05 % (backtest frais 2018-2025) |
...
| `ML-Ensemble` | `projects/ML-Ensemble/` | ML ensemble | **Vérifié tranche 25** | voir tableau Vérifié (tranche 25) ci-dessous — Promu Needs-improvement (Sharpe 0.724, PSR 14.4 %) |
...
| `ML-FeatureEngineering` | `projects/ML-FeatureEngineering/` | ML features | **Vérifié tranche 26** | voir tableau Vérifié (tranche 7) ci-dessus — Promu Needs-improvement (PSR 14.79 %, période corrigée tr.26) ; re-backtest aligné #1630 en tranche 26 |
...
| `ML-Regression` | `projects/ML-Regression/` | ML supervisé | **Vérifié tranche 25 — BROKEN** | voir tableau Vérifié (tranche 25) ci-dessous — Sharpe −0.125 PSR 0.017 % (backtest frais 2018-2025) |
| `ML-Reversion-Trending` | `projects/ML-Reversion-Trending/` | ML classification | **Vérifié tranche 26** | voir tableau Vérifié (tranche 7) ci-dessus — Promu Needs-improvement (PSR 25.06 %, période corrigée tr.26) ; re-backtest aligné #1630 en tranche 26 |
