...
## Entry #032 — Probas/Infer.NET 1-19 (owner po-2023, c.739)

> **Note de séquence** : numérotée #032 — suit l'entry #031 (GameTheory .NET twins, PR #7647). Si #7647 merge en premier, la numérotation est séquentielle (#031 GT → #032 Infer.NET) ; si cette PR merge en premier, ai-01 renumérote (le ledger est cumulatif, l'ordre numérique n'a pas d'impact fonctionnel).

| Champ | Valeur |
|-------|--------|
| Famille | `MyIA.AI.Notebooks/Probas/Infer/Infer-{1..19}*.ipynb` (19 .ipynb .NET C# canoniques ; 5 fichiers `*_output.ipynb` gitignored exclus — artefacts d'exécution papermill locaux, non suivis sur `origin/main`) |
| Owner-lane | **po-2023** (Infer.NET = `Microsoft.ML.Probabilistic` C# .NET, turf po-2023 par partition language-based ; distinct du turf po-2025 Probas-Python PyMC entries #005/#018/#028/#030) |
| Cycle | c.739 (2026-07-21) |
| Kernel | `.net-csharp` (Infer.NET via `Microsoft.ML.Probabilistic`) |
| Recensement | `git ls-tree origin/main` (canonical, `_output` exclus) + script python3 sur HEAD `9035e37e6` |
| Ledger entry | `docs/ledgers/3801-sota-axe2.md` (entry #032, format cumulatif) |
| Verdict agrégé | **SOTA-OK** (19/19) — vrai moteur Infer.NET invoqué proprement, modèles non-triviaux |

### Findings détaillés (lecture directe G.1 des 19 notebooks canoniques)

| Nb | md | code | EXEC | C.1 | Constructions Infer.NET | Verdict |
|-----|----|------|------|-----|-------------------------|---------|
| Infer-1-Setup | 29 | 12 | 12/12 | 0 | VArray/Range/ForEach/Factor | **SOTA-OK** |
| Infer-2-Gaussian-Mixtures | 55 | 26 | 26/26 | 0 | VArray/Range/ForEach/If-Switch/Factor | **SOTA-OK** |
| Infer-3-Factor-Graphs | 38 | 15 | 15/15 | 0 | If-Switch/Factor | **SOTA-OK** |
| Infer-4-Bayesian-Networks | 46 | 20 | 20/20 | 0 | VArray/Range/ForEach/If-Switch/Factor | **SOTA-OK** |
| Infer-5-Causal-Inference | 23 | 15 | 15/15 | 0 | If-Switch (do-calculus sur réseaux bayésiens) | **SOTA-OK** |
| Infer-6-Debugging | 33 | 17 | 17/17 | 0 | VArray/Range/Factor | **SOTA-OK** |
| Infer-7-Skills-IRT | 51 | 23 | 23/23 | 0 | VArray/Range/ForEach/If-Switch/Factor (IRT 1PL/2PL + DINA + slip/guess + ROC) | **SOTA-OK** |
| Infer-8-TrueSkill | 40 | 19 | 19/19 | 0 | Factor (système de classement Microsoft, inférence de skill) | **SOTA-OK** |
| Infer-9-Classification | 37 | 18 | 18/18 | 0 | VArray/Range/ForEach/Factor | **SOTA-OK** |
| Infer-10-Model-Selection | 40 | 18 | 18/18 | 0 | VArray/Range/ForEach/If-Switch/Factor (évidence modèle) | **SOTA-OK** |
| Infer-11-Topic-Models | 38 | 17 | 17/17 | 0 | VArray/Range/ForEach/If-Switch (LDA-style) | **SOTA-OK** |
| Infer-12-Modeles-Hierarchiques | 10 | 8 | 8/8 | 0 | VArray/Range/ForEach/Factor | **SOTA-OK** |
| Infer-13-Crowdsourcing | 36 | 14 | 14/14 | 0 | VArray/Range/ForEach/If-Switch/Factor (modèle Dawid-Skene) | **SOTA-OK** |
| Infer-14-Sequences | 41 | 21 | 21/21 | 0 | VArray/Range/ForEach/If-Switch/Factor (HMM) | **SOTA-OK** |
| Infer-15-Recommenders | 55 | 26 | 26/26 | 0 | VArray/Range/ForEach/Factor (Matchbox recommender) | **SOTA-OK** |
| Infer-16-Sparse-Gaussian-Process | 19 | 12 | 12/12 | 0 | VArray/Factor (GP creux) | **SOTA-OK** |
| Infer-17-Kalman-Filter | 11 | 7 | 7/7 | 0 | VArray/Range/ForEach/Factor (filtrage Kalman) | **SOTA-OK** |
| Infer-18-Change-Point | 14 | 9 | 9/9 | 0 | VArray/Range/ForEach/If-Switch/Factor | **SOTA-OK** |
| Infer-19-Survival-Analysis | 17 | 10 | 10/10 | 0 | Range (modèle de survie) | **SOTA-OK** |
| **TOTAL** | **633** | **307** | **307/307** | **0** | Infer.NET natif (Variable/VArray/Range/ForEach/If-Switch/Factor/infer) | **SOTA-OK 19/19** |

- **EXEC_PROVED** : 307/307 (100%) — `execution_count != null` sur les 307 cellules code des 19 notebooks canoniques.
- **Erreurs runtime** : 0.
- **Violations C.1** : 0 (regex `raise NotImplementedError|assert False|1/0` = 0 hit sur les 19 notebooks).

### Vrais outils SOTA invoqués (vérifiés G.1 constructions Infer.NET natives)

- **Microsoft.ML.Probabilistic (Infer.NET)** — vrai moteur d'inférence probabiliste SOTA, invoqué via ses constructions natives distinctives :
  - **`Variable.*`** primitives (`Variable.Gaussian`, `Variable.Bernoulli`, `Variable.Beta`, `Variable.Array`, `Variable.Random`) — variables aléatoires du modèle.
  - **`VariableArray` / `Range` / `Variable.ForEach`** — *plates* (réplication de sous-graphes), construction centrale d'Infer.NET pour les modèles indexés multi-individus (étudiants IRT, joueurs TrueSkill, items, documents topic-models).
  - **`Variable.If` / `Variable.Switch`** — *stochastic branching* (mélange de modèles, gating) — construction distinctive Infer.NET pour les modèles à structure conditionnelle (Gaussian Mixtures, Change-Point, Crowdsourcing Dawid-Skene, Model-Selection).
  - **`Factor.*`** — facteurs personnalisés (extension du langage de modèles).
  - **`infer()`** — moteur d'inférence (EP/VMP) exécuté.
- Contrairement à l'entry #031 GameTheory (23/24 INTRINSIC faute de Gambit.NET équivalent), Infer.NET **EST** un vrai moteur SOTA disponible et invoqué — verdict SOTA-OK direct (pas INTRINSIC).

### Prong-B — problèmes non-triviaux (DISCRIMINATING)

Les 19 notebooks démontrent Infer.NET sur des modèles **structurellement non-triviaux** où l'inférence variationnelle (EP/VMP) est la capacité distinctive — pas de baseline triviale :

- **Infer-7-Skills-IRT** (le plus riche, 51 md / 23 code) : modèles **IRT 1PL/2PL** (difficulté/abilité) + **DINA** (compétences discrètes, slip/guess) + courbes **ROC** + relations **many-to-many** via `Subarray` + estimation slip/guess par simulation + comparaison priors informatifs vs non-informatifs. Problème multi-étudiants/multi-questions (`Range nEtudiants/nQuestions`) — l'inférence de capacité latente n'a pas de baseline triviale.
- **Infer-8-TrueSkill** : système de classement Microsoft (inférence de skill gaussienne + dynamique de performance), modèle signature d'Infer.NET (son cas d'usage industriel historique sur Xbox Live).
- **Infer-13-Crowdsourcing** : modèle de **Dawid-Skene** (estimation de la fiabilité des annotateurs + vraie étiquette latente) — `If/Switch` pour le mécanisme de confusion.
- **Infer-17-Kalman-Filter** : filtrage de Kalman (inférence d'état latent temporel).
- **Infer-11-Topic-Models** : topic models de type LDA (`ForEach` plates sur documents/mots/sujets).

**Aucun cas dégénéré** : chaque notebook pose un modèle graphique à variables latentes où l'inférence (EP/VMP) discrimine — l'estimation de skill/abilité/fiabilité/sujet latent n'a pas d'équivalent trivial.

### Conformité aux règles

| Règle | Statut | Preuve |
|-------|--------|--------|
| C.1 (pas d'erreur volontaire) | **CONFORME** | 0 violation sur 19 notebooks (regex) |
| C.2 (notebooks AVEC outputs) | **CONFORME** | 307/307 exec_count!=null (null=0) |
| Anti-regression | **CONFORME** | Audit read-only, 0 code notebook modifié |
| SOTA Prong A (5 verdicts) | **CONFORME** | SOTA-OK direct (vrai Infer.NET natif, pas INTRINSIC — moteur SOTA disponible et invoqué) |
| SOTA Prong B (non-trivial) | **CONFORME** | DISCRIMINATING (modèles latents IRT/TrueSkill/Dawid-Skene/Kalman/LDA — inférence = capacité distinctive) |
| Stop & Repair (secrets §6) | **CONFORME** | audit read-only, 0 sortie touchée |

### Notes de vérification G.1

- **Recensement worker firsthand** (script python3 sur `origin/main` HEAD `9035e37e6`, canonical `_output` exclus) : 19 notebooks, 633 md, 307 code, **307/307 EXEC_PROVED**, 0 err, 0 C.1. Constructions Infer.NET détectées : Variable/VArray/Range/ForEach/If-Switch/Factor/infer sur 19/19.
- **Fausse piste évitée** : 5 fichiers `Infer-{2,5,8,9,12}*_output.ipynb` détectés sur disque (numéros incohérents 5/8/9/12 vs canoniques 7/10/11/15 = résidus de renumérotation) avec une cellule `ec=null` dans `Infer-2-Gaussian-Mixtures_output`. **G.1 firsthand `git cat-file -e origin/main:...`** : ces 5 fichiers **NE sont PAS sur `origin/main`** (path exists on disk, not in origin/main) et sont **gitignored** (`git check-ignore` match) — artefacts d'exécution papermill locaux, pas du repo. Rien à cleanup (bon réflexe anti-suppression : ne pas agir sur des artefacts locaux).
- **Deep-read cellules modèles** : Infer-7-Skills-IRT (51 md / 23 code, multi-modèles IRT+DINA+ROC+slip/guess+many-to-many, Range nEtudiants/nQuestions) confirmé non-trivial firsthand.

### Owner-lane volet

**po-2023** (Infer.NET = C# .NET, turf po-2023 par partition language-based). po-2025 possède le turf Probas-Python (PyMC, entries #005/#018/#028/#030) — **distinct** de cette famille Infer.NET .NET. Audit consultatif additif (0 code modifié), safe owner-lane.

### Conclusions audit

- **Probas/Infer.NET 1-19 = substance solide 19/19**, SOTA-OK direct : vrai moteur `Microsoft.ML.Probabilistic` (Infer.NET) invoqué via ses constructions natives (Variable/VArray/Range/ForEach/If-Switch/Factor/infer). Prong-B DISCRIMINING (modèles latents IRT/TrueSkill/Dawid-Skene/Kalman/LDA — l'inférence variationnelle est la capacité distinctive). **Pas de fix nécessaire** : audit = SOTA-OK 19/19, 0 PR de substance.
- **Continuité c.739** : post-c.738 (GameTheory .NET entry #031), R6 variété → pivot famille Probas/Infer.NET (.NET probabilistic, distinct du turf po-2025 PyMC Python et de GameTheory). STEER ai-01 c.~1:52Z (substance rouvre, varier familles, CAP prover-guards/MGS saturé) appliqué : audit MED registre axe-2 famille non-documentée (entries #001-#031 ne couvraient pas Infer.NET).
- **Cumulatif** : entry #032 = **famille Probas/Infer.NET COMPLÈTE 19/19** dans le registre axe-2. Complète entries #001-#031 ; couvre désormais les 2 familles probabilistes (PyMC Python #005/#018/#028/#030 + Infer.NET #032).

Part of #3801, See #4956
