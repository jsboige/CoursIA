...
> **Statut.** Document de cadrage formel, grade **C-documentaire** (cadrage, pas de dispatch expérimental). Consolide la **dimension D1** de la strate 7, à côté de la **dimension D2** (5 bancs d'essai contrôlés [#7746](https://github.com/jsboige/CoursIA/issues/7746) — MERGED, ICT-26 → ICT-30) et de la **dimension D3** ([#7747](https://github.com/jsboige/CoursIA/issues/7747) — cadrage narratif *boussole de la strate 7*, livré en PR [#9579](https://github.com/jsboige/CoursIA/pull/9579) MERGED c.1243, [`strate7-boussole-myth.md`](strate7-boussole-myth.md)). Vocabulaire aligné sur la source de vérité unique des strates ([`ICT-0-Framing.md` § « Deux axes de lecture »](../../MyIA.AI.Notebooks/IIT/ICT-Series/ICT-0-Framing.md#deux-axes-de-lecture--strates-verticales--tresse-transverse)) : *une jambe (C1-C5) est une expérience sur une strate ; une dimension (D1-D3) cadre l'horizon des strates hautes*. **D2 est une dimension, et ses cinq notebooks sont des pattes, pas des barreaux** : ils **alimentent** la strate 7 sans l'occuper — la strate 7 (freebits d'ordre 2 et réversibilité agentique) **n'est pas livrée par ICT-26 → ICT-30** (divergence 3 de [#13908](https://github.com/jsboige/CoursIA/issues/13908)). D1 et D3 sont **complémentaires, pas redondants** : D1 fixe *le formalisme* (variables libres, jeu évolutif, 6 proxys, dette d'irréversibilité), D3 fixe *la boussole narrative* (deux cascades d'investissement performatif, les trois verbes comme variables libres, mechanism design schmidhuberesque). Le présent document tient au grade C : **nommé sans démonstration**, posé pour cadrer, jamais revendiqué comme résultat.
...
Scott Aaronson (cf. *« The Ghost in the Quantum Turing Machine »*, 2013, *Theoretical Computer Science*) a proposé le **freebit**. *Renvoi retiré (2026-09-20, #16777)* : la version précédente écrivait « repris dans les notebooks *Mechanism Design* de la série ICT » — **renvoi fabriqué sur ses deux jambes**. Aucun fichier de `IIT/ICT-Series/` ne contient `Mechan` (les trois notebooks *Mechanism Design* appartiennent à **GameTheory**), et **aucun `.ipynb` du dépôt ne contient le mot `freebit`** (`git grep -il freebit -- '*.ipynb'` → vide) : le terme ne vit que dans la **prose** `.md`. Le cadrage auquel cette section appartient est donc son propre porteur dans le dépôt ; pour la transposition hors du dépôt, voir [`magnifica-humanitas-dialogue.md`](../magnifica-humanitas-dialogue.md) (§ freebits de second ordre). comme un bit de valeur que l'agent ne peut pas connaître *avant* l'acte de mesure — une incertitude *sur la valeur* dans un **espace déjà fixé**. La strate 7 d'ICT pose une question **strictement plus forte** : l'incertitude ne porte pas sur une valeur dans un espace fixé, mais sur **l'espace lui-même**.
