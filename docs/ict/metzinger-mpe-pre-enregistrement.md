...
## 6bis. Amendement pré-exécution v1 → v2 (scellé avant toute exécution du banc)

**Raison : plafond structurel du bras phasique v1 — deux fois mesuré analytiquement, jamais sur données.** La spécification §4 appariait le budget **par consommateur** : à masse de réduction de seuil égale, le gain en probabilité d'adoption étant concave en z-score, la concentration sur la fraction `f = s/K` de lecteurs du canal boosté borne `max_k g_k ≈ 1 + f·(z-boost doublé)` — de l'ordre de **1.6**, sous la bande P3 (≥ 2.0), pour toute la zone utile du plan de base (`propa ∈ [0.15, 0.40]`). C'est le défaut exact que la case 15 a documenté pour sa P1 « strictement décroissante au plafond » : un critère insatisfiable par construction n'est pas une prédiction. La jambe de substance (double dissociation tonique-uniforme vs phasique-hiérarchisé) est **re-spécifiée, pas re-seuillée** : les bandes P1/P2/P3, les graines, les portes instrumentales et les verdicts restent inchangés.

1. **Bras phasique v2 (compétition inter-contenus, appariement par contenu moyen)** : à chaque essai, **deux contenus** sont injectés simultanément (canaux dominants `k₁ ≠ k₂`). Le module de saillance phasique — orienté-contenu, bottom-up, comme l'alerte phasique §3.2 du papier — booste uniquement les consommateurs lisant le canal dominant du contenu **le plus saillant** (score de saillance intrinsèque `s_k` par canal, tiré une fois par graine, ordre stable ; saillance d'un contenu = `s_{k_dominant}`). **Appariement** : le module ne servant qu'un contenu sur deux, sa masse par contenu servi est doublée à budget total constant — `Δ_ph = 2 · Δ_to · N / N_lecteurs(k_gagnant)` (le coût d'opportunité de la sélection : le tonique, lui, sert tous les contenus, uniformément, sans jamais les lire). `g_k = propa(contenus de canal k | module actif) / propa(contenus de canal k | module inactif)` : la hiérarchie phasique vit dans `P(k gagne la compétition)`, croissante en `s_k` — la **sélection** est orientée-contenu, le tonique reste **sans contenu**.
2. **Adoptions indépendantes par contenu** en mode compétition : chaque consommateur évalue chaque contenu séparément contre le seuil (il peut adopter les deux, un seul, aucun) ; la seule interaction entre contenus est l'allocation du boost. Simplification déclarée.
3. **Null (a) requalifié en contrôle mécanique d'implémentation** : le bras « sans étage tonique, couplage apparié » revient à ne rien moduler — `R_alert = 1.0` exact attendu (la parade aux bugs de pipeline : aucune masse de réduction → aucun gain). C'est l'analogue du `D({tout}) = 0` mécanique de la case 14, pas un null informatif ; le poids discriminant repose sur P1 (existence du gain) et la conjonction P2+P3 (structure du gain), comme déjà dit en §6.3.
4. **Paramètres du jouet scellés avec calibration gelée** (graines de calibration `11, 22, 33` — disjointes des graines de test) : `K = 4` canaux, `N = 48` consommateurs lisant chacun `s = 2` canaux (tirés uniformément, stables par graine), `σ_read = 0.60`, contenu **unicast** (signal d'amplitude 1.0 sur son canal dominant, 0 ailleurs), score d'adoption `A_j = Σ_{k ∈ lus_j} (l_{jk}·b_k)/σ_read`, adoption si `A_j > θ_eff`. Calibration sur graines non-test : `θ` visant `propa(τ bas) ≈ 0.25`, puis `Δ_to` visant `R_alert(tonique) ∈ [1.4, 1.8]` (milieu de bande P1) — **gelés avant le run principal** (le pattern `ε_self` de la case 4 : la calibration ne lit jamais les graines de test).

## 7. Ce qui suit

**Statut au 2026-09-21 : EXÉCUTÉ — verdict SUPPORTED (5/5 graines).** Après l'amendement v2 ci-dessus (scellé avant toute exécution), la calibration a été gelée sur les graines disjointes (11/22/33 : θ = 2.0, Δ_to = 0.7, propa(τ bas) médiane 0.245, R_alert médiane 1.603) puis le run principal a exécuté les graines de test : R_alert médiane **1.619** ∈ [1.2, 3.0] (P1 ✓), uniformité tonique 1.040–1.108, 5/5 ≤ 1.25 (P2 ✓), hiérarchie phasique 2.087–2.210, 5/5 ≥ 2.0 (P3 ✓ — marge mince, min 2.087), aucune porte instrumentale déclenchée, contrôle mécanique exact 5/5. Banc : `MyIA.AI.Notebooks/IIT/ICT-Series/ict/tonic_alertness_gain.py`, tests `ict/tests/test_tonic_alertness_gain.py` (10/10), mesures `ict/results/tonic_alertness_gain_results.json`. Observation instrumentale rapportée : la porte plafond (propa_haut > 0.95) est **dormante dans la zone utile** du jouet unicast (les non-lecteurs du canal dominant restent à adoption ~0.5) — elle n'a été tirée qu'en vérification de branche (seuil négatif profond).
...
2. Le banc `MyIA.AI.Notebooks/IIT/ICT-Series/ict/tonic_alertness_gain.py` + tests vivront dans une PR séparée, avec ligne matrice case 16 mise à jour au statut `TESTÉ (...)`. *(Fait — même PR que le scellé, commits ordonnés : la relation de parenté des commits porte l'antériorité, leçon case 8c.)*
