...
Le document companion [« La mer qui monte »](../grothendieckian-lens.md) (issue [#7299](https://github.com/jsboige/CoursIA/issues/7299)) lit cette falsification comme une **classe d'obstruction non nulle** au recollement : on était parti chercher une section globale, et l'on a trouvé une classe de cohomologie `H¹ ≠ 0`. Cette grille en tire trois régiments de lecture, distincts et complémentaires, que chaque substrat déjà instrumenté (tri auto-organisé, réaction-diffusion, Axelrod, grokking, Jeu de la Vie) peut traverser l'un après l'autre.
...
- **Scalaire universel falsifié** (strate 5) : la non-existence d'une section globale recollant tous les proxys = une classe d'obstruction non nulle. *C'est le résultat, pas un manque.*
