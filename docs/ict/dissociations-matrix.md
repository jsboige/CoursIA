...
| **ICT-15d** | La cochaîne de Čech pondérée discriminate les substrats (`NON_TRIVIAL` = les proxys ne se recollent pas en 1D) | `s` ✓ | `mean_coboundary`, `s2_over_s1`, `effective_rank`, verdict `TRIVIAL`/`NON_TRIVIAL` | Sanity check + 4 substrats ICT-15c | 4 substrats × 3 proxys × fenêtrage | reproduced_in_toy_model | **Spéculatif** | **Mesure courante (post-[#9792](https://github.com/jsboige/CoursIA/pull/9792)) : 4/4 substrats `NON_TRIVIAL`** — gray_scott (s2/s1=0.3418, rank 2), axelrod (0.5976, 2), grokking (0.7649, 3), may (0.5992, 3). **Péremption datée** : l'ancienne rangée « 0/4, tous `TRIVIAL` (s2/s1=0, rank=1) » avait mesuré un module de sensibilité défaillant — fixes `sensitivity.py` [#9770](https://github.com/jsboige/CoursIA/pull/9770)/[#9766](https://github.com/jsboige/CoursIA/pull/9766)/[#9776](https://github.com/jsboige/CoursIA/pull/9776), re-exécution #9792 ; le rang plafonné à 1 était l'artefact du bug, pas une propriété des substrats. **Réserve de non-discrimination** : `axelrod` affiche `mean_cocycle=0`, `obstruction_ratio=0`, `sign_consistency=0` — un verdict `NON_TRIVIAL` sur un cocycle exactement nul ne soutient pas la claim : le verdict est dominé par la SVD (`s2_over_s1`, `effective_rank`) et ne consulte ni le nerf ni H1. Arbitrage de la claim au zoo : au user (hors scope). See [#7744](https://github.com/jsboige/CoursIA/issues/7744) (CLOSED) |
