...
**Statut :** analyse first-hand + plan d'attaque, **PAS de code de notebook** ce cycle ([CLAUDE.md § G.2](../../CLAUDE.md) métriques honnêtes + réponse écrite nominative)
...
Le **point clé** ([CLAUDE.md § G.1](../../CLAUDE.md) vérif biblio c.741 — arXiv abstracts + textes complets via fetch) :
...
## 3. Inventaire des notebooks existants preflight first-hand
...
## 4. Sources canoniques [bibliography-hygiene](../../.claude/rules/bibliography-hygiene.md)

| Réf | Type | Source (arXiv ID ou DOI vérifié [CLAUDE.md § G.1](../../CLAUDE.md) c.741) | Année | Substantif |
|---|---|---|---|---|
...
| **R7** | pub | Heskes, Sijben, Bucur, Claassen, [arXiv:2011.01625](https://arxiv.org/abs/2011.01625) — *Causal Shapley Values: Exploiting Causal Knowledge to Explain Individual Predictions* (NeurIPS 2020, **4 auteurs**) | 2020 | **Causal Shapley Values** par do-calculus $v(S) = \mathbb{E}[f(X) \mid do(X_S = x_S)]$, séparant contributions directes vs indirectes ; adaptable au DAG et à l'estimand visé (ATE, ATT, CDE). **Référence opérationnelle du pont Shap↔do sous DAG**. Note : le cadrage c.655 citait « Heskes et al. 2020 §4.2 » avec une attribution erronée (5 auteurs incluant Kappen — Kappen absent du papier réel) |

[bibliography-hygiene](../../.claude/rules/bibliography-hygiene.md) : PDF archivés hors Git (GDrive `G:\Mon Drive\MyIA\IA\Bibliographie IA\`). R6 et R7 à archiver premier cycle c.656 si non déjà présents (vérifier GDrive).
...
- Création du notebook `XAI-Shap-Attribution.ipynb` (kernel `coursia-ml-training`) :
  ...
  - **Section 2 — Tree SHAP** : `shap.TreeExplainer` sur RandomForest.
    - Additivité vérifiée à 1e-16 (acceptance).
  ...
  - **Section 5 — Jonction Shap ↔ do-calculus** (amend, ne plus porter la formulation caduque « conditional ≅ do ») :
    - DAG explicite : **confondeur classique** $Z \to X$, $Z \to Y$, $X \to Y$ (Z est un parent commun de X et Y → confondeur backdoor). **À ne pas confondre avec le DAG $X \to Y \leftarrow Z$** (Y serait alors un **collider**, Z ne serait pas un confondeur, et $P(Y \mid X) = P(Y \mid do(X))$ trivialement, ne démontrant rien — Concern 2 du reviewer NanoClaw c.662).
...
- Commit sans push (JAMAIS push muet → attendre re-exec SUCCESS).
...
- **Cycle 1** (c.655) : cadrage stratégique + claim posé — pas de code (+ impossible sans analyse first-hand).
...
Estimation [CLAUDE.md § G.2](../../CLAUDE.md) métriques honnêtes : 3 cycles cron (≈90 min) au total.

## 7. [bibliography-hygiene](../../.claude/rules/bibliography-hygiene.md)
...
## 8. Acceptance revue P2 ([CLAUDE.md § G.2](../../CLAUDE.md) métriques honnêtes)

- [ ] Notebook Python (kernel `coursia-ml-training`) exécuté localement Papermill 0 erreur.
- [ ] Sorties réelles commises (règle C.2 — pas de scrub [secrets-hygiene § regle 6](../../.claude/rules/secrets-hygiene.md) JAMAIS hand-edit).
- [ ] ≥ 2 visualisations SHAP (summary plot + force plot local) — [sota-not-workaround](../../.claude/rules/sota-not-workaround.md).png natif.
...
**Estimation [CLAUDE.md § G.2](../../CLAUDE.md)** : ces acceptance se mesurent en 3 cycles cron (90 min total), pas en un seul cycle de 30 min.

## 9. Conformité tells c.655

- counter maintenu : 0 merge / 0 close d'autrui (strict worker).
- réponse écrite nominative (claim #16616 + ce cadrage).
- JAMAIS rerun/re-push ripe merge post-DWELL respecté.
- [secrets-hygiene § regle 6](../../.claude/rules/secrets-hygiene.md) JAMAIS hand-edit respecté.
- single-lane --force-with-lease OK.
- pool narrow-cache hostile sustained.
- R1/G-VAR-1 HELD tenu LIVRÉ #16665 tient G-VAR-1.
- preflight first-hand ×108ᵈ sustained.
- voie L3 update-branch stale-guard-red acquis.
- pivot WSL Ubuntu règle env maintenu.
- cron `51dd3e19` 17,47 * * * armé maintenu.
- [bibliography-hygiene](../../.claude/rules/bibliography-hygiene.md) : PDF R1-R5 archivés hors Git dans GDrive.
- [CLAUDE.md § G.2](../../CLAUDE.md) métriques honnêtes : cadrage + plan, pas de « DONE » sans re-exec SUCCESS.

## Suite c.656

Si build WSL Knots.Basic SUCCESS et quiescence narrow-cache hostile :
...
**Erreur reconnue** : la formulation c.655 présentait « la Shapley **conditionnelle** approche **l'effet causal** $do(X=x)$ » comme la thèse centrale. **Inversion de Janzing 2020** [R6], où c'est l'inverse : la **marginale** coincide avec l'intervention, et la **conditionnelle** est observée et contamine l'attribution par les corrélés. [CLAUDE.md § G.9](../../CLAUDE.md) posture humble fondateur.
...
**[CLAUDE.md § G.1](../../CLAUDE.md)** : les ajouts R6/R7 et la nouvelle structure §5 n'ont **pas** été ré-exécutés (cycle 1 = cadrage, pas code) — la vérif de leur mise en œuvre effective viendra au cycle 2 (c.656, exécution Papermill).
