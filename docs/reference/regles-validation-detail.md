...
- **`strip-dotnet-nuget-ext`** — strip automatique du message `Loading extensions from \`<NuGet cache>\`` (data["text/plain"] output du kernel `.NET Interactive` quand `#r "nuget:..."` charge une extension interactive). Le chemin cache NuGet local (`C:\Users\<user>\.nuget\packages\...`) leak le username machine (PII-lite, cf #6443). Même classe category-A kernel-injected que `probeAddresses` : re-injecté à chaque kernel re-exec → fix durable = strip output-only pre-commit. Signature FP-free (8/8 occurrences portent un path user-profile, 0 FP sur 53 notebooks leak). Strip CI/standalone : `python scripts/notebook_tools/strip_machine_paths.py --scan-all --check` (exit 1 si résiduel), `--apply-all` (fix repo-wide). Tests : `pytest scripts/notebook_tools/tests/test_strip_machine_paths.py` (17/17 PASS). Fix structurel : voir #6529.
