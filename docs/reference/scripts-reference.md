...
| `scripts/coordination/debt_ledger.py` | **Reducteur des deux ledgers de dette partages** (`issue-debt`, `pr-actions`) : schemas versionnes, validation stricte des observations (UTC explicite, provenance obligatoire, cles inconnues refusees), fusion par champ « plus recente observation compatible gagne » avec provenance et historique, contrat de checkpoint archive-aware (un export `incremental` sans snapshot precedent leve `MISSING_CHECKPOINT`), regle de tete PR (une tete deja remplacee ne revient pas et ses champs `head_bound` sont refuses en `stale_head`). La lecture se fait dans la **forme du producteur**, pas dans une forme inventee : l'export `roosync_dashboard read` imbrique son journal (`data.intercom.messages`) et son auteur est un objet (`machineId:workspace`). Le **transport partage n'est pas un fichier** : observations = messages append-only des dashboards dedies `CoursIA-issue-debt-ledger` / `CoursIA-pr-action-ledger`, snapshot ecrit par ai-01 seul en section `status` — jamais d'ecriture sous `$ROOSYNC_SHARED_PATH` (Drive : pas de verrou, `assert_local_output` refuse, sans override). CLI `init` (dry-run par defaut) / `append` (imprime l'appel MCP, n'ecrit rien) / `reduce` (artefacts locaux atomiques sous verrou). Schema, contrat et metriques : [scripts/coordination/README.md](../../scripts/coordination/README.md) ; lectures : `scripts/tests/test_debt_ledger.py` |
