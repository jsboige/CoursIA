...
|---|---|---|
| **Hermes** | Cron intercom review, fleet ping, patrol erreurs, PR review fallback | `clusterManager-Myia` (compte **partagé** avec NanoClaw — résolu 2026-09-08 #3219) |
| **NanoClaw** | Audit à 60-83 % de faux positifs (cf. `audit-reassessment-findings.md`) | `clusterManager-Myia` (même compte, distingué par préfixe `[NanoClaw]` dans le body de review) |
...
- **Annuaire principal** : [`docs/harness/reference/bots-directory.md`](https://github.com/jsboige/roo-extensions/blob/main/docs/harness/reference/bots-directory.md)
  - Identité GitHub résolue (#3219)
  - Schéduler Python `hermes-agent` (`github.com/jsboige/hermes-agent`, fork de `NousResearch/hermes-agent`) — `cron/scheduler.py`
  - Distribution des minutes des 183 reviews `[Hermes]` mesurée (po-2025, échantillon 200 PRs, fenêtre 20/09)
  - MAJ continue via commits datés, le plus récent : `45294ef0` (2026-09-20T22:56Z) — fenêtre 20/09, ratio ~1.2:1, Hermes 6/6 formels
...
Suite au commentaire user 2026-08-29 sur #13472 (« graphviz pas installée, les graphes de facteurs ne sont pas générés, les 2 bots n'ont pas fait un travail de review suffisamment attentif »), PR #14254 (`docs(pr-review,#13517): §D.6 — vérifier le verdict Output-failure ratchet (base vs PR)`, merged 2026-09-02T18:25:52Z par po-2027) a ajouté la règle §D.6 dans `.claude/rules/pr-review-discipline.md` :
...
- `docs/reference/audit-reassessment-findings.md` — NanoClaw false positive patterns (60-83 % mesuré)
- `docs/reference/pr-review-context.md` — règle §D.6 + incidents fondateurs #3473/#11685/#13517
- Issue #13517 (CLOSED) — origine du résidu
- Issue #16635 (cette issue) — résolu par ce pointeur
