...
docker build -t coursia/genai-orchestrator:latest .

# Démarrage du service
docker run -d \
  --name coursia-orchestrator \
  --network genai-network \
  -p 8193:8193 \
  -v /var/run/docker.sock:/var/run/docker.sock:ro \
  coursia/genai-orchestrator:latest
```

### Intégration avec Docker Compose

L'orchestrator est intégré dans l'écosystème via le docker-compose principal (obsolète) ou peut être utilisé comme service standalone.
...
**Version** : 1.0.0  
**Statut** : En développement - Intégration genai-auth en cours  
